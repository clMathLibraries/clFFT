...
public:
  ...
  Supported_Fft_Sizes_Callback() : max_mixed_radices_to_test(4096) {
      ...
      while (1) {
        ...
        while (1) {
          ...
          while (1) {
            ...
            if (sum > max_mixed_radices_to_test)
              break;
...
INSTANTIATE_TEST_CASE_P(mixed_radices_postcallback, mixed_radix_postcallback,
                        ::testing::ValuesIn(supported_sizes_callback.sizes));
...
void mixed_radix_complex_to_complex_precallback(size_t problem_size) {
  try {
    if (verbose)
      std::cout << "Now testing problem size " << problem_size << std::endl;
...
TEST_P(mixed_radix_precallback,
       single_precision_complex_to_complex_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_complex_to_complex_precallback<float, cl_float, fftwf_complex>(
      problem_size);
}

TEST_P(mixed_radix_precallback,
       double_precision_complex_to_complex_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_complex_to_complex_precallback<double, cl_double, fftw_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_postcallback,
       single_precision_complex_to_complex_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_complex_to_complex_postcallback<float, cl_float, fftwf_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_postcallback,
  ...
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_complex_to_complex_postcallback<double, cl_double, fftw_complex>(
      problem_size);
}
...
void precall_normal_1D_forward_in_place_complex_to_complex_userdatatype() {
  ...
  precallback_complex_to_complex<T, cl_T, fftw_T>(
      pattern, direction, lengths, batch, input_strides, output_strides,
      input_distance, output_distance, in_layout, out_layout, placeness, 1.0f,
      true);
}

TEST_F(accuracy_test_callback_single,
       precall_normal_1D_forward_in_place_complex_to_complex_userdatatype) {
  try {
    precall_normal_1D_forward_in_place_complex_to_complex_userdatatype<
...
void precall_lds_1D_forward_64_in_place_complex_to_complex() {
  ...
  data_pattern pattern = impulse;
  precallback_complex_to_complex_lds<T, cl_T, fftw_T>(
      pattern, direction, lengths, batch, input_strides, output_strides,
      input_distance, output_distance, in_layout, out_layout, placeness);
}

TEST_F(accuracy_test_callback_single,
       precall_lds_1D_forward_64_in_place_complex_to_complex) {
  try {
    precall_lds_1D_forward_64_in_place_complex_to_complex<float, cl_float,
                                                          fftwf_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
void postcall_lds_1D_forward_64_in_place_complex_to_complex() {
  ...
  data_pattern pattern = impulse;
  postcallback_complex_to_complex_lds<T, cl_T, fftw_T>(
      pattern, direction, lengths, batch, input_strides, output_strides,
      input_distance, output_distance, in_layout, out_layout, placeness);
}

TEST_F(accuracy_test_callback_single,
       postcall_lds_1D_forward_64_in_place_complex_to_complex) {
  try {
    postcall_lds_1D_forward_64_in_place_complex_to_complex<float, cl_float,
                                                           fftwf_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_callback_single,
       pre_and_post_callback_lds_1D_forward_64_in_place_complex_to_complex) {
  try {
    pre_and_post_callback_lds_1D_forward_64_in_place_complex_to_complex<
        float, cl_float, fftwf_complex>();
  } catch (const std::exception &err) {
...
TEST_P(mixed_radix_precallback,
       single_precision_hermitian_to_real_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_hermitian_to_real_precallback<float, cl_float, fftwf_complex>(
      problem_size);
}

TEST_P(mixed_radix_precallback,
       double_precision_hermitian_to_real_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_hermitian_to_real_precallback<double, cl_double, fftw_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_postcallback,
  ...
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_hermitian_to_real_postcallback<float, cl_float, fftwf_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_postcallback,
  ...
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_hermitian_to_real_postcallback<double, cl_double, fftw_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_precallback,
       single_precision_real_to_hermitian_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_real_to_hermitian_precallback<float, cl_float, fftwf_complex>(
      problem_size);
}

TEST_P(mixed_radix_precallback,
       double_precision_real_to_hermitian_auto_generated) {
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_real_to_hermitian_precallback<double, cl_double, fftw_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_postcallback,
  ...
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_real_to_hermitian_postcallback<float, cl_float, fftwf_complex>(
      problem_size);
}
...
TEST_P(mixed_radix_postcallback,
  ...
  size_t problem_size = GetParam();
  RecordProperty("problem_size", (int)problem_size);
  mixed_radix_real_to_hermitian_postcallback<double, cl_double, fftw_complex>(
      problem_size);
}
