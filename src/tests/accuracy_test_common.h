...
// tightly packed is inferred from strides.empty()
template <class T, class cl_T, class fftw_T>
void precallback_complex_to_complex(
    data_pattern pattern, direction::direction_t direction,
    std::vector<size_t> lengths, size_t batch,
...
// tightly packed is inferred from strides.empty()
template <class T, class cl_T, class fftw_T>
void postcallback_complex_to_complex(
    data_pattern pattern, direction::direction_t direction,
    std::vector<size_t> lengths, size_t batch,
    std::vector<size_t> input_strides, std::vector<size_t> output_strides,
    size_t input_distance, size_t output_distance,
...
// tightly packed is inferred from strides.empty()
template <class T, class cl_T, class fftw_T>
void pre_and_post_callback_complex_to_complex(
    data_pattern pattern, direction::direction_t direction,
    std::vector<size_t> lengths, size_t batch,
    std::vector<size_t> input_strides, std::vector<size_t> output_strides,
    size_t input_distance, size_t output_distance,
...
template <class T, class cl_T, class fftw_T>
void precallback_complex_to_complex_lds(
    data_pattern pattern, direction::direction_t direction,
    std::vector<size_t> lengths, size_t batch,
    std::vector<size_t> input_strides, std::vector<size_t> output_strides,
    size_t input_distance, size_t output_distance,
...
template <class T, class cl_T, class fftw_T>
void postcallback_complex_to_complex_lds(
    data_pattern pattern, direction::direction_t direction,
    std::vector<size_t> lengths, size_t batch,
    std::vector<size_t> input_strides, std::vector<size_t> output_strides,
    size_t input_distance, size_t output_distance,
...
template <class T, class cl_T, class fftw_T>
void real_to_complex(data_pattern pattern, std::vector<size_t> lengths,
                     size_t batch, std::vector<size_t> input_strides,
                     std::vector<size_t> output_strides, size_t input_distance,
                     size_t output_distance, layout::buffer_layout_t out_layout,
                     placeness::placeness_t placeness, T scale = 1.0f) {
  clfft<T, cl_T> test_fft(static_cast<clfftDim>(lengths.size()), &lengths[0],
                          input_strides.empty() ? NULL : &input_strides[0],
                          output_strides.empty() ? NULL : &output_strides[0],
...
template <class T, class cl_T, class fftw_T>
void complex_to_real(data_pattern pattern, std::vector<size_t> lengths,
                     size_t batch, std::vector<size_t> input_strides,
                     std::vector<size_t> output_strides, size_t input_distance,
                     size_t output_distance, layout::buffer_layout_t in_layout,
                     placeness::placeness_t placeness, T scale = 1.0f) {
...
void complex_to_complex_round_trip(data_pattern pattern,
  ...
  clfft<T, cl_T> test_fft(static_cast<clfftDim>(lengths.size()), &lengths[0],
                          NULL, NULL, batch, 0, 0, cl_layout(layout),
                          cl_layout(layout), cl_placeness(placeness));

  buffer<T> expected(lengths.size(), &lengths[0], NULL, batch, 0, layout,
                     CLFFT_OUTOFPLACE);
  ...
  if (pattern == sawtooth) {
    ...
    throw std::runtime_error(
        "invalid pattern type in complex_to_complex_round_trip()");
  }
...
void precallback_complex_to_complex_round_trip(data_pattern pattern,
  ...
  buffer<T> userdata(lengths.size(), &lengths[0], NULL, batch, 0, layout::real,
                     CLFFT_OUTOFPLACE);
...
void real_to_complex_round_trip(data_pattern pattern,
  ...
  clfft<T, cl_T> test_fft(static_cast<clfftDim>(lengths.size()), &lengths[0],
                          NULL, NULL, batch, 0, 0, cl_layout(layout::real),
                          cl_layout(layout::hermitian_interleaved),
                          cl_placeness(placeness));

  buffer<T> expected(lengths.size(), &lengths[0], NULL, batch, 0, layout::real,
                     CLFFT_OUTOFPLACE);
  ...
  if (pattern == sawtooth) {
    ...
    throw std::runtime_error(
        "invalid pattern type in real_to_complex_round_trip()");
  }
