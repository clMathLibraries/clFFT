...
protected:
  ...
  virtual void SetUp() {
    ...
    size_t memSizeBytes =
        lengths[0] * lengths[1] * lengths[2] * sizeof(std::complex<float>);

    device_id = initializeCL(g_device_type, g_device_id, g_platform_id, context,
                             printInfo);
    createOpenCLCommandQueue(context, commandQueueFlags, queue, device_id,
                             memSizeBytes, 1, &cl_mem_input, memSizeBytes, 1,
                             &cl_mem_output);
...
TEST_F(clfft_UnitTest, copyPlan_should_copy_plan) {
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetLayout(test_plan, CLFFT_COMPLEX_INTERLEAVED,
  ...
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanInStride(test_plan, CLFFT_2D, clStrides));
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanOutStride(test_plan, CLFFT_2D, clStrides));

  // TODO need to have created context by now; clfftPlanHandle is no longer a
  // pointer
...
TEST_F(clfft_UnitTest,
       setPlanPrecision_should_set_precision_to_supported_values) {
...
TEST_F(clfft_UnitTest, setPlanPrecision_should_fail_to_set_unsupported_values) {
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetPlanPrecision(test_plan, CLFFT_SINGLE_FAST));
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetPlanPrecision(test_plan, CLFFT_DOUBLE_FAST));
}
...
TEST_F(clfft_UnitTest, getPlanScale_should_yield_appropriate_values) {
  ...
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanScale(test_plan, CLFFT_BACKWARD, 2.718f));
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanScale(test_plan, CLFFT_BACKWARD, &scale));
...
TEST_F(clfft_UnitTest, getPlanScale_should_fail_on_invalid_direction) {
  ...
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftGetPlanScale(test_plan, ENDDIRECTION, &scale));
}
...
TEST_F(clfft_UnitTest, setPlanScale_should_fail_on_invalid_direction) {
  ...
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetPlanScale(test_plan, ENDDIRECTION, 42.0));
}

TEST_F(clfft_UnitTest,
       setPlanDimLength_should_set_dimensions_to_supported_values) {
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanLength(test_plan, CLFFT_1D, testLengths));
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanLength(test_plan, CLFFT_2D, testLengths));
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanLength(test_plan, CLFFT_3D, testLengths));
...
TEST_F(clfft_UnitTest,
       setPlanDimLength_should_fail_if_a_length_is_set_to_zero) {
  lengths[0] = 0;
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetPlanLength(test_plan, CLFFT_1D, lengths));

  lengths[0] = 4;
  lengths[1] = 0;
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetPlanDim(test_plan, CLFFT_2D));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetPlanLength(test_plan, CLFFT_2D, lengths));
  ...
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetPlanDim(test_plan, CLFFT_3D));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetPlanLength(test_plan, CLFFT_3D, lengths));
...
TEST_F(
    clfft_UnitTest,
    setPlanDimLength_should_fail_on_radices_that_have_non_supported_factors) {
...
TEST_F(clfft_UnitTest,
       setPlanDimLength_should_set_values_to_second_set_when_called_twice) {
  cl_uint lengthSize = 0;
  clfftDim dim;
...
TEST_F(clfft_UnitTest, getPlanDimLength_should_yield_correct_values) {
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_1D, lengths));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanLength(test_plan, CLFFT_1D, gotten_lengths));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetPlanDim(test_plan, &dim, &lengthSize));
  EXPECT_EQ(CLFFT_1D, dim);
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_2D, lengths));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanLength(test_plan, CLFFT_2D, gotten_lengths));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetPlanDim(test_plan, &dim, &lengthSize));
  EXPECT_EQ(CLFFT_2D, dim);
  EXPECT_EQ(1, gotten_lengths[0]);
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_3D, lengths));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanLength(test_plan, CLFFT_3D, gotten_lengths));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetPlanDim(test_plan, &dim, &lengthSize));
  EXPECT_EQ(CLFFT_3D, dim);
  EXPECT_EQ(1, gotten_lengths[0]);
  EXPECT_EQ(1, gotten_lengths[1]);
...
TEST_F(clfft_UnitTest, createDefaultPlan_should_fail_when_passed_length_of_0) {
  ...
  length[0] = 0;
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftCreateDefaultPlan(&test_plan, context, CLFFT_1D, length));

  length[0] = 1;
  length[1] = 0;
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftCreateDefaultPlan(&test_plan, context, CLFFT_2D, length));
  length[0] = 0;
  length[1] = 1;
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftCreateDefaultPlan(&test_plan, context, CLFFT_2D, length));
  ...
  length[2] = 0;
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftCreateDefaultPlan(&test_plan, context, CLFFT_3D, length));
  length[0] = 1;
  length[1] = 0;
  length[2] = 1;
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
...
TEST_F(clfft_UnitTest,
       createDefaultPlan_should_fail_when_passed_invalid_dimension) {
  size_t length[3] = {1, 1, 1};

  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftCreateDefaultPlan(&test_plan, context, ENDDIMENSION, length));
}

TEST_F(clfft_UnitTest,
       createDefaultPlan_should_fail_when_passed_unsupported_length) {
  size_t length[3] = {1, 1, 1};
...
TEST_F(clfft_UnitTest,
       setPlanInStride_should_set_input_strides_to_supported_values) {
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanInStride(test_plan, CLFFT_1D, gotten_strides));
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanInStride(test_plan, CLFFT_2D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanInStride(test_plan, CLFFT_3D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  EXPECT_EQ(16, gotten_strides[1]);
...
TEST_F(clfft_UnitTest, getPlanInStride_should_yield_correct_values) {
  ...
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanOutStride(test_plan, CLFFT_1D, output_strides));
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_2D, lengths));
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanInStride(test_plan, CLFFT_2D, input_strides));
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanOutStride(test_plan, CLFFT_2D, output_strides));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanInStride(test_plan, CLFFT_2D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_3D, lengths));
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanInStride(test_plan, CLFFT_3D, input_strides));
  ASSERT_EQ(CLFFT_SUCCESS,
            clfftSetPlanOutStride(test_plan, CLFFT_3D, output_strides));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanInStride(test_plan, CLFFT_3D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  EXPECT_EQ(8, gotten_strides[1]);
...
TEST_F(clfft_UnitTest,
       setPlanOutStride_should_set_output_strides_to_supported_values) {
  size_t strides[] = {1, 16, 16 * 32};
  size_t gotten_strides[3];
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanOutStride(test_plan, CLFFT_1D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanOutStride(test_plan, CLFFT_2D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  EXPECT_EQ(16, gotten_strides[1]);
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanOutStride(test_plan, CLFFT_3D, gotten_strides));
  EXPECT_EQ(1, gotten_strides[0]);
  EXPECT_EQ(16, gotten_strides[1]);
  EXPECT_EQ(16 * 32, gotten_strides[2]);
}
...
TEST_F(clfft_UnitTest,
       setPlanDistance_should_set_distance_to_supported_values) {
  size_t inDistance, outDistance;
  lengths[0] = 8;
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_1D, lengths));
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetPlanDistance(test_plan, 8 + 10, 8 + 2));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanDistance(test_plan, &inDistance, &outDistance));
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_2D, lengths));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanDistance(test_plan, 2 * 2 + 10, 2 * 2 + 2));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanDistance(test_plan, &inDistance, &outDistance));
  EXPECT_EQ(14, inDistance);
  EXPECT_EQ(6, outDistance);
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanDistance(test_plan, 2 * 2 + 3, 2 * 2 + 11));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftGetPlanDistance(test_plan, &inDistance, &outDistance));
...
TEST_F(clfft_UnitTest,
       setPlanDistance_should_fail_to_set_pitch_to_smaller_than_one_dataset) {
  lengths[0] = 32;
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_1D, lengths));
  ...
  ASSERT_EQ(CLFFT_SUCCESS, clfftSetPlanLength(test_plan, CLFFT_1D, lengths));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanDistance(test_plan, 32 * 32 - 30, 32 * 32 - 30));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanDistance(test_plan, 32 * 32 - 16, 32 * 32 - 16));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanDistance(test_plan, 32 * 32 - 1, 32 * 32 - 1));
}
...
TEST_F(clfft_UnitTest,
       setLayout_and_getLayout_should_set_and_get_layouts_respectively) {
  clfftLayout in, out;

  EXPECT_EQ(CLFFT_SUCCESS, clfftSetLayout(test_plan, CLFFT_COMPLEX_INTERLEAVED,
                                          CLFFT_COMPLEX_INTERLEAVED));
  ...
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetLayout(test_plan, CLFFT_COMPLEX_PLANAR,
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_HERMITIAN_INTERLEAVED));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetLayout(test_plan, &in, &out));
  EXPECT_EQ(CLFFT_REAL, in);
  EXPECT_EQ(CLFFT_HERMITIAN_INTERLEAVED, out);

  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_HERMITIAN_PLANAR));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetLayout(test_plan, &in, &out));
  EXPECT_EQ(CLFFT_REAL, in);
  EXPECT_EQ(CLFFT_HERMITIAN_PLANAR, out);

  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_HERMITIAN_PLANAR, CLFFT_REAL));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetLayout(test_plan, &in, &out));
  EXPECT_EQ(CLFFT_HERMITIAN_PLANAR, in);
  EXPECT_EQ(CLFFT_REAL, out);

  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_HERMITIAN_INTERLEAVED, CLFFT_REAL));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetLayout(test_plan, &in, &out));
...
TEST_F(clfft_UnitTest, setLayout_should_fail_on_invalid_argument_values) {
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetLayout(test_plan, CLFFT_COMPLEX_INTERLEAVED, ENDLAYOUT));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetLayout(test_plan, ENDLAYOUT, CLFFT_COMPLEX_INTERLEAVED));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetLayout(test_plan, ENDLAYOUT, ENDLAYOUT));

  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetLayout(test_plan, static_cast<clfftLayout>(ENDLAYOUT + 42),
                           CLFFT_COMPLEX_INTERLEAVED));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetLayout(test_plan, CLFFT_COMPLEX_INTERLEAVED,
                           static_cast<clfftLayout>(ENDLAYOUT + 42)));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetLayout(test_plan, static_cast<clfftLayout>(ENDLAYOUT + 42),
                           static_cast<clfftLayout>(ENDLAYOUT + 42)));
}
...
TEST_F(clfft_UnitTest,
       valid_layout_combinations_for_real_to_complex_should_succeed_on_bake_1) {
...
TEST_F(clfft_UnitTest,
       valid_layout_combinations_for_real_to_complex_should_succeed_on_bake_2) {
  clfftCreateDefaultPlan(&test_plan, context, CLFFT_1D, lengths);
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetResultLocation(test_plan, CLFFT_INPLACE));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_HERMITIAN_INTERLEAVED, CLFFT_REAL));
...
TEST_F(clfft_UnitTest,
       valid_layout_combinations_for_real_to_complex_should_succeed_on_bake_3) {
...
TEST_F(clfft_UnitTest,
       valid_layout_combinations_for_real_to_complex_should_succeed_on_bake_4) {
  clfftCreateDefaultPlan(&test_plan, context, CLFFT_1D, lengths);
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetResultLocation(test_plan, CLFFT_OUTOFPLACE));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_HERMITIAN_PLANAR));
...
TEST_F(clfft_UnitTest,
       valid_layout_combinations_for_real_to_complex_should_succeed_on_bake_5) {
  clfftCreateDefaultPlan(&test_plan, context, CLFFT_1D, lengths);
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetResultLocation(test_plan, CLFFT_OUTOFPLACE));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_HERMITIAN_INTERLEAVED, CLFFT_REAL));
...
TEST_F(clfft_UnitTest,
       valid_layout_combinations_for_real_to_complex_should_succeed_on_bake_6) {
  clfftCreateDefaultPlan(&test_plan, context, CLFFT_1D, lengths);
  EXPECT_EQ(CLFFT_SUCCESS, clfftSetResultLocation(test_plan, CLFFT_OUTOFPLACE));
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_HERMITIAN_PLANAR, CLFFT_REAL));
...
TEST_F(clfft_UnitTest,
       invalid_layout_combinations_for_real_to_complex_should_fail) {
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_REAL));
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_COMPLEX_INTERLEAVED));
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetLayout(test_plan, CLFFT_COMPLEX_INTERLEAVED, CLFFT_REAL));
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_COMPLEX_PLANAR));
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetLayout(test_plan, CLFFT_COMPLEX_PLANAR, CLFFT_REAL));
}
...
TEST_F(clfft_UnitTest, real_to_planar_should_fail_on_bake) {
  ...
  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetLayout(test_plan, CLFFT_REAL, CLFFT_HERMITIAN_PLANAR));
  EXPECT_EQ(CLFFT_INVALID_PLAN,
            clfftBakePlan(test_plan, 1, &queue, NULL, NULL));
}
...
TEST_F(clfft_UnitTest,
       setResultLocation_should_set_placeness_to_supported_values) {
...
TEST_F(clfft_UnitTest,
       setResultLocation_should_fail_on_invalid_argument_values) {
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetResultLocation(test_plan, ENDPLACE));
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetResultLocation(
                test_plan, static_cast<clfftResultLocation>(ENDPLACE + 42)));
}
...
TEST_F(clfft_UnitTest,
       SetPlanTransposeResult_should_set_resulttransposed_to_supported_values) {
  clfftResultTransposed transposed;

  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanTransposeResult(test_plan, CLFFT_TRANSPOSED));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetPlanTransposeResult(test_plan, &transposed));
  EXPECT_EQ(CLFFT_TRANSPOSED, transposed);

  EXPECT_EQ(CLFFT_SUCCESS,
            clfftSetPlanTransposeResult(test_plan, CLFFT_NOTRANSPOSE));
  EXPECT_EQ(CLFFT_SUCCESS, clfftGetPlanTransposeResult(test_plan, &transposed));
  EXPECT_EQ(CLFFT_NOTRANSPOSE, transposed);
}

TEST_F(clfft_UnitTest,
       setPlanTransposeResult_should_fail_on_invalid_argument_values) {
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetPlanTransposeResult(test_plan, ENDTRANSPOSED));
  EXPECT_EQ(
      CLFFT_INVALID_ARG_VALUE,
      clfftSetPlanTransposeResult(
          test_plan, static_cast<clfftResultTransposed>(ENDTRANSPOSED + 42)));
}
...
TEST_F(clfft_UnitTest, getTmpBufSize_should_fail_on_unbaked_plan) {
  size_t buffersize;
  EXPECT_EQ(CLFFT_INVALID_OPERATION,
            clfftGetTmpBufSize(test_plan, &buffersize));
}
...
TEST_F(clfft_UnitTest, bake_plan_should_fail_to_bake_multi_GPU_plan) {
  ...
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftBakePlan(test_plan, number_of_gpus, &queue, NULL, NULL));
}
...
TEST_F(clfft_UnitTest, bake_plan_should_fail_on_non_null_function_pointer) {
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftBakePlan(test_plan, 1, &queue, &foofies, NULL));
}

TEST_F(clfft_UnitTest, bake_plan_should_fail_on_non_null_user_data) {
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftBakePlan(test_plan, 1, &queue, NULL, (void *)0xf00f1e5));
}
...
TEST_F(clfft_UnitTest, set_precision_should_fail_on_invalid_value) {
  EXPECT_EQ(CLFFT_INVALID_ARG_VALUE,
            clfftSetPlanPrecision(test_plan, ENDPRECISION));
}
...
TEST_F(clfft_UnitTest, set_length_should_fail_on_invalid_dimension) {
  ...
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetPlanLength(test_plan, ENDDIMENSION, lengths));
}

TEST_F(clfft_UnitTest, get_length_should_fail_on_invalid_dimension) {
  size_t lengths[3] = {2, 4, 8};
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftGetPlanLength(test_plan, ENDDIMENSION, lengths));
}

TEST_F(clfft_UnitTest, set_in_stride_should_fail_on_null_pointer) {
  size_t *strides = NULL;
  EXPECT_EQ(CLFFT_INVALID_HOST_PTR,
            clfftSetPlanInStride(test_plan, CLFFT_2D, strides));
}

TEST_F(clfft_UnitTest, set_in_stride_should_fail_on_invalid_dimension) {
  size_t strides[3] = {2, 4, 8};
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetPlanInStride(test_plan, ENDDIMENSION, strides));
}

TEST_F(clfft_UnitTest, get_in_stride_should_fail_on_invalid_dimension) {
  size_t strides[3] = {2, 4, 8};
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftGetPlanInStride(test_plan, ENDDIMENSION, strides));
}

TEST_F(clfft_UnitTest, set_out_stride_should_fail_on_null_pointer) {
  size_t *strides = NULL;
  EXPECT_EQ(CLFFT_INVALID_HOST_PTR,
            clfftSetPlanOutStride(test_plan, CLFFT_2D, strides));
}

TEST_F(clfft_UnitTest, set_out_stride_should_fail_on_invalid_dimension) {
  size_t strides[3] = {2, 4, 8};
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftSetPlanOutStride(test_plan, ENDDIMENSION, strides));
}

TEST_F(clfft_UnitTest, get_out_stride_should_fail_on_invalid_dimension) {
  size_t strides[3] = {2, 4, 8};
  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftGetPlanOutStride(test_plan, ENDDIMENSION, strides));
}

TEST_F(
    clfft_UnitTest,
    enqueue_transform_should_fail_with_num_queues_and_events_greater_than_1) {
  cl_mem *cl_mem_input = NULL;
  cl_mem *cl_mem_output = NULL;

  EXPECT_EQ(CLFFT_NOTIMPLEMENTED,
            clfftEnqueueTransform(test_plan, CLFFT_FORWARD, 2, &queue, 0, NULL,
                                  &outEvent, cl_mem_input, cl_mem_output,
                                  NULL));
}
