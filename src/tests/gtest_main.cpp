...
//	Globals that user can set on the command line, that need to be passed
//down to unit tests
...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help,h", "produces this help message")(
      ...
      "cpu,c", "Run tests on a CPU device")("gpu,g",
                                            "Run tests on a GPU device")(
      "all,a", "Run tests on any device type (default)")(
      "platform", po::value<cl_int>(&g_platform_id)->default_value(0),
      "Select a specific OpenCL platform id as it is reported by clinfo")(
      "device", po::value<cl_int>(&g_device_id)->default_value(0),
      "Select a specific OpenCL device id as it is reported by clinfo")(
      "pointwise,p", "Do a pointwise comparison to determine test correctness "
                     "(default: use root mean square)")(
      "tolerance,t", po::value<float>(&tolerance)->default_value(0.001f),
      "tolerance level to use when determining test pass/fail")(
      "numRandom,r",
      po::value<size_t>(&number_of_random_tests)->default_value(2000),
      "number of random tests to run")(
      "seed",
      po::value<time_t>(&random_test_parameter_seed)
          ->default_value(time(NULL) % 1308000000),
      "seed to use for the random test. defaults to time(NULL)")
      // modulo lops off the first few digits of the time value to make the seed
      // easier to type even without these digits, the seed value won't wrap
      // around until 2036 or later
  ...
  // precision transform the actual rmse tolerance is this value times
  // sqrt(problem-size/4096)
  rmse_tolerance = 0.00002;

  //	Parse the command line options, ignore unrecognized options and collect
  ...
  if ((mutex & (mutex - 1)) != 0) {
    ...
    if (vm.count("cpu") > 0)
      terr << _T("    cpu,c   Force selection of OpenCL CPU devices only" )
           << std::endl;
    if (vm.count("all") > 0)
      terr << _T("    all,a   Force selection of all OpenCL devices (default)" )
           << std::endl;
    return 1;
  }
  ...
  if (!vm.count("noVersion")) {
    ...
    tout << std::left << std::setw(indent) << _T( "clFFT client API version: " )
         << clfftVersionMajor << _T( "." ) << clfftVersionMinor << _T( "." )
  ...
  if (!vm.count("noInfoCL")) {
    ...
    ::initializeCL(g_device_type, g_device_id, g_platform_id, tempContext,
                   true);
  ...
  if (vm.count("short") && vm.count("medium")) {
    terr << _T("Options 'short' and 'medium' are mutually-exclusive.  Please ")
            _T("select only one.")
         << std::endl;
    return 1;
  }
  ...
  //	Push into our new argv vector any parameter the user passed, except to
  //filter their gtest_filter expressions
