...
struct clMem_deleter {
  template <class T> void operator()(T *clMemObj) {
    if (clMemObj != NULL)
      OPENCL_V_THROW(::clReleaseMemObject(clMemObj),
...
struct plan_handle_deleter {
  template <class T> void operator()(T *handle) {
    ...
    clfftTeardown(); // when multi-GPU tests are written, this will need to
                     // occur in the gtest cleanup
  };
};
...
struct clCommQueue_deleter {
  template <class T> void operator()(T *clQueueObj) {
    if (clQueueObj != NULL)
      OPENCL_V_THROW(clReleaseCommandQueue(clQueueObj),
...
struct clContext_deleter {
  template <class T> void operator()(T *clContextObj) {
    if (clContextObj != NULL)
      OPENCL_V_THROW(clReleaseContext(clContextObj),
                     "Error: In clReleaseContext\n");
  };
};

template <class T> class Precision_Setter {
public:
  Precision_Setter(clfftPlanHandle plan_handle) {
    throw std::runtime_error(
        "Precision_Setter: this code path should never be executed");
  }
...
template <> class Precision_Setter<float> {
public:
  Precision_Setter(clfftPlanHandle plan_handle) {
...
template <> class Precision_Setter<double> {
public:
  Precision_Setter(clfftPlanHandle plan_handle) {
    clfftStatus ret = clfftSetPlanPrecision(plan_handle, CLFFT_DOUBLE);

    //	If device does not support double precision, skip this test, don't fail
    //it
...
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
template <class T, class cl_T> class clfft {
private:
...
public:
  ...
  clfft(const clfftDim dimensions_in, const size_t *lengths_in,
        ...
        const size_t batch_size_in, const size_t input_distance_in,
        const size_t output_distance_in, const clfftLayout input_layout_in,
        const clfftLayout output_layout_in,
        const clfftResultLocation placeness_in) try
      : _input_layout(input_layout_in), _output_layout(output_layout_in),
        _placeness(placeness_in),
        input(static_cast<size_t>(dimensions_in), lengths_in, input_strides_in,
              batch_size_in, input_distance_in,
              cl_layout_to_buffer_layout(_input_layout), _placeness),
        output(static_cast<size_t>(dimensions_in), lengths_in,
               output_strides_in, batch_size_in, output_distance_in,
               cl_layout_to_buffer_layout(_output_layout), _placeness),
        number_of_data_points(input.number_of_data_points()),
        _forward_scale(1.0f), _backward_scale(1.0f / T(number_of_data_points)),
        commandQueueFlags(0), init_failure(false), dataset_too_large(false),
        deviceType(0), plan_handle(new clfftPlanHandle),
        _transformation_direction(ENDDIRECTION), dimension(dimensions_in)

  {
    if (_placeness == CLFFT_INPLACE) {
      if ((is_real(_input_layout) && is_planar(_output_layout)) ||
        ...
        throw std::runtime_error(
            "in-place transforms may not be real<->planar");
      }
    }
  ...
  /*****************************************************/
  bool is_real(const clfftLayout layout) { return layout == CLFFT_REAL; }
  ...
  bool is_interleaved(const clfftLayout layout) {
    return (layout == CLFFT_COMPLEX_INTERLEAVED ||
            layout == CLFFT_HERMITIAN_INTERLEAVED);
  }

  /*****************************************************/
  bool is_complex(const clfftLayout layout) {
    return (layout == CLFFT_COMPLEX_INTERLEAVED ||
            layout == CLFFT_COMPLEX_PLANAR);
  }

  /*****************************************************/
  bool is_hermitian(const clfftLayout layout) {
    return (layout == CLFFT_HERMITIAN_INTERLEAVED ||
            layout == CLFFT_HERMITIAN_PLANAR);
  }
  ...
  void initialize_openCL() {
    try {
      ...
      device_id = initializeCL(g_device_type, g_device_id, g_platform_id,
                               tempContext, printInfo);
      ...
      queue = std::unique_ptr<_cl_command_queue, clCommQueue_deleter>(
          ::clCreateCommandQueue(context.get(), device_id[0], commandQueueFlags,
                                 &status));
      OPENCL_V_THROW(status,
      ...
      for (cl_int i = 0; i < CLFFT_COMPLEX_INTERLEAVED; ++i) {
        cl_int status = 0;
        std::unique_ptr<_cl_mem, clMem_deleter> inBuff(::clCreateBuffer(
            context.get(), CL_MEM_READ_WRITE, bufferSizeBytes, NULL, &status));
        OPENCL_V_THROW(status, "Creating Buffer ( ::clCreateBuffer() )");

        cl_mem_input.push_back(std::move(inBuff));

        std::unique_ptr<_cl_mem, clMem_deleter> outBuff(::clCreateBuffer(
            context.get(), CL_MEM_READ_WRITE, bufferSizeBytes, NULL, &status));
        OPENCL_V_THROW(status, "Creating Buffer ( ::clCreateBuffer() )");
  ...
  void initialize_plan() {
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftCreateDefaultPlan(plan_handle.get(), context.get(),
                                     dimension, &lengths[0]));
    set_layouts(_input_layout, _output_layout);
    placeness(_placeness);
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftSetPlanInStride(*plan_handle, dimension, input.strides()));
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftSetPlanOutStride(*plan_handle, dimension, output.strides()));
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftSetPlanBatchSize(*plan_handle, input.batch_size()));
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftSetPlanDistance(*plan_handle, input.distance(),
                                   output.distance()));
  ...
  void verbose_output() {
    if (verbose) {
      ...
      std::cout << dimensions << " dimension(s): " << lengths_plaintext()
                << std::endl;
      ...
      if (precision == CLFFT_SINGLE)
        ...
        throw std::runtime_error(
            "can't figure out the precision in verbose_output()");
      ...
      get_layouts();
      std::cout << layout_plaintext(_input_layout) << " -> "
                << layout_plaintext(_output_layout) << std::endl;

      std::cout << "input stride(s): " << input_strides_plaintext()
                << std::endl;
      std::cout << "output stride(s): " << output_strides_plaintext()
                << std::endl;
  ...
  /*****************************************************/
  void set_forward_transform() { _transformation_direction = CLFFT_FORWARD; }

  /*****************************************************/
  void set_backward_transform() { _transformation_direction = CLFFT_BACKWARD; }

  /*****************************************************/
  void set_transposed() {
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftSetPlanTransposeResult(*plan_handle, CLFFT_TRANSPOSED));
  }

  /*****************************************************/
  void set_layouts(clfftLayout new_input_layout,
                   clfftLayout new_output_layout) {
    cl_mem_input.clear();
    cl_mem_output.clear();
    ...
    if (is_planar(new_input_layout))
      ...
      throw std::runtime_error(
          "we shouldn't make it here [set_layouts(), input]");
    ...
    for (size_t i = 0; i < number_of_input_buffers; ++i) {
      ...
      std::unique_ptr<_cl_mem, clMem_deleter> buff(
          ::clCreateBuffer(context.get(), CL_MEM_READ_WRITE,
                           input_buffer_size_in_bytes, NULL, &status));
      OPENCL_V_THROW(status, "Creating Buffer ( ::clCreateBuffer() )");
    ...
    for (size_t i = 0; i < number_of_output_buffers; ++i) {
      ...
      std::unique_ptr<_cl_mem, clMem_deleter> buff(
          ::clCreateBuffer(context.get(), CL_MEM_READ_WRITE,
                           output_buffer_size_in_bytes, NULL, &status));
      OPENCL_V_THROW(status, "Creating Buffer ( ::clCreateBuffer() )");

      cl_mem_output.push_back(std::move(buff));
    }

    EXPECT_EQ(CLFFT_SUCCESS, clfftSetLayout(*plan_handle, new_input_layout,
                                            new_output_layout));
    get_layouts();
  }

  /*****************************************************/
  // swap_layouts should only be used with in-place real-to-complex or
  ...
  void forward_scale(T in) {
    EXPECT_EQ(CLFFT_SUCCESS, clfftSetPlanScale(*plan_handle, CLFFT_FORWARD,
  ...
  void backward_scale(T in) {
    EXPECT_EQ(CLFFT_SUCCESS, clfftSetPlanScale(*plan_handle, CLFFT_BACKWARD,
                                               static_cast<float>(in)));
  ...
  T forward_scale() {
    cl_T scale;
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftGetPlanScale(*plan_handle, CLFFT_FORWARD,
                                reinterpret_cast<cl_float *>(&scale)));
    return scale;
  }
  ...
  T backward_scale() {
    cl_T scale;
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftGetPlanScale(*plan_handle, CLFFT_BACKWARD,
                                reinterpret_cast<cl_float *>(&scale)));
    return scale;
  }

  /*****************************************************/
  void set_input_to_value(T real) { input.set_all_to_value(real); }
  ...
  /*****************************************************/
  void set_input_to_sawtooth(T max) { input.set_all_to_sawtooth(max); }

  /*****************************************************/
  void set_input_to_impulse() { input.set_all_to_impulse(); }
  ...
  void set_input_precallback(unsigned int localMemSize = 0) {
    ...
    if (localMemSize > 0) {
      ...
      if (input.is_interleaved()) {
        precallbackstr = (precision == CLFFT_SINGLE) ? STRINGIFY(PRE_MULVAL)
                                                     : STRINGIFY(PRE_MULVAL_DP);
      } else if (input.is_planar()) {
        precallbackstr = (precision == CLFFT_SINGLE)
                             ? STRINGIFY(PRE_MULVAL_PLANAR)
                             : STRINGIFY(PRE_MULVAL_PLANAR_DP);
      } else if (input.is_real()) {
        precallbackstr = (precision == CLFFT_SINGLE)
                             ? STRINGIFY(PRE_MULVAL_REAL)
                             : STRINGIFY(PRE_MULVAL_REAL_DP);
      }
    }

    // precallback user data
    buffer<T> userdata(static_cast<size_t>(dimension), input.lengths(),
                       input.strides(), input.batch_size(), input.distance(),
                       layout::real, _placeness);
    ...
    std::unique_ptr<_cl_mem, clMem_deleter> userdataBuff(
    ...
    OPENCL_V_THROW(clfftSetPlanCallback(*plan_handle, "mulval_pre",
                                        precallbackstr, localMemSize,
                                        PRECALLBACK, &uptr, 1),
                   "clFFTSetPlanCallback failed");
  }
  ...
  void set_input_precallback_userdatatype() {
    ...
    size_t totalPts =
        input.total_number_of_points_including_data_and_intervening();

    buffer<T> temp(static_cast<size_t>(dimension), input.lengths(),
                   input.strides(), input.batch_size(), input.distance(),
                   layout::real, _placeness);
      ...
      for (size_t z = 0; z < input.length(dimz); z++)
        for (size_t y = 0; y < input.length(dimy); y++)
          for (size_t x = 0; x < input.length(dimx); x++) {
            the_index = (input.stride(dimx) * x + input.stride(dimy) * y +
                         input.stride(dimz) * z + input.distance() * batch);

            userdata[the_index].scalar1 = (float)temp.real(x, y, z, batch);
            userdata[the_index].scalar2 = 1;
          }

    std::unique_ptr<_cl_mem, clMem_deleter> userdataBuff(clCreateBuffer(
    ...
    // Register the callback
    OPENCL_V_THROW(clfftSetPlanCallback(*plan_handle, "mulval_pre",
                                        precallbackstr, 0, PRECALLBACK, &uptr,
                                        1),
                   "clFFTSetPlanCallback failed");
  }
  ...
  void set_output_postcallback(unsigned int localMemSize = 0) {
    ...
    if (localMemSize > 0) {
      ...
      if (output.is_interleaved()) {
        postcallbackstr = (precision == CLFFT_SINGLE)
                              ? STRINGIFY(POST_MULVAL)
                              : STRINGIFY(POST_MULVAL_DP);
      } else if (output.is_planar()) {
        postcallbackstr = (precision == CLFFT_SINGLE)
                              ? STRINGIFY(POST_MULVAL_PLANAR)
                              : STRINGIFY(POST_MULVAL_PLANAR_DP);
      } else if (output.is_real()) {
        postcallbackstr = (precision == CLFFT_SINGLE)
                              ? STRINGIFY(POST_MULVAL_REAL)
                              : STRINGIFY(POST_MULVAL_REAL_DP);
      }
    }

    // post-callback user data
    buffer<T> userdata(static_cast<size_t>(dimension), output.lengths(),
    ...
    OPENCL_V_THROW(clfftSetPlanCallback(*plan_handle, "mulval_post",
                                        postcallbackstr, localMemSize,
                                        POSTCALLBACK, &uptr, 1),
                   "clFFTSetPlanCallback failed");
  }

  /*****************************************************/
  bool device_list_has_devices() { return !device_id.empty(); }

  /*****************************************************/
  // returns true if the memory required for input + output (if applicable) +
  // intermediate (if applicable) buffers is too large compared with the OpenCL
  ...
  bool total_memory_footprint_is_too_large_for_device() {
    ...
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftBakePlan(*plan_handle, 1, &tempQueue, NULL, NULL));
    ...
    // we are only going to include the result space if the transform is out of
    // place
    ...
    try {
      ...
      if (explicit_intermediate_buffer) {
        ...
        if (buffer_size) {
          ...
          std::unique_ptr<_cl_mem, clMem_deleter> temp_buffer(::clCreateBuffer(
              context.get(), CL_MEM_READ_WRITE, buffer_size, NULL, &status));
          OPENCL_V_THROW(status,
                         "Creating intermediate Buffer ( ::clCreateBuffer() )");
      ...
      if (buffer_size) {
        status = clfftEnqueueTransform(
            *plan_handle, _transformation_direction, 1, &tempQueue, 0, NULL,
            &tevent, &tempInput[0], &tempOutput[0], intermediate_buffer.get());
      } else {
        status = clfftEnqueueTransform(*plan_handle, _transformation_direction,
                                       1, &tempQueue, 0, NULL, &tevent,
                                       &tempInput[0], &tempOutput[0], NULL);
      }
      clFinish(tempQueue);
      tempEvent.reset(tevent);
      tevent = NULL;
      ...
      if (wait_status == CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST) {
        cl_int error_code;
        clGetEventInfo(revent, CL_EVENT_COMMAND_EXECUTION_STATUS,
                       sizeof(cl_int), &error_code, NULL);
      ...
      std::cout << "Exception occurred during clfftEnqueueTransform" << __FILE__
                << __LINE__ << std::endl;
      throw;
    }
  ...
  size_t maximum_problem_size() {
    ...
    // TODO *2 needs to be either *1 or *2, depending, once real numbers are
    // implemented in clfft
  }

  /*****************************************************/
  size_t number_of_opencl_devices() { return device_id.size(); }

  /*****************************************************/
  bool initialize_failed() { return init_failure; }

  /*****************************************************/
  bool dataset_is_too_large_for_device() { return dataset_too_large; }
...
private:
  ...
  void get_layouts() {
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftGetLayout(*plan_handle, &_input_layout, &_output_layout));
  }
  ...
  void capture_output() {
    if (is_planar(output_layout())) {
      OPENCL_V_THROW(
          clEnqueueReadBuffer(queue.get(), cl_mem_output[REAL].get(), CL_TRUE,
                              0, output.size_in_bytes(), output.real_ptr(), 0,
                              NULL, NULL),
          "reading output buffer - planar real ( ::clEnqueueReadBuffer() )");
      OPENCL_V_THROW(clEnqueueReadBuffer(queue.get(), cl_mem_output[IMAG].get(),
                                         CL_TRUE, 0, output.size_in_bytes(),
                                         output.imag_ptr(), 0, NULL, NULL),
                     "reading output buffer - planar imaginary ( "
                     "::clEnqueueReadBuffer() )");
    } else if (is_interleaved(output_layout())) {
      OPENCL_V_THROW(
          clEnqueueReadBuffer(queue.get(), cl_mem_output[0].get(), CL_TRUE, 0,
                              output.size_in_bytes(), output.interleaved_ptr(),
                              0, NULL, NULL),
          "reading output buffer - interleaved ( ::clEnqueueReadBuffer() )");
  ...
  void capture_input() {
    if (is_planar(input_layout())) {
      OPENCL_V_THROW(
          clEnqueueReadBuffer(queue.get(), cl_mem_input[REAL].get(), CL_TRUE, 0,
                              input.size_in_bytes(), input.real_ptr(), 0, NULL,
                              NULL),
          "reading input buffer - planar real ( ::clEnqueueReadBuffer() )");
      OPENCL_V_THROW(clEnqueueReadBuffer(queue.get(), cl_mem_input[IMAG].get(),
                                         CL_TRUE, 0, input.size_in_bytes(),
                                         input.imag_ptr(), 0, NULL, NULL),
                     "reading input buffer - planar imaginary ( "
                     "::clEnqueueReadBuffer() )");
    } else if (is_interleaved(input_layout())) {
      OPENCL_V_THROW(
          clEnqueueReadBuffer(queue.get(), cl_mem_input[0].get(), CL_TRUE, 0,
                              input.size_in_bytes(), input.interleaved_ptr(), 0,
                              NULL, NULL),
          "reading input buffer - interleaved ( ::clEnqueueReadBuffer() )");
  ...
  void write_local_output_buffer_to_gpu() {
    if (is_planar(output_layout())) {
      OPENCL_V_THROW(
          clEnqueueWriteBuffer(queue.get(), cl_mem_output[REAL].get(), CL_TRUE,
                               0, output.size_in_bytes(), output.real_ptr(), 0,
                               NULL, NULL),
          "writing output buffer - planar real ( ::clEnqueueWriteBuffer() )");
      OPENCL_V_THROW(clEnqueueWriteBuffer(queue.get(),
                                          cl_mem_output[IMAG].get(), CL_TRUE, 0,
                                          output.size_in_bytes(),
                                          output.imag_ptr(), 0, NULL, NULL),
                     "writing output buffer - planar imaginary ( "
                     "::clEnqueueWriteBuffer() )");
    } else if (is_interleaved(output_layout())) {
      OPENCL_V_THROW(
          clEnqueueWriteBuffer(queue.get(), cl_mem_output[0].get(), CL_TRUE, 0,
                               output.size_in_bytes(), output.interleaved_ptr(),
                               0, NULL, NULL),
          "writing output buffer - interleaved ( ::clEnqueueWriteBuffer() )");
    } else if (is_real(output_layout())) {
      OPENCL_V_THROW(
          clEnqueueWriteBuffer(queue.get(), cl_mem_output[REAL].get(), CL_TRUE,
                               0, output.size_in_bytes(), output.real_ptr(), 0,
      ...
      throw std::runtime_error(
          "we shouldn't make it here [write_local_output_buffer_to_gpu()]");
    }
  }
  ...
  void write_local_input_buffer_to_gpu() {
    if (is_planar(input_layout())) {
      OPENCL_V_THROW(
          clEnqueueWriteBuffer(queue.get(), cl_mem_input[REAL].get(), CL_TRUE,
                               0, input.size_in_bytes(), input.real_ptr(), 0,
                               NULL, NULL),
          "writing input buffer - planar real ( ::clEnqueueWriteBuffer() )");
      OPENCL_V_THROW(clEnqueueWriteBuffer(queue.get(), cl_mem_input[IMAG].get(),
                                          CL_TRUE, 0, input.size_in_bytes(),
                                          input.imag_ptr(), 0, NULL, NULL),
                     "writing input buffer - planar imaginary ( "
                     "::clEnqueueWriteBuffer() )");
    } else if (is_interleaved(input_layout())) {
      OPENCL_V_THROW(
          clEnqueueWriteBuffer(queue.get(), cl_mem_input[0].get(), CL_TRUE, 0,
                               input.size_in_bytes(), input.interleaved_ptr(),
                               0, NULL, NULL),
          "writing input buffer - interleaved ( ::clEnqueueWriteBuffer() )");
    } else if (is_real(input_layout())) {
      OPENCL_V_THROW(
          clEnqueueWriteBuffer(queue.get(), cl_mem_input[REAL].get(), CL_TRUE,
                               0, input.size_in_bytes(), input.real_ptr(), 0,
      ...
      throw std::runtime_error(
          "we shouldn't make it here [write_local_input_buffer_to_gpu()]");
    }
  }

  /*****************************************************/
  cl_ulong cl_device_max_memory_to_allocate(size_t device_index) {
    if (number_of_opencl_devices() == 0 ||
        device_index > number_of_opencl_devices()) {
      return 0;
    } else {
      cl_ulong device_max_to_allocate = 0;
      OPENCL_V_THROW(::clGetDeviceInfo(
                         device_id[device_index], CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                         sizeof(cl_ulong), &device_max_to_allocate, NULL),
  ...
  cl_ulong cl_device_max_global_memory(size_t device_index) {
    if (number_of_opencl_devices() == 0 ||
      ...
      OPENCL_V_THROW(
          ::clGetDeviceInfo(device_id[device_index], CL_DEVICE_GLOBAL_MEM_SIZE,
...
#if defined(                                                                   \
    PERSISTENT_PLANS_FEATURE_HAS_BEEN_DEFEATURED_WHICH_MEANS_IT_IS_NO_LONGER_A_FEATURE)
  /*****************************************************/
  void write_plan_to_file(std::string filename) {
    cl_command_queue tempQueue = queue.get();
    EXPECT_EQ(CLFFT_SUCCESS,
              clfftBakePlan(*plan_handle, 1, &tempQueue, NULL, NULL));
    // we need to make sure the plan is baked before we write it out, or we
    // won't get any juicy binaries along with it
  ...
  void read_plan_from_file(std::string filename) {
    ...
    // if we've changed from the default for input and output layouts, we need
    // to re-set the layouts to make sure buffers get set up completely
