...
public:
  ...
  Supported_Fft_Sizes() : max_mixed_radices_to_test(4096) {
...
INSTANTIATE_TEST_CASE_P(mixed_radices, mixed_radix,
...
TEST_F(accuracy_test_mixed_single,
...
TEST_F(accuracy_test_mixed_double,
       hermitian_to_real_transforms_with_non_unit_output_strides_should_pass) {
  try {
    hermitian_to_real_transforms_with_non_unit_output_strides_should_pass<
...
TEST_F(accuracy_test_mixed_single, small_targeted_real_to_hermitian_transform) {
  try {
    small_targeted_real_to_hermitian_transform<float, cl_float,
                                               fftwf_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_mixed_double, small_targeted_real_to_hermitian_transform) {
  try {
    small_targeted_real_to_hermitian_transform<double, cl_double,
                                               fftw_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_mixed_single,
       larger_targeted_real_to_hermitian_transform) {
  try {
    larger_targeted_real_to_hermitian_transform<float, cl_float,
                                                fftwf_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_mixed_double,
       larger_targeted_real_to_hermitian_transform) {
  try {
    larger_targeted_real_to_hermitian_transform<double, cl_double,
                                                fftw_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_mixed_single,
       another_targeted_real_to_hermitian_transform) {
  try {
    another_targeted_real_to_hermitian_transform<float, cl_float,
                                                 fftwf_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_mixed_double,
       another_targeted_real_to_hermitian_transform) {
  try {
    another_targeted_real_to_hermitian_transform<double, cl_double,
                                                 fftw_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
...
TEST_F(accuracy_test_mixed_single, possible_driver_bug_1D_length_375_fails) {
  try {
    possible_driver_bug_1D_length_375_fails<float, cl_float, fftwf_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
}
...
TEST_F(accuracy_test_mixed_double, possible_driver_bug_1D_length_375_fails) {
  try {
    possible_driver_bug_1D_length_375_fails<double, cl_double, fftw_complex>();
  } catch (const std::exception &err) {
    handle_exception(err);
  }
}