...
void handle_exception(const std::exception &except) {
  ...
  if (error_message.find("problem too large for device") != std::string::npos ||
      ...
      error_message.find("CLFFT_MEM_OBJECT_ALLOCATION_FAILURE") !=
          std::string::npos ||
      error_message.find("CLFFT_OUT_OF_HOST_MEMORY") != std::string::npos ||
      error_message.find("CLFFT_OUT_OF_RESOURCES") != std::string::npos) {
    std::cout << "Data set is too large for this device -- skipping test"
              << std::endl;
    // TODO put in (this problem size[data + stride]/max problem size/gpu or
    // cpu) specifics
  } else if (error_message.find("system memory allocation failure") !=
             std::string::npos) {
    std::cout
        << "Framework was denied enough system memory to support the data set"
        << " -- skipping test" << std::endl;
  } else if (error_message.find("CLFFT_DEVICE_NO_DOUBLE") !=
             std::string::npos) {
    std::cout << "Device in context does not support double precision"
              << " -- skipping test" << std::endl;
  } else if (error_message.find("dereference null pointer") !=
             std::string::npos) {
    std::cout << error_message << std::endl;
    FAIL();
  } else if (error_message.find(
                 "in-place transform, unmatched in/out layouts") !=
             std::string::npos) {
    std::cout << "Invalid arguments: for an in-place transform, "
              << "in/output layouts must be the same" << std::endl;
    FAIL();
  } else if (error_message.find("device list is empty at transform") !=
             std::string::npos) {
    std::cout << "A clfft transform is requested, but the device list is empty"
              << std::endl;
    FAIL();
  } else {
...
size_t max_mem_available_on_cl_device(size_t device_index) {
  ...
  // this is not thread-safe using globals, it is just quick fix for now, todo
  // proper fix
  if (g_device_max_mem_size == 0) {
    std::vector<cl_device_id> device_id;
    cl_context tempContext = NULL;
    device_id = initializeCL(g_device_type, (cl_int)device_index, g_platform_id,
                             tempContext, false);
