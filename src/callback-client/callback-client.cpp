...
int main(int argc, char **argv) {
  size_t lengths[3] = {BATCH_LENGTH, 1,
                       1}; // For simplicity, assuming 1D fft with fixed batch
                           // length of BATCH_LENGTH
  cl_uint profile_count = 0;
  clfftPrecision precision = CLFFT_SINGLE; // Testing for single precision.
                                           // Easily extendable for double
  ...
  try {
    ...
    desc.add_options()("help,h", "produces this help message")(
        ...
        "profile,p", po::value<cl_uint>(&profile_count)->default_value(10),
        "Time and report the kernel speed of the FFT (default: profiling on)");
    ...
    tout << "\nRunning FFT for length " << BATCH_LENGTH << " and batch size "
         << batchSize << std::endl;

    // Real-Complex cases, SP

    R2C_transform<float>(setupData, lengths, batchSize, dim, precision,
                         profile_count);
...
void R2C_transform(std::auto_ptr<clfftSetupData> setupData, size_t *inlengths,
  ...
  cl_command_queue commandQueue = ::clCreateCommandQueue(
      context, device_id[0], command_queue_flags, &status);
  ...
  if (precision == CLFFT_SINGLE) {
    ...
    runR2C_FFT_PreAndPostprocessKernel<float>(
        setupData, context, commandQueue, device_id[0], inlengths, dim,
        precision, batchSize, vectorLength, fftLength, profile_count);

    // Run clFFT using pre-callback
    runR2C_FFT_WithCallback<float>(setupData, context, commandQueue, inlengths,
                                   dim, precision, batchSize, vectorLength,
                                   fftLength, profile_count);
  }
...
void runR2C_FFT_WithCallback(std::auto_ptr<clfftSetupData> setupData,
  ...
  cl_mem infftbuffer =
      ::clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                       in_size_of_buffers, (void *)input24bitData, &status);
  OPENCL_V_THROW(status, "Creating Buffer ( ::clCreateBuffer(infftbuffer) )");

  // out-place transform.
  cl_mem outfftbuffer = ::clCreateBuffer(context, CL_MEM_READ_WRITE,
                                         out_size_of_buffers, NULL, &status);
  ...
  OPENCL_V_THROW(clfftCreateDefaultPlan(&plan_handle, context, dim, inlengths),
  ...
  OPENCL_V_THROW(clfftSetPlanCallback(plan_handle, "convert24To32bit",
                                      precallbackstr, 0, PRECALLBACK, NULL, 0),
                 "clFFTSetPlanCallback failed");
  OPENCL_V_THROW(clfftSetPlanCallback(plan_handle, "extractMagnitude",
                                      postcallbackstr, 0, POSTCALLBACK, NULL,
                                      0),
                 "clFFTSetPlanCallback failed");
  ...
  OPENCL_V_THROW(clfftSetLayout(plan_handle, inLayout, outLayout),
                 "clfftSetLayout failed");
  OPENCL_V_THROW(clfftSetPlanBatchSize(plan_handle, batchSize),
                 "clfftSetPlanBatchSize failed");
  OPENCL_V_THROW(clfftSetPlanPrecision(plan_handle, precision),
                 "clfftSetPlanPrecision failed");
  OPENCL_V_THROW(clfftSetPlanDistance(plan_handle, BATCH_LENGTH + 2,
                                      (BATCH_LENGTH / 2 + 1)),
                 "clfftSetPlanDistance failed");

  // Bake Plan
  OPENCL_V_THROW(clfftBakePlan(plan_handle, 1, &commandQueue, NULL, NULL),
  ...
  OPENCL_V_THROW(clfftEnqueueTransform(
                     plan_handle, CLFFT_FORWARD, 1, &commandQueue, 0, NULL,
                     NULL, &infftbuffer, &outfftbuffer, clMedBuffer),
  ...
  if (profile_count > 1) {
    ...
    tout << "\nExecution wall time (with clFFT Callback): " << 1000.0 * wtime
         << " ms" << std::endl;
  }
  ...
  if (profile_count == 1) {
    ...
    OPENCL_V_THROW(clEnqueueReadBuffer(commandQueue, outfftbuffer, CL_TRUE, 0,
                                       out_size_of_buffers, &output[0], 0, NULL,
                                       NULL),
    ...
    refout = get_R2C_fftwf_output(inlengths, fftLength, (int)batchSize,
                                  inLayout, dim);

    if (!compare<fftwf_complex, T>(refout, output, fftLength / 2)) {
      std::cout
          << "\n\n\t\tInternal Client Test (with clFFT Callback) *****FAIL*****"
          << std::endl;
    } else {
      std::cout
          << "\n\n\t\tInternal Client Test (with clFFT Callback) *****PASS*****"
          << std::endl;
    }
  ...
  // cleanup
  OPENCL_V_THROW(clReleaseMemObject(infftbuffer),
                 "Error: In clReleaseMemObject\n");
  OPENCL_V_THROW(clReleaseMemObject(outfftbuffer),
                 "Error: In clReleaseMemObject\n");
}
...
void runR2C_FFT_PreAndPostprocessKernel(
  ...
  // input data buffer
  cl_mem in24bitfftbuffer =
      ::clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                       in_size_of_buffers, (void *)input24bitData, &status);
  OPENCL_V_THROW(status,
                 "Creating Buffer ( ::clCreateBuffer(in24bitfftbuffer) )");

  cl_mem in32bitfftbuffer = ::clCreateBuffer(
      context, CL_MEM_READ_WRITE, out_size_of_buffers, NULL, &status);
  OPENCL_V_THROW(status,
                 "Creating Buffer ( ::clCreateBuffer(in32bitfftbuffer) )");
  ...
  cl_mem magoutfftbuffer = ::clCreateBuffer(
      context, CL_MEM_WRITE_ONLY, out_size_of_buffers / 2, NULL, &status);
  OPENCL_V_THROW(status,
                 "Creating Buffer ( ::clCreateBuffer(magoutfftbuffer) )");
  ...
  cl_program program =
      clCreateProgramWithSource(context, 1, &source, NULL, &status);
  ...
  if (status != CL_SUCCESS) {
    if (status == CL_BUILD_PROGRAM_FAILURE) {
      ...
      OPENCL_V_THROW(clGetProgramBuildInfo(program, device_id,
  ...
  // Input 24bit Buffer
  OPENCL_V_THROW(
      clSetKernelArg(prekernel, 0, sizeof(cl_mem), (void *)&in24bitfftbuffer),
      "clSetKernelArg failed");

  // output 32bit Buffer
  OPENCL_V_THROW(
      clSetKernelArg(prekernel, 1, sizeof(cl_mem), (void *)&in32bitfftbuffer),
      "clSetKernelArg failed");
  ...
  OPENCL_V_THROW(
      clSetKernelArg(postkernel, 0, sizeof(cl_mem), (void *)&outfftbuffer),
      "clSetKernelArg failed");
  OPENCL_V_THROW(
      clSetKernelArg(postkernel, 1, sizeof(cl_mem), (void *)&magoutfftbuffer),
      "clSetKernelArg failed");

  // Launch pre-process kernel
  size_t gSize_pre = fftLength;
  status = clEnqueueNDRangeKernel(commandQueue, prekernel, 1, NULL, &gSize_pre,
                                  NULL, 0, NULL, NULL);
  ...
  OPENCL_V_THROW(clfftEnqueueTransform(
                     plan_handle, CLFFT_FORWARD, 1, &commandQueue, 0, NULL,
                     NULL, &in32bitfftbuffer, &outfftbuffer, clMedBuffer),
                 "clfftEnqueueTransform failed");

  size_t gSize_post = fftLength / 2;
  // Launch post-process kernel
  status = clEnqueueNDRangeKernel(commandQueue, postkernel, 1, NULL,
                                  &gSize_post, NULL, 0, NULL, NULL);
  OPENCL_V_THROW(status, "clEnqueueNDRangeKernel failed");
  ...
  if (profile_count > 1) {
    ...
    for (cl_uint i = 0; i < profile_count; ++i) {
      ...
      // Input 24bit Buffer
      OPENCL_V_THROW(clSetKernelArg(prekernel, 0, sizeof(cl_mem),
                                    (void *)&in24bitfftbuffer),
                     "clSetKernelArg failed");

      // output 32bit Buffer
      OPENCL_V_THROW(clSetKernelArg(prekernel, 1, sizeof(cl_mem),
                                    (void *)&in32bitfftbuffer),
                     "clSetKernelArg failed");

      status = clEnqueueNDRangeKernel(commandQueue, prekernel, 1, NULL,
                                      &gSize_pre, NULL, 0, NULL, NULL);
      OPENCL_V_THROW(status, "clEnqueueNDRangeKernel failed");
      ...
      OPENCL_V_THROW(
          clSetKernelArg(postkernel, 0, sizeof(cl_mem), (void *)&outfftbuffer),
          "clSetKernelArg failed");
      OPENCL_V_THROW(clSetKernelArg(postkernel, 1, sizeof(cl_mem),
                                    (void *)&magoutfftbuffer),
                     "clSetKernelArg failed");
    ...
    tout << "\nExecution wall time (Separate Pre and Post process kernels): "
         << 1000.0 * wtime << " ms" << std::endl;
  }
  ...
  if (profile_count == 1) {
    ...
    OPENCL_V_THROW(clEnqueueReadBuffer(commandQueue, magoutfftbuffer, CL_TRUE,
                                       0, out_size_of_buffers / 2, &output[0],
                                       0, NULL, NULL),
                   "Reading the result buffer failed");
    ...
    if (!compare<fftwf_complex, T>(refout, output, fftLength / 2)) {
      std::cout << "\n\n\t\tInternal Client Test (Separate Pre and Post "
                   "process kernels) *****FAIL*****"
                << std::endl;
    } else {
      std::cout << "\n\n\t\tInternal Client Test (Separate Pre and Post "
                   "process kernels) *****PASS*****"
                << std::endl;
    }
  ...
  // cleanup
  OPENCL_V_THROW(clReleaseMemObject(in24bitfftbuffer),
                 "Error: In clReleaseMemObject\n");
  OPENCL_V_THROW(clReleaseMemObject(in32bitfftbuffer),
                 "Error: In clReleaseMemObject\n");
  OPENCL_V_THROW(clReleaseMemObject(outfftbuffer),
                 "Error: In clReleaseMemObject\n");
  OPENCL_V_THROW(clReleaseMemObject(magoutfftbuffer),
                 "Error: In clReleaseMemObject\n");
}
...
fftwf_complex *get_R2C_fftwf_output(size_t *lengths, size_t fftbatchLength,
  ...
  int outembed[3] = {(int)lengths[2], (int)lengths[1],
                     (int)(lengths[0] / 2 + 1)};
  ...
  fftwf_complex *refout = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * outfftVectorLength * batch_size);

  refPlan = fftwf_plan_many_dft_r2c(
      dim, &fftwLengths[3 - dim], batch_size, refin, &inembed[3 - dim], 1,
      infftVectorLength, refout, &outembed[3 - dim], 1, outfftVectorLength,
      FFTW_ESTIMATE);
  ...
  for (size_t i = 0; i < fftbatchLength; i++) {
    val = (float)(in24bitData[i][0] << 16 | in24bitData[i][1] << 8 |
                  in24bitData[i][2]);
