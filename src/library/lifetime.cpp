...
clfftStatus clfftSetup(const clfftSetupData *sData) {
  ...
  //FFTRepo::getInstance to minimize lock overhead; this is only necessary on
  //first creation
  ...
  if (fftRepo.timerHandle) {
    ...
    PFGETSTATTIMER pfGetStatTimer = reinterpret_cast<PFGETSTATTIMER>(
        LoadFunctionAddr(fftRepo.timerHandle, "getStatTimer"));

    //	Create and initialize our timer class, if the external timer shared
    //library loaded
    if (pfGetStatTimer) {
      fftRepo.pStatTimer =
          reinterpret_cast<GpuStatTimer *>(pfGetStatTimer(CLFFT_GPU));
    }
  }
  ...
  ////	If the major version number between the client and library do not match,
  ///return mismatch
  // if( sData->major > clfftVersionMajor )
  //	return CLFFT_VERSION_MISMATCH;

  ////	If the minor version number between the client and library do not match,
  ///return mismatch
