...
template <Precision PR> inline size_t PrecisionWidth() {
  switch (PR) {
  case P_SINGLE:
    return 1;
  case P_DOUBLE:
...
template <Precision PR> inline std::string ClPragma() {
  switch (PR) {
  case P_SINGLE:
    return "";
  case P_DOUBLE:
...
template <Precision PR> inline std::string RegBaseType(size_t count) {
  switch (PR) {
  case P_SINGLE:
    switch (count) {
    case 1:
...
template <Precision PR> inline std::string FloatSuffix() {
...
public:
  ...
  template <Precision PR> void GenerateTwiddleTable(std::string &twStr) {
...
// FFT butterfly
template <Precision PR> class Butterfly {
  ...
  size_t BitReverse(size_t n, size_t N) const {
    return (N < 2)
               ? n
               : (BitReverse(n >> 1, N >> 1) | ((n & 1) != 0 ? (N >> 1) : 0));
  }
  ...
  void GenerateButterflyStr(std::string &bflyStr) const {
    ...
    // = 0) or if cReg is true, then allocate temporary variables only for non
    // power-of-2 radices
