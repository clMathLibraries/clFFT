...
inline size_t AtomicAdd(volatile size_t *value, size_t op) {
  return _InterlockedExchangeAdd64(reinterpret_cast<volatile __int64 *>(value),
                                   op);
}
#else
inline void BSF(unsigned long *index, size_t &mask) {
...
//	This header file is not visible to clients, and contains internal
//structures and functions for use 	by the FFT library.  Since this header is
//private to this implementation, there is no need to keep 	strict C compliance.
...
template <typename FileStreamType, typename StringType> class tofstreamRAII {
...
public:
  ...
  ~tofstreamRAII() { outFile.close(); }

  StringType &getName() { return fileName; }

  void setName(const StringType &name) { fileName = name; }

  FileStreamType &get() { return outFile; }
};
...
//	This is used to either wrap an OpenCL function call, or to explicitly
//check a variable for an OpenCL error condition. 	If an error occurs, we issue a
...
CLFFTAPI clfftStatus clfftLocalMemSize(const clfftPlanHandle plHandle,
                                       cl_ulong *local_mem_size);

/*! @brief Save to disk a file containing the contents of a baked plan.
 *  @details A plan is a repository of state for calculating FFT's. Saves the
 *details for a plan to allow the user to easily recreate a plan and execute it
 *without having to first build the kernel.
 ...
 */
CLFFTAPI clfftStatus clfftWritePlanToDisk(clfftPlanHandle plHandle,
                                          const char *filename);

/*! @brief Read from disk a file containing the contents of a baked plan.
 *  @details A plan is a repository of state for calculating FFT's. Reads the
 *details for a plan from a file on disk and duplicates the plan in the provided
 *plan handle.
 ...
 *  @return Enum describing error condition; superset of OpenCL error codes
 */
CLFFTAPI clfftStatus clfftReadPlanFromDisk(clfftPlanHandle plHandle,
                                           const char *filename);
