...
const std::string end_of_binary(
    "<[�_I_may_be_a_sorry_case,_but_I_don't_write_jokes_in_base_13_�]>");
const std::string end_of_file(
    "<[�_You're_off_the_edge_of_the_map,_mate._Here_there_be_monsters_�]>");
...
static bool split1D_for_inplace(size_t num, vector<vector<size_t>> &splitNums,
                                clfftPrecision precision, size_t threshold) {
  ...
  */
  ...
  // now the remaining num should have even number of pow2, pow3 and pow5 and we
  // can do sqrt
  auto temp = (size_t)sqrt((double)num);
  vector<size_t> splitVec;
  splitVec.push_back(temp * divide_factor);
  splitVec.push_back(temp);
  splitNums.push_back(splitVec);

  status = status && split1D_for_inplace(temp * divide_factor, splitNums,
                                         precision, threshold);
...
clfftStatus checkDevExt(std::string ext, const cl_device_id &device) {
  ...
  OPENCL_V(
      ::clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, 0, nullptr, &deviceExtSize),
      "Getting CL_DEVICE_EXTENSIONS Platform Info string size ( "
      "::clGetDeviceInfo() )");

  std::vector<char> szDeviceExt(deviceExtSize);
  OPENCL_V(::clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, deviceExtSize,
                             &szDeviceExt[0], nullptr),
...
clfftStatus clfftCreateDefaultPlanInternal(clfftPlanHandle *plHandle,
                                           cl_context context,
                                           const clfftDim dim,
                                           const size_t *clLengths) {
  if (clLengths == nullptr)
  ...
  case CLFFT_2D: {
    ...
    if (!IsASupportedLength(clLengths[DimX]) ||
        !IsASupportedLength(clLengths[DimY])) {
      return CLFFT_NOTIMPLEMENTED;
    }
  ...
  OPENCL_V(fftRepo.createPlan(plHandle, fftPlan),
           _T( "fftRepo.insertPlan failed" ));
  ...
  lockRAII *planLock = nullptr;
  OPENCL_V(fftRepo.getPlan(*plHandle, fftPlan, planLock),
           _T( "fftRepo.getPlan failed" ));
...
clfftStatus clfftCreateDefaultPlan(clfftPlanHandle *plHandle,
                                   cl_context context, const clfftDim dim,
                                   const size_t *clLengths) {
  clfftStatus ret =
      clfftCreateDefaultPlanInternal(plHandle, context, dim, clLengths);
...
std::string getKernelName(const clfftGenerators gen,
                          const clfftPlanHandle plHandle, bool withPlHandle) {
  ...
  case Stockham:
    generatorName = "Stockham";
    break;
  case Transpose_GCN:
    generatorName = "Transpose";
    break;
  case Transpose_SQUARE:
    generatorName = "Transpose";
    break;
  case Transpose_NONSQUARE:
    generatorName = "TransposeNonSquare";
    break;
  case Copy:
    generatorName = "Copy";
    break;
...
clfftStatus selectAction(FFTPlan *fftPlan, FFTAction *&action,
  ...
  case Stockham: {
    ...
    action = new FFTGeneratedStockhamAction(fftPlan->plHandle, fftPlan,
                                            *commQueueFFT, err);
    OPENCL_V(err, "FFTGeneratedStockhamAction() failed");
  } break;

  case Transpose_GCN: {
    action = new FFTGeneratedTransposeGCNAction(fftPlan->plHandle, fftPlan,
                                                *commQueueFFT, err);
    OPENCL_V(err, "FFTGeneratedTransposeGCNAction() failed");
  } break;

  case Copy: {
    action = new FFTGeneratedCopyAction(fftPlan->plHandle, fftPlan,
                                        *commQueueFFT, err);
...
clfftStatus clfftBakePlan(
  ...
  //transforms, and this must be implemented to make only a single callback to
  //the user.
  ...
  clGetCommandQueueInfo(*commQueueFFT, CL_QUEUE_DEVICE, sizeof(cl_device_id),
                        &fftPlan->bakeDevice, nullptr);

  // find product of lengths
  size_t maxLengthInAnyDim = 1;
  switch (fftPlan->dim) {
  case CLFFT_3D:
    maxLengthInAnyDim = maxLengthInAnyDim > fftPlan->length[DimZ]
                            ? maxLengthInAnyDim
                            : fftPlan->length[DimZ];
  case CLFFT_2D:
    maxLengthInAnyDim = maxLengthInAnyDim > fftPlan->length[DimY]
                            ? maxLengthInAnyDim
                            : fftPlan->length[DimY];
  case CLFFT_1D:
    maxLengthInAnyDim = maxLengthInAnyDim > fftPlan->length[DimX]
                            ? maxLengthInAnyDim
                            : fftPlan->length[DimX];
  }

  const bool rc = (fftPlan->inputLayout == CLFFT_REAL) ||
  ...
  if ((fftPlan->precision == CLFFT_SINGLE) &&
  ...
  if (nullptr != fftPlan->intBuffer) {
    OPENCL_V(clReleaseMemObject(fftPlan->intBuffer),
             _T( "Failed to release internal temporary buffer" ));
    fftPlan->intBuffer = nullptr;
  }
  if (nullptr != fftPlan->intBufferRC) {
    OPENCL_V(clReleaseMemObject(fftPlan->intBufferRC),
             _T( "Failed to release internal temporary buffer" ));
    fftPlan->intBufferRC = nullptr;
  }
  if (nullptr != fftPlan->intBufferC2R) {
    OPENCL_V(clReleaseMemObject(fftPlan->intBufferC2R),
             _T( "Failed to release internal temporary buffer" ));
    fftPlan->intBufferC2R = nullptr;
  }

  if (fftPlan->userPlan) // confirm it is top-level plan (user plan)
  {
    if (fftPlan->placeness == CLFFT_INPLACE) {
      if ((fftPlan->inputLayout == CLFFT_HERMITIAN_PLANAR) ||
  ...
  if (fftPlan->gen == Copy) {
    clfftStatus err;
    fftPlan->action =
        new FFTGeneratedCopyAction(plHandle, fftPlan, *commQueueFFT, err);
  ...
  if (fftPlan->userPlan) {
    //	If the user specifies double precision, check that the device supports
    //double precision first
    if (fftPlan->precision == CLFFT_DOUBLE ||
        fftPlan->precision == CLFFT_DOUBLE_FAST) {
      ...
      if (retAmdFp64 != CLFFT_SUCCESS) {
        ...
        clfftStatus retKhrFp64 =
            checkDevExt("cl_khr_fp64", fftPlan->bakeDevice);
  ...
  OPENCL_V(fftPlan->GetMax1DLength(&Large1DThreshold),
           _T("GetMax1DLength failed"));
    ...
    if (!Is1DPossible(fftPlan->length[0], Large1DThreshold)) {
      ...
      if (IsPo2(fftPlan->length[0])) {
        ...
        if ((fftPlan->inStride[0] == 1) && (fftPlan->outStride[0] == 1) &&
            !rc &&
            (fftPlan->length[0] <=
             262144 / PrecisionWidth(fftPlan->precision)) &&
          ...
          if (clfftGetRequestLibNoMemAlloc() && !rc &&
              (fftPlan->placeness == CLFFT_INPLACE)) {
            in_x = BitScanF(fftPlan->length[0]);
            in_x /= 2;
            clLengths[1] = (size_t)1 << in_x;
          } else if (fftPlan->length[0] >
                     (Large1DThreshold * Large1DThreshold)) {
            clLengths[1] = fftPlan->length[0] / Large1DThreshold;
          } else {
            in_1d =
                BitScanF(Large1DThreshold); // this is log2(LARGE1D_THRESHOLD)
        ...
        size_t halfPowerLength =
            (size_t)1 << ((StockhamGenerator::CeilPo2(fftPlan->length[0]) + 1) /
                          2);
      ...
      */
      ...
      if (clfftGetRequestLibNoMemAlloc() &&
        ...
        bool implemented = split1D_for_inplace(fftPlan->length[0], splitNums,
                                               fftPlan->precision, threshold);
        if (implemented)
      ...
      while (true && (fftPlan->inputLayout != CLFFT_REAL) &&
             (fftPlan->outputLayout != CLFFT_REAL)) {
        ...
        size_t biggerDim =
            clLengths[0] > clLengths[1] ? clLengths[0] : clLengths[1];
        size_t smallerDim =
            biggerDim == clLengths[0] ? clLengths[1] : clLengths[0];
        size_t padding = 0;
        ...
        if (clfftGetRequestLibNoMemAlloc() && dim_residue == 0 &&
            ((dim_ratio % 2 == 0) || (dim_ratio % 3 == 0) ||
             (dim_ratio % 5 == 0) || (dim_ratio % 10 == 0)) &&
            fftPlan->placeness == CLFFT_INPLACE &&
            (fftPlan->inputLayout == fftPlan->outputLayout) &&
        ...
        if (clfftGetRequestLibNoMemAlloc() && (clLengths[0] == clLengths[1]) &&
            fftPlan->placeness == CLFFT_INPLACE) {
          padding = 0;
          fftPlan->allOpsInplace = true;
        ...
        // Transpose
        // Input --> tmp buffer
        OPENCL_V(clfftCreateDefaultPlanInternal(
                     &fftPlan->planTX, fftPlan->context, CLFFT_2D, clLengths),
                 _T( "CreateDefaultPlan Large1d transpose 1 failed" ));

        FFTPlan *trans1Plan = nullptr;
        lockRAII *trans1Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planTX, trans1Plan, trans1Lock),
                 _T( "fftRepo.getPlan failed" ));

        trans1Plan->placeness =
            fftPlan->allOpsInplace ? CLFFT_INPLACE : CLFFT_OUTOFPLACE;
        ...
        trans1Plan->outputLayout = fftPlan->allOpsInplace
                                       ? fftPlan->inputLayout
        ...
        if (trans1Plan->gen == Transpose_NONSQUARE ||
            trans1Plan->gen == Transpose_SQUARE) // inplace transpose
        {
          for (size_t index = 1; index < fftPlan->length.size(); index++) {
            // trans1Plan->length.push_back(fftPlan->length[index]);
            /*
            replacing the line above with the two lines below since:
            fftPlan is still 1D, thus the broken down transpose should be 2D not
            3D the batchSize for the transpose should increase accordingly. the
            iDist should decrease accordingly. Push back to length will cause a
            3D transpose
            */
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planTX, numQueues, commQueueFFT, nullptr, nullptr),
            _T( "BakePlan large1d trans1 plan failed" ));

        // Row transform
        // tmp->output
        // size clLengths[1], batch clLengths[0], with length[0] twiddle factor
        // multiplication
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planX,
                                                fftPlan->context, CLFFT_1D,
                                                &clLengths[1]),
                 _T( "CreateDefaultPlan Large1d column failed" ));

        FFTPlan *row1Plan = nullptr;
        lockRAII *row1Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planX, row1Plan, row1Lock),
                 _T( "fftRepo.getPlan failed" ));
        ...
        row1Plan->inputLayout = fftPlan->allOpsInplace
                                    ? fftPlan->inputLayout
                                    : CLFFT_COMPLEX_INTERLEAVED;
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planX, numQueues, commQueueFFT, nullptr, nullptr),
            _T( "BakePlan large1d first row plan failed" ));

        // Transpose 2
        // Output --> tmp buffer
        clLengths[2] = clLengths[0];
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planTY,
                                                fftPlan->context, CLFFT_2D,
                                                &clLengths[1]),
                 _T( "CreateDefaultPlan Large1d transpose 2 failed" ));

        FFTPlan *trans2Plan = nullptr;
        lockRAII *trans2Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planTY, trans2Plan, trans2Lock),
                 _T( "fftRepo.getPlan failed" ));

        trans2Plan->placeness =
            fftPlan->allOpsInplace ? CLFFT_INPLACE : CLFFT_OUTOFPLACE;
        ...
        trans2Plan->outputLayout = fftPlan->allOpsInplace
                                       ? fftPlan->inputLayout
                                       : CLFFT_COMPLEX_INTERLEAVED;
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planTY, numQueues, commQueueFFT, nullptr, nullptr),
            _T( "BakePlan large1d trans2 plan failed" ));

        // Row transform 2
        // tmp->tmp
        // size clLengths[0], batch clLengths[1]
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planY,
                                                fftPlan->context, CLFFT_1D,
                                                &clLengths[0]),
                 _T( "CreateDefaultPlan Large1d second row plan failed" ));

        FFTPlan *row2Plan = nullptr;
        lockRAII *row2Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planY, row2Plan, row2Lock),
                 _T( "fftRepo.getPlan failed" ));
        ...
        row2Plan->inputLayout = fftPlan->allOpsInplace
                                    ? fftPlan->inputLayout
                                    : CLFFT_COMPLEX_INTERLEAVED;
        row2Plan->outputLayout = fftPlan->allOpsInplace
                                     ? fftPlan->inputLayout
                                     : CLFFT_COMPLEX_INTERLEAVED;
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planY, numQueues, commQueueFFT, nullptr, nullptr),
            _T( "BakePlan large1d second row plan failed" ));

        // Transpose 3
        // tmp --> output
        OPENCL_V(clfftCreateDefaultPlanInternal(
                     &fftPlan->planTZ, fftPlan->context, CLFFT_2D, clLengths),
                 _T( "CreateDefaultPlan Large1d transpose 3 failed" ));

        FFTPlan *trans3Plan = nullptr;
        lockRAII *trans3Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planTZ, trans3Plan, trans3Lock),
                 _T( "fftRepo.getPlan failed" ));

        trans3Plan->placeness =
            fftPlan->allOpsInplace ? CLFFT_INPLACE : CLFFT_OUTOFPLACE;
        ...
        trans3Plan->inputLayout = fftPlan->allOpsInplace
                                      ? fftPlan->inputLayout
                                      : CLFFT_COMPLEX_INTERLEAVED;
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planTZ, numQueues, commQueueFFT, nullptr, nullptr),
      ...
      if ((fftPlan->inputLayout == CLFFT_REAL) && IsPo2(fftPlan->length[0]) &&
        ...
        // column FFT, size clLengths[1], batch clLengths[0], with length[0]
        // twiddle factor multiplication transposed output
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planX,
                                                fftPlan->context, CLFFT_1D,
                                                &clLengths[1]),
                 _T("CreateDefaultPlan Large1d column failed"));

        FFTPlan *colTPlan = nullptr;
        lockRAII *colLock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planX, colTPlan, colLock),
                 _T("fftRepo.getPlan failed"));
        ...
        // the distance of the first two elements in the original buffer. Like a
        // transpose of the matrix we need to pass clLengths[0] and instride
        ...
        // another column FFT, size clLengths[0], batch clLengths[1], output
        // without transpose
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planY,
                                                fftPlan->context, CLFFT_1D,
                                                &clLengths[0]),
        ...
        FFTPlan *col2Plan = nullptr;
        lockRAII *rowLock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planY, col2Plan, rowLock),
                 _T("fftRepo.getPlan failed"));

        // This is second column fft, intermediate buffer is packed and
        // interleaved we need to pass clLengths[1] and instride size to kernel,
        // so kernel can tell the difference
        ...
        // large twiddle mul cannot be done with this algorithm sequence
        ...
        if ((fftPlan->outputLayout == CLFFT_HERMITIAN_INTERLEAVED) ||
          ...
          OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planRCcopy,
                                                  fftPlan->context, CLFFT_1D,
                                                  &fftPlan->length[0]),
                   _T("CreateDefaultPlan RC copy failed"));

          FFTPlan *copyPlan = nullptr;
          lockRAII *copyLock = nullptr;
          OPENCL_V(fftRepo.getPlan(fftPlan->planRCcopy, copyPlan, copyLock),
                   _T("fftRepo.getPlan failed"));
          ...
          OPENCL_V(clfftBakePlan(fftPlan->planRCcopy, numQueues, commQueueFFT,
                                 nullptr, nullptr),
                   _T("BakePlan large1d RC copy plan failed"));
        }
        ...
        if ((fftPlan->length[0] >
          ...
          OPENCL_V(clfftCreateDefaultPlanInternal(
                       &fftPlan->planTX, fftPlan->context, CLFFT_2D, len),
          ...
          OPENCL_V(clfftBakePlan(fftPlan->planTX, numQueues, commQueueFFT, nullptr,
                                 nullptr),
                   _T( "BakePlan large1d trans1 plan failed" ));
          ...
          FFTPlan *rowPlan = nullptr;
          lockRAII *rowLock = nullptr;
          OPENCL_V(fftRepo.getPlan(fftPlan->planX, rowPlan, rowLock),
                   _T( "fftRepo.getPlan failed" ));
          ...
          OPENCL_V(clfftBakePlan(fftPlan->planX, numQueues, commQueueFFT, nullptr,
                                 nullptr),
                   _T( "BakePlan large1d first row plan failed" ));
          ...
          FFTPlan *col2Plan = nullptr;
          lockRAII *colLock = nullptr;
          OPENCL_V(fftRepo.getPlan(fftPlan->planY, col2Plan, colLock),
                   _T( "fftRepo.getPlan failed" ));
          ...
          OPENCL_V(clfftBakePlan(fftPlan->planY, numQueues, commQueueFFT, nullptr,
                                 nullptr),
                   _T( "BakePlan large1d second column plan failed" ));

          // copy plan to get results back to packed output
          OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planCopy,
                                                  fftPlan->context, CLFFT_1D,
                                                  &clLengths[0]),
          ...
          FFTPlan *copyPlan = nullptr;
          lockRAII *copyLock = nullptr;
          OPENCL_V(fftRepo.getPlan(fftPlan->planCopy, copyPlan, copyLock),
                   _T( "fftRepo.getPlan failed" ));
          ...
          OPENCL_V(clfftBakePlan(fftPlan->planCopy, numQueues, commQueueFFT,
                                 nullptr, nullptr),
                   _T( "BakePlan large1d copy plan failed" ));
        } else {
          ...
          // between each FFT is the distance of the first two elements in the
          // original buffer. Like a transpose of the matrix we need to pass
          // clLengths[0] and instride size to kernel, so kernel can tell the
          // difference
          ...
          if ((colTPlan->inStride[0] == length0) && IsPo2(fftPlan->length[0]) &&
              (fftPlan->length[0] < 524288)) {
          ...
          if (!integratedTranposes) {
            // Transpose
            // tmp --> output
            OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planTZ,
                                                    fftPlan->context, CLFFT_2D,
                                                    clLengths),
            ...
            OPENCL_V(clfftBakePlan(fftPlan->planTZ, numQueues, commQueueFFT,
                                   nullptr, nullptr),
  ...
  case CLFFT_2D: {
    ...
    if (fftPlan->transflag) // Transpose for 2D
    {
      ...
      if (fftPlan->gen == Transpose_GCN)
        fftPlan->action = new FFTGeneratedTransposeGCNAction(
            plHandle, fftPlan, *commQueueFFT, err);
      else if (fftPlan->gen == Transpose_SQUARE)
        fftPlan->action = new FFTGeneratedTransposeSquareAction(
            plHandle, fftPlan, *commQueueFFT, err);
      else if (fftPlan->gen == Transpose_NONSQUARE) {
        if (fftPlan->nonSquareKernelType != NON_SQUARE_TRANS_PARENT)
          fftPlan->action = new FFTGeneratedTransposeNonSquareAction(
              plHandle, fftPlan, *commQueueFFT, err);
        else {
          size_t clLengths[] = {1, 1, 0};
          ...
          // if leading dim is larger than the other dim it makes sense to swap
          // and transpose
          ...
          // std::cout << "currKernelOrder = " << fftPlan->nonSquareKernelOrder
          // << std::endl; ends tranpose kernel order
          ...
          if (trans1Plan->nonSquareKernelType ==
            ...
            // need to treat a non square matrix as a sqaure matrix with bigger
            // batch size
            size_t lengthX = trans1Plan->length[0];
            size_t lengthY = trans1Plan->length[1];

            size_t BatchFactor =
                (lengthX > lengthY) ? (lengthX / lengthY) : (lengthY / lengthX);
          ...
          // Transpose stage 2
          OPENCL_V(clfftCreateDefaultPlanInternal(
                       &fftPlan->planTY, fftPlan->context, CLFFT_2D, clLengths),
          ...
          OPENCL_V(clfftBakePlan(fftPlan->planTY, numQueues, commQueueFFT, nullptr,
                                 nullptr),
    ...
    while (true && (fftPlan->inputLayout != CLFFT_REAL) &&
      ...
      for (size_t Idx = 0; !isnvidia && Idx < numQueues; Idx++) {
        ...
        cl_device_id Device;
        clGetCommandQueueInfo(QIdx, CL_QUEUE_DEVICE, sizeof(Device), &Device,
                              nullptr);
        char Vendor[256];
        clGetDeviceInfo(Device, CL_DEVICE_VENDOR, sizeof(Vendor), &Vendor,
                        nullptr);
      ...
      if (fftPlan->length[1] < 32)
        break;
      // TBD: restrict the use large2D in x!=y case becase we will need two temp
      // buffers
      ...
      // interleaved format if (fftPlan->length[0] != fftPlan->length[1] &&
      // fftPlan->outputLayout == CLFFT_COMPLEX_PLANAR) break;
      if (fftPlan->inStride[0] != 1 || fftPlan->outStride[0] != 1 ||
      ...
      // CLFFT_COMPLEX_PLANAR) 	break; if (fftPlan->batchsize != 1) break; if
      // (fftPlan->precision != CLFFT_SINGLE) break;

      fftPlan->transflag = true;

      // create row plan,
      // x=y & x!=y, In->In for inplace, In->out for outofplace
      OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planX, fftPlan->context,
                                              CLFFT_1D, &fftPlan->length[DimX]),
      ...
      fftPlan->transpose_in_2d_inplace =
          (clLengths[0] == clLengths[1]) ? true : false;
      ...
      FFTPlan *transPlanX = nullptr;
      lockRAII *transLockX = nullptr;
      OPENCL_V(fftRepo.getPlan(fftPlan->planTX, transPlanX, transLockX),
               _T( "fftRepo.getPlan failed" ));
      ...
      OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planY, fftPlan->context,
                                              CLFFT_1D, &fftPlan->length[DimY]),
               _T( "CreateDefaultPlan for planY failed" ));

      FFTPlan *colPlan = nullptr;
      lockRAII *colLock = nullptr;
      OPENCL_V(fftRepo.getPlan(fftPlan->planY, colPlan, colLock),
               _T( "fftRepo.getPlan failed" ));
      ...
      OPENCL_V(clfftCreateDefaultPlanInternal(
                   &fftPlan->planTY, fftPlan->context, CLFFT_2D, clLengthsY),
               _T( "CreateDefaultPlan for planTY failed" ));

      FFTPlan *transPlanY = nullptr;
      lockRAII *transLockY = nullptr;
      OPENCL_V(fftRepo.getPlan(fftPlan->planTY, transPlanY, transLockY),
               _T( "fftRepo.getPlan failed" ));
    ...
    if (fftPlan->inputLayout == CLFFT_REAL) {
      ...
      if ((rowPlan->inStride[0] == 1) && (rowPlan->outStride[0] == 1) &&
        ...
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planTX,
                                                fftPlan->context, CLFFT_2D,
                                                transLengths),
      ...
      if ((fftPlan->inStride[0] == 1) && (fftPlan->outStride[0] == 1) &&
        ...
        FFTPlan *trans1Plan = nullptr;
        lockRAII *trans1Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planTY, trans1Plan, trans1Lock),
                 _T( "fftRepo.getPlan failed" ));
        ...
        FFTPlan *trans2Plan = nullptr;
        lockRAII *trans2Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planTX, trans2Plan, trans2Lock),
                 _T( "fftRepo.getPlan failed" ));
      ...
      // This is the row fft, the first elements distance between the first two
      // FFTs is the distance of the first elements of the first two rows in the
      // original buffer.
      rowPlan->batchsize = fftPlan->batchsize;
      rowPlan->inStride[0] = fftPlan->inStride[0];
  ...
  case CLFFT_3D: {
    if (fftPlan->inputLayout == CLFFT_REAL) {
      ...
      FFTPlan *xyPlan = nullptr;
      lockRAII *rowLock = nullptr;
      OPENCL_V(fftRepo.getPlan(fftPlan->planX, xyPlan, rowLock),
               _T( "fftRepo.getPlan failed" ));
      ...
      OPENCL_V(
          clfftBakePlan(fftPlan->planX, numQueues, commQueueFFT, nullptr, nullptr),
          _T( "BakePlan 3D->2D planX failed" ));
      ...
      if ((xyPlan->inStride[0] == 1) && (xyPlan->outStride[0] == 1) &&
        ...
        // Create column plan as a row plan
        OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planZ,
                                                fftPlan->context, CLFFT_1D,
                                                &fftPlan->length[DimZ]),
        ...
        FFTPlan *colPlan = nullptr;
        lockRAII *colLock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planZ, colPlan, colLock),
                 _T( "fftRepo.getPlan failed" ));
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planZ, numQueues, commQueueFFT, nullptr, nullptr),
        ...
        // create 1D col plan
        OPENCL_V(clfftCreateDefaultPlanInternal(
                     &fftPlan->planZ, fftPlan->context, CLFFT_1D, clLengths),
                 _T( "CreateDefaultPlan for planZ failed" ));
        ...
        OPENCL_V(
            clfftBakePlan(fftPlan->planZ, numQueues, commQueueFFT, nullptr, nullptr),
            _T( "BakePlan 3D->1D planZ failed" ));
      }
    } else if (fftPlan->outputLayout == CLFFT_REAL) {
      size_t length0 = fftPlan->length[DimX];
      size_t length1 = fftPlan->length[DimY];
      ...
      if ((fftPlan->inStride[0] == 1) && (fftPlan->outStride[0] == 1) &&
        ...
        FFTPlan *trans1Plan = nullptr;
        lockRAII *trans1Lock = nullptr;
        OPENCL_V(fftRepo.getPlan(fftPlan->planTZ, trans1Plan, trans1Lock),
                 _T( "fftRepo.getPlan failed" ));
        ...
        size_t clLengths[] = {length0, length1, 0};
        OPENCL_V(clfftCreateDefaultPlanInternal(
                     &fftPlan->planX, fftPlan->context, CLFFT_2D, clLengths),
      ...
      clLengths[1] = clLengths[2] = 0;
      // create 1D col plan
      OPENCL_V(clfftCreateDefaultPlanInternal(&fftPlan->planZ, fftPlan->context,
                                              CLFFT_1D, clLengths),
               _T( "CreateDefaultPlan for planZ failed" ));
...
clfftStatus clfftCopyPlan(clfftPlanHandle *out_plHandle, cl_context new_context,
  ...
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *in_fftPlan = nullptr, *out_fftPlan = nullptr;
  lockRAII *in_planLock = nullptr, *out_planLock = nullptr;

  OPENCL_V(fftRepo.getPlan(in_plHandle, in_fftPlan, in_planLock),
           _T( "fftRepo.getPlan failed" ));

  OPENCL_V(clfftCreateDefaultPlan(out_plHandle, new_context, in_fftPlan->dim,
                                  &in_fftPlan->length[0]),
           _T( "clfftCreateDefaultPlan failed" ));

  OPENCL_V(fftRepo.getPlan(*out_plHandle, out_fftPlan, out_planLock),
           _T( "fftRepo.getPlan failed" ));
...
clfftStatus
FFTPlan::ConstructAndEnqueueConstantBuffers(cl_command_queue *commQueueFFT) {
  ...
  ConstantBufferParams[0].u =
      std::max<cl_uint>(1, cl_uint(/*fftPlan->*/ batchsize));

  OPENCL_V(clEnqueueWriteBuffer(*commQueueFFT,
                                /*fftPlan->*/ const_buffer,
                                1, // TODO? non-blocking write?
                                0, sizeof(ConstantBufferParams),
                                &ConstantBufferParams, 0, nullptr, nullptr),
           _T("clEnqueueWriteBuffer failed"));
...
clfftStatus FFTPlan::SetEnvelope() {
  ...
  if (0 == envelope.limit_LocalMemSize)
    do {
      ...
      OPENCL_V(::clGetContextInfo(context, CL_CONTEXT_DEVICES, 0, nullptr,
                                  &deviceListSize),
               _T("Getting device array size ( ::clGetContextInfo() )" ));
      auto n = cl_uint(deviceListSize / sizeof(cl_device_id));
      if (n == 0)
        break;
      ...
      //	Get the device list
      //
      OPENCL_V(::clGetContextInfo(context, CL_CONTEXT_DEVICES, deviceListSize,
                                  &devices[0], nullptr),
      ...
      OPENCL_V(
          ::clGetDeviceInfo(devices[0], CL_DEVICE_VERSION, 0, nullptr,
                            &deviceVersionSize),
          _T("Getting CL_DEVICE_VERSION Info string size ( ::clGetDeviceInfo() )" ));

      std::vector<char> szDeviceVersion(deviceVersionSize);
      OPENCL_V(
          ::clGetDeviceInfo(devices[0], CL_DEVICE_VERSION, deviceVersionSize,
                            &szDeviceVersion[0], nullptr),
      ...
      if (!strncmp((const char *)&szDeviceVersion[0], openclstr, 10)) {
        ...
        OPENCL_V(
            ::clGetContextInfo(context, CL_CONTEXT_NUM_DEVICES,
                               sizeof(cContextDevices), &cContextDevices, nullptr),
      ...
      for (unsigned long & u : envelope.limit_Size) {
        u = 256;
      }
      ...
      for (cl_uint i = 0; i < cContextDevices; ++i) {
        ...
        OPENCL_V(::clGetDeviceInfo(devId, CL_DEVICE_LOCAL_MEM_SIZE,
                                   sizeof(cl_ulong), &memsize, nullptr),
                 _T("Getting CL_DEVICE_LOCAL_MEM_SIZE device info ( ")
                 _T("::clGetDeviceInfo() )"));
        envelope.limit_LocalMemSize =
            std::min<size_t>(envelope.limit_LocalMemSize, memsize);

        OPENCL_V(::clGetDeviceInfo(devId, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                                   sizeof(unsigned int), &maxdim, nullptr),
                 _T("Getting CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS device info ( ")
                 _T("::clGetDeviceInfo() )"));
        BUG_CHECK(countOf(envelope.limit_Size) >= maxdim);
        envelope.limit_Dimensions =
            std::min<size_t>(envelope.limit_Dimensions, maxdim);

        OPENCL_V(::clGetDeviceInfo(devId, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                                   sizeof(size_t), &temp[0], nullptr),
                 _T("Getting CL_DEVICE_MAX_WORK_GROUP_SIZE device info ( ")
                 _T("::clGetDeviceInfo() )"));
        envelope.limit_WorkGroupSize =
            std::min<size_t>(envelope.limit_WorkGroupSize, temp[0]);

        OPENCL_V(::clGetDeviceInfo(devId, CL_DEVICE_MAX_WORK_ITEM_SIZES,
                                   sizeof(temp), &temp[0], nullptr),
                 _T("Getting CL_DEVICE_MAX_WORK_ITEM_SIZES device info ( ")
                 _T("::clGetDeviceInfo() )"));
        for (size_t u = 0; u < envelope.limit_Dimensions; ++u) {
          BUG_CHECK(temp[u] > 0)
          envelope.limit_Size[u] =
              std::min<size_t>(envelope.limit_Size[u], temp[u]);
        }
      }
...
clfftStatus FFTPlan::AllocateBuffers() {
  ...
  do {
    const_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                  CLFFT_CB_SIZE * sizeof(int), nullptr, &status);
...
clfftStatus FFTPlan::ReleaseBuffers() {
  ...
  if ((nullptr != intBuffer) && libCreatedIntBuffer) {
...
clfftStatus FFTPlan::GetMax1DLength(size_t *longest) const {
  switch (gen) {
  case Stockham:
    return GetMax1DLengthStockham(longest);
  case Transpose_GCN:
    *longest = 4096;
    return CLFFT_SUCCESS;
  case Transpose_SQUARE:
...
size_t FFTPlan::ElementSize() const {
  return (((precision == CLFFT_DOUBLE) || (precision == CLFFT_DOUBLE_FAST))
