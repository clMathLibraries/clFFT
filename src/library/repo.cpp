...
//	Handle/Address of the dynamic module that contains the timer, that we
//discover and load during runtime
void *FFTRepo::timerHandle = nullptr;
GpuStatTimer *FFTRepo::pStatTimer = nullptr;
...
clfftStatus FFTRepo::releaseResources() {
  ...
  //
  for (auto & mapKernel : mapKernels) {
    cl_kernel k = mapKernel.second.kernel_fwd;
    mapKernel.second.kernel_fwd = nullptr;
    if (nullptr != k)
      clReleaseKernel(k);
    k = mapKernel.second.kernel_back;
    mapKernel.second.kernel_back = nullptr;
    if (nullptr != k)
      clReleaseKernel(k);

    if (nullptr != mapKernel.second.kernel_fwd_lock) {
      delete mapKernel.second.kernel_fwd_lock;
      mapKernel.second.kernel_fwd_lock = nullptr;
    }

    if (nullptr != mapKernel.second.kernel_back_lock) {
      delete mapKernel.second.kernel_back_lock;
      mapKernel.second.kernel_back_lock = nullptr;
    }
  }
  mapKernels.clear();
  ...
  //
  for (auto & mapFFT : mapFFTs) {
    if (mapFFT.first.data != nullptr) {
      const_cast<FFTRepoKey *>(&mapFFT.first)->deleteData();
    }

    cl_program p = mapFFT.second.clProgram;
    mapFFT.second.clProgram = nullptr;
    if (nullptr != p)
      clReleaseProgram(p);
  }
  ...
  //
  for (auto & repoPlan : repoPlans) {
    FFTPlan *plan = repoPlan.second.first;
    lockRAII *lock = repoPlan.second.second;
    if (plan != nullptr) {
      delete plan;
    }
    if (lock != nullptr) {
...
clfftStatus FFTRepo::setProgramCode(const clfftGenerators gen,
...
clfftStatus FFTRepo::getProgramCode(const clfftGenerators gen,
                                    const FFTKernelSignatureHeader *data,
                                    std::string &kernel,
                                    const cl_device_id &device,
                                    const cl_context &planContext) {
...
clfftStatus FFTRepo::setProgramEntryPoints(const clfftGenerators gen,
                                           const FFTKernelSignatureHeader *data,
...
clfftStatus FFTRepo::getProgramEntryPoint(const clfftGenerators gen,
                                          const FFTKernelSignatureHeader *data,
                                          clfftDirection dir,
                                          std::string &kernel,
                                          const cl_device_id &device,
...
clfftStatus FFTRepo::setclProgram(const clfftGenerators gen,
                                  const FFTKernelSignatureHeader *data,
...
clfftStatus FFTRepo::getclProgram(const clfftGenerators gen,
                                  const FFTKernelSignatureHeader *data,
  ...
  clGetProgramInfo(prog, CL_PROGRAM_CONTEXT, sizeof(cl_context), &progContext,
                   nullptr);
...
clfftStatus FFTRepo::setclKernel(cl_program prog, clfftDirection dir,
...
clfftStatus FFTRepo::getclKernel(cl_program prog, clfftDirection dir,
                                 cl_kernel &kernel, lockRAII *&kernelLock) {
  scopedLock sLock(lockRepo(), _T( "getclKernel" ));

  auto pos = mapKernels.find(prog);
