...
FFTGeneratedTransposeGCNAction::FFTGeneratedTransposeGCNAction(
    clfftPlanHandle plHandle, FFTPlan *plan, cl_command_queue queue,
    clfftStatus &err)
...
inline std::stringstream &clKernWrite(std::stringstream &rhs,
...
static void OffsetCalc(std::stringstream &transKernel,
  ...
  for (size_t i = params.fft_DataDim - 2; i > 0; i--) {
    clKernWrite(transKernel, 3)
        << offset << " += (currDimIndex/numGroupsY_" << i << ")*"
        << stride[i + 1] << ";" << std::endl;
    clKernWrite(transKernel, 3)
        << "currDimIndex = currDimIndex % numGroupsY_" << i << ";" << std::endl;
  }

  clKernWrite(transKernel, 3)
      << "rowSizeinUnits = " << stride[1] << ";" << std::endl;
...
static clfftStatus genTwiddleMath(const FFTKernelGenKeyParams &params,
                                  std::stringstream &transKernel,
                                  const std::string &dtComplex, bool fwd) {
  clKernWrite(transKernel, 9)
      << dtComplex
      << " W = TW3step( (groupIndex.x * wgTileExtent.x + xInd) * (currDimIndex "
  ...
  if (fwd) {
    clKernWrite(transKernel, 9)
        << "T.x = ( W.x * tmp.x ) - ( W.y * tmp.y );" << std::endl;
    clKernWrite(transKernel, 9)
        << "T.y = ( W.y * tmp.x ) + ( W.x * tmp.y );" << std::endl;
  } else {
    clKernWrite(transKernel, 9)
        << "T.x =  ( W.x * tmp.x ) + ( W.y * tmp.y );" << std::endl;
    clKernWrite(transKernel, 9)
...
static clfftStatus
genTransposePrototype(const FFTGeneratedTransposeGCNAction::Signature &params,
                      const tile &lwSize, const std::string &dtPlanar,
  ...
  clKernWrite(transKernel, 0)
      << "__attribute__(( reqd_work_group_size( " << lwSize.x << ", "
      << lwSize.y << ", 1 ) ))" << std::endl;
  ...
  case CLFFT_COMPLEX_INTERLEAVED:
    dtInput = dtComplex;
    clKernWrite(transKernel, 0)
        << "global " << dtInput << "* restrict " << pmComplexIn;
    ...
    case CLFFT_OUTOFPLACE:
      ...
      case CLFFT_COMPLEX_INTERLEAVED:
        dtOutput = dtComplex;
        clKernWrite(transKernel, 0)
            << ", global " << dtOutput << "* restrict " << pmComplexOut;
        break;
      case CLFFT_COMPLEX_PLANAR:
        dtOutput = dtPlanar;
        clKernWrite(transKernel, 0)
            << ", global " << dtOutput << "* restrict " << pmRealOut
  ...
  case CLFFT_COMPLEX_PLANAR:
    dtInput = dtPlanar;
    clKernWrite(transKernel, 0)
        << "global " << dtInput << "* restrict " << pmRealIn << ", global "
        << dtInput << "* restrict " << pmImagIn;
  ...
  case CLFFT_REAL:
    dtInput = dtPlanar;
    clKernWrite(transKernel, 0)
        << "global " << dtInput << "* restrict " << pmRealIn;
...
static clfftStatus
genTransposeKernel(const FFTGeneratedTransposeGCNAction::Signature &params,
                   std::string &strKernel, const tile &lwSize,
                   const size_t reShapeFactor, const size_t loopCount,
                   const tile &blockSize) {
  ...
  clKernWrite(transKernel, 0)
      << "// Local structure to embody/capture tile dimensions" << std::endl;
  ...
  for (size_t bothDir = 0; bothDir < 2; bothDir++) {
    ...
    clKernWrite(transKernel, 3)
        ...
        << std::endl;
    clKernWrite(transKernel, 3) << "const Tile localExtent = { get_local_size( "
                                   "0 ), get_local_size( 1 ) }; "
                                << std::endl;
    clKernWrite(transKernel, 3)
    ...
    clKernWrite(transKernel, 3)
        ...
        << std::endl;
    clKernWrite(transKernel, 3) << "// Transpose of input & output blocks "
                                   "happens with the Offset calculation"
                                << std::endl;
    clKernWrite(transKernel, 3)
        << "const size_t reShapeFactor = " << reShapeFactor << ";" << std::endl;
    clKernWrite(transKernel, 3)
        << "const size_t wgUnroll = " << loopCount << ";" << std::endl;
    clKernWrite(transKernel, 3)
    ...
    for (int i = 2; i < params.fft_DataDim - 1; i++) {
      ...
      clKernWrite(transKernel, 3) << "const size_t numGroupsY_" << i << " = "
                                  << numGroupsTemp << ";" << std::endl;
    }
    ...
    clKernWrite(transKernel, 3) << "size_t rowSizeinUnits;" << std::endl
                                << std::endl;
    ...
    switch (params.fft_inputLayout) {
    case CLFFT_COMPLEX_INTERLEAVED:
      // No need of tileIn declaration when precallback is set as the global
      // buffer is used directly
      if (!params.fft_hasPreCallback) {
        clKernWrite(transKernel, 3)
            << "global " << dtInput << "* tileIn = " << pmComplexIn
            << " + iOffset;" << std::endl;
      }
      break;
    case CLFFT_COMPLEX_PLANAR:
      ...
      if (!params.fft_hasPreCallback) {
        clKernWrite(transKernel, 3)
            << "global " << dtInput << "* realTileIn = " << pmRealIn
            << " + iOffset;" << std::endl;
        clKernWrite(transKernel, 3)
            << "global " << dtInput << "* imagTileIn = " << pmImagIn
            << " + iOffset;" << std::endl;
      }
      break;
    case CLFFT_HERMITIAN_INTERLEAVED:
    ...
    case CLFFT_REAL:
      ...
      if (!params.fft_hasPreCallback) {
        clKernWrite(transKernel, 3)
            << "global " << dtInput << "* tileIn = " << pmRealIn
            << " + iOffset;" << std::endl;
      }
      break;
    }
    ...
    clKernWrite(transKernel, 3)
        << "rowSizeinUnits = " << params.fft_inStride[1] << ";" << std::endl;
    clKernWrite(transKernel, 3) << std::endl << std::endl;

    //
    // Group index traversal is logical where X direction is horizontal in input
    // buffer and vertical in output buffer when transOutHorizontal is enabled X
    // direction is vertical in input buffer and horizontal in output buffer Not
    // to be confused within a tile, where X is horizontal in input and vertical
    // in output always
    ...
    for (size_t i = 0; i < branchBlocks; i++) {
      ...
      clKernWrite(transKernel, 6)
          << "for( uint t=0; t < wgUnroll; t++ )" << std::endl;
      ...
      if (branchingInBoth) {
        if (i == 0) {
          ...
          clKernWrite(transKernel, 9) << std::endl;
          clKernWrite(transKernel, 9) << "if( (" << wIndexX << " < "
                                      << wIndexXEnd << ") )" << std::endl;
          clKernWrite(transKernel, 9) << "{" << std::endl;
        } else if (i == 2) {
          clKernWrite(transKernel, 9) << std::endl;
          clKernWrite(transKernel, 9) << "if( (" << wIndexY << " < "
                                      << wIndexYEnd << ") )" << std::endl;
          clKernWrite(transKernel, 9) << "{" << std::endl;
        } else
          clKernWrite(transKernel, 9) << "{" << std::endl;
      ...
      case CLFFT_COMPLEX_INTERLEAVED: {
        if (params.fft_hasPreCallback) {
          if (params.fft_preCallback.localMemSize > 0) {
            clKernWrite(transKernel, 9)
                << "tmp = " << params.fft_preCallback.funcname << "("
                << pmComplexIn << ", iOffset + gInd, pre_userdata, localmem);"
                << std::endl;
          } else {
            clKernWrite(transKernel, 9)
                << "tmp = " << params.fft_preCallback.funcname << "("
                << pmComplexIn << ", iOffset + gInd, pre_userdata);"
                << std::endl;
          }
        } else {
      ...
      case CLFFT_COMPLEX_PLANAR: {
        if (params.fft_hasPreCallback) {
          if (params.fft_preCallback.localMemSize > 0) {
            clKernWrite(transKernel, 9)
                << "retCallback = " << params.fft_preCallback.funcname << "("
                << pmRealIn << ", " << pmImagIn
                << ", iOffset + gInd, pre_userdata, localmem);" << std::endl;
          } else {
            clKernWrite(transKernel, 9)
                << "retCallback = " << params.fft_preCallback.funcname << "("
                << pmRealIn << ", " << pmImagIn
                << ", iOffset + gInd, pre_userdata);" << std::endl;
          }
          clKernWrite(transKernel, 9) << "tmp.s0 = retCallback.x;" << std::endl;
          clKernWrite(transKernel, 9) << "tmp.s1 = retCallback.y;" << std::endl;
        } else {
          clKernWrite(transKernel, 9)
              << "tmp.s0 = realTileIn[ gInd ];" << std::endl;
          clKernWrite(transKernel, 9)
              << "tmp.s1 = imagTileIn[ gInd ];" << std::endl;
        }
      } break;
      case CLFFT_HERMITIAN_INTERLEAVED:
      case CLFFT_HERMITIAN_PLANAR:
      ...
      case CLFFT_REAL:
        if (params.fft_hasPreCallback) {
          if (params.fft_preCallback.localMemSize > 0) {
            clKernWrite(transKernel, 9)
                << "tmp = " << params.fft_preCallback.funcname << "("
                << pmRealIn << ", iOffset + gInd, pre_userdata, localmem);"
                << std::endl;
          } else {
            clKernWrite(transKernel, 9)
                << "tmp = " << params.fft_preCallback.funcname << "("
                << pmRealIn << ", iOffset + gInd, pre_userdata);" << std::endl;
          }
        } else {
          clKernWrite(transKernel, 9) << "tmp = tileIn[ gInd ];" << std::endl;
        }
      ...
      clKernWrite(transKernel, 9)
          << "// Transpose of Tile data happens here" << std::endl;
    ...
    switch (params.fft_outputLayout) {
    case CLFFT_COMPLEX_INTERLEAVED:
      // No need of tileOut declaration when postcallback is set as the global
      // buffer is used directly
      if (!params.fft_hasPostCallback) {
        clKernWrite(transKernel, 3)
            << "global " << dtOutput << "* tileOut = " << pmComplexOut
            << " + oOffset;" << std::endl
            << std::endl;
      }
      break;
    case CLFFT_COMPLEX_PLANAR:
      ...
      if (!params.fft_hasPostCallback) {
        clKernWrite(transKernel, 3)
            << "global " << dtOutput << "* realTileOut = " << pmRealOut
            << " + oOffset;" << std::endl;
        clKernWrite(transKernel, 3)
            << "global " << dtOutput << "* imagTileOut = " << pmImagOut
            << " + oOffset;" << std::endl;
      }
      break;
    case CLFFT_HERMITIAN_INTERLEAVED:
    ...
    case CLFFT_REAL:
      clKernWrite(transKernel, 3)
          << "global " << dtOutput << "* tileOut = " << pmRealOut
          << " + oOffset;" << std::endl
          << std::endl;
      break;
    }
    ...
    for (size_t i = 0; i < branchBlocks; i++) {
      ...
      clKernWrite(transKernel, 9)
          << "size_t xInd = localIndex.x + localExtent.x * ( localIndex.y % "
             "groupingPerY ); "
          << std::endl;
      clKernWrite(transKernel, 9) << "size_t yInd = localIndex.y/groupingPerY "
      ...
      case CLFFT_COMPLEX_INTERLEAVED:
        if (params.fft_hasPostCallback) {
          clKernWrite(transKernel, 9)
              << params.fft_postCallback.funcname << "(" << pmComplexOut
              << ", (oOffset + gInd), post_userdata, tmp";
          if (params.fft_postCallback.localMemSize > 0) {
      ...
      case CLFFT_COMPLEX_PLANAR:
        if (params.fft_hasPostCallback) {
          clKernWrite(transKernel, 9)
              << params.fft_postCallback.funcname << "(" << pmRealOut << ", "
              << pmImagOut
              << ", (oOffset + gInd), post_userdata, tmp.s0, tmp.s1";
          if (params.fft_postCallback.localMemSize > 0) {
            clKernWrite(transKernel, 0) << ", localmem";
          }
          clKernWrite(transKernel, 0) << ");" << std::endl;
          ...
          clKernWrite(transKernel, 9)
              << "realTileOut[ gInd ] = tmp.s0;" << std::endl;
          clKernWrite(transKernel, 9)
              << "imagTileOut[ gInd ] = tmp.s1;" << std::endl;
        }
        break;
      case CLFFT_HERMITIAN_INTERLEAVED:
      case CLFFT_HERMITIAN_PLANAR:
...
clfftStatus FFTGeneratedTransposeGCNAction::initParams() {
  ...
  this->signature.transOutHorizontal =
      this->plan->transOutHorizontal; // using the twiddle front flag to specify
                                      // horizontal write we do this so as to
                                      // reuse flags in FFTKernelGenKeyParams
  ...
  if (CLFFT_INPLACE == this->signature.fft_placeness) {
    ...
    //	*MUST* be the same.
    //
    ARG_CHECK(this->signature.fft_inputLayout ==
              this->signature.fft_outputLayout)
  ...
  // TODO:  Since I am going with a 2D workgroup size now, I need a better check
  // than this 1D use Check:
  // CL_DEVICE_MAX_WORK_GROUP_SIZE/CL_KERNEL_WORK_GROUP_SIZE
  // CL_DEVICE_MAX_WORK_ITEM_SIZES
  this->signature.fft_R = 1; // Dont think i'll use
  this->signature.fft_SIMD =
...
static clfftStatus CalculateBlockSize(const clfftPrecision precision,
...
//	OpenCL does not take unicode strings as input, so this routine returns
//only ASCII strings 	Feed this generator the FFTPlan, and it returns the
//generated program as a string
clfftStatus FFTGeneratedTransposeGCNAction::generateKernel(
    FFTRepo &fftRepo, const cl_command_queue commQueueFFT) {

  size_t loopCount = 0;
  tile blockSize = {0, 0};
  OPENCL_V(
      CalculateBlockSize(this->signature.fft_precision, loopCount, blockSize),
      _T("CalculateBlockSize() failed!"));
  ...
  if ((this->signature.fft_hasPreCallback &&
    ...
    else if (this->signature.fft_hasPostCallback &&
             this->signature.fft_postCallback.localMemSize > 0)
      requestedCallbackLDS = this->signature.fft_postCallback.localMemSize;

    validLDSSize =
        ((length * this->plan->ElementSize()) + requestedCallbackLDS) <
  ...
  OPENCL_V(genTransposeKernel(this->signature, programCode, lwSize,
                              reShapeFactor, loopCount, blockSize),
           _T( "GenerateTransposeKernel() failed!" ));
  ...
  OPENCL_V(fftRepo.setProgramCode(Transpose_GCN, this->getSignatureData(),
                                  programCode, Device, QueueContext),
           _T( "fftRepo.setclString() failed!" ));

  // Note:  See genFunctionPrototype( )
  if (this->signature.fft_3StepTwiddle) {
    OPENCL_V(fftRepo.setProgramEntryPoints(
                 Transpose_GCN, this->getSignatureData(),
                 "transpose_gcn_tw_fwd", "transpose_gcn_tw_back", Device,
                 QueueContext),
             _T( "fftRepo.setProgramEntryPoint() failed!" ));
  } else {
    OPENCL_V(fftRepo.setProgramEntryPoints(
                 Transpose_GCN, this->getSignatureData(), "transpose_gcn",
                 "transpose_gcn", Device, QueueContext),
             _T( "fftRepo.setProgramEntryPoint() failed!" ));
  }
...
clfftStatus
FFTGeneratedTransposeGCNAction::getWorkSizes(std::vector<size_t> &globalWS,
