...
enum {
  ...
  LDS_PADDING =
      false, // true,
             ...
             //  banks of the LDS. 2 ** LDS_BANK_BITS is the number of LDS
             //  banks. If LDS_PADDING is non-zero, the kernel generator should
             //  pad the LDS arrays to reduce or eliminate bank conflicts.
  ...
  LDS_FRACTION_MAX =
      4, // i.e., 1/4
         ...
         //  of LDS per workgroup when necessary to perform the FFT in a single
         //  pass instead of multiple passes. This tuning parameter is a good
         //  value for Evergreen gpus, but might change in the future.
  ...
  //  LDS_COMPLEX, then we declare an LDS array or scalar numbers (4 bytes each)
  ...
  //  The latter uses half as much LDS space, so twice as many wavefronts can be
  //  run in parallel.
...
//	Sorting operator for struct FFTKernelGenKeyParams, such that it can be
//used in a map
bool operator<(const FFTKernelGenKeyParams &lhs,
               const FFTKernelGenKeyParams &rhs);
...
public:
  ...
  virtual clfftStatus enqueue(clfftPlanHandle plHandle, clfftDirection dir,
...
protected:
  ...
  clfftStatus compileKernels(const cl_command_queue commQueueFFT,
                             const clfftPlanHandle plHandle, FFTPlan *fftPlan);
  clfftStatus writeKernel(const clfftPlanHandle plHandle,
                          const clfftGenerators gen,
                          const FFTKernelSignatureHeader *data,
                          const cl_context &context,
                          const cl_device_id &device);

  virtual clfftStatus generateKernel(FFTRepo &fftRepo,
                                     const cl_command_queue commQueueFFT) = 0;
  virtual clfftStatus getWorkSizes(std::vector<size_t> &globalws,
                                   std::vector<size_t> &localws) = 0;
...
private:
  clfftStatus selectBufferArguments(FFTPlan *plan, cl_mem *clInputBuffers,
                                    cl_mem *clOutputBuffers,
...
//	This class contains objects that are specific to a particular FFT
//transform, and the data herein is useful 	for us to know ahead of transform
//time such that we can optimize for these settings
class FFTPlan {
...
public:
  ...
  FFTPlan()
      : baked(false), dim(CLFFT_1D), inputLayout(CLFFT_COMPLEX_INTERLEAVED),
        outputLayout(CLFFT_COMPLEX_INTERLEAVED), placeness(CLFFT_INPLACE),
        transposed(CLFFT_NOTRANSPOSE), precision(CLFFT_SINGLE), context(nullptr),
        forwardScale(1.0), backwardScale(1.0), iDist(1), oDist(1), batchsize(1),
        tmpBufSize(0), intBuffer(nullptr), libCreatedIntBuffer(false),
        tmpBufSizeRC(0), intBufferRC(nullptr), tmpBufSizeC2R(0),
        intBufferC2R(nullptr), large1D(0), large2D(false), twiddleFront(false),
        planX(0), planY(0), planZ(0), transflag(false),
        transOutHorizontal(false), RCsimple(false), realSpecial(false),
        realSpecial_Nr(0), userPlan(false), allOpsInplace(false),
        transpose_in_2d_inplace(false), blockCompute(false),
        blockComputeType(BCT_C2C), planTX(0), planTY(0), planTZ(0),
        planRCcopy(0), planCopy(0), const_buffer(nullptr), gen(Stockham),
        action(nullptr), nonSquareKernelType(NON_SQUARE_TRANS_PARENT),
        transposeMiniBatchSize(1), nonSquareKernelOrder(NOT_A_TRANSPOSE),
        plHandle(0), hasPreCallback(false), hasPostCallback(false), offsetIn(0),
        offsetOut(0){};
  ...
  clfftStatus
  ConstructAndEnqueueConstantBuffers(cl_command_queue *commQueueFFT);
