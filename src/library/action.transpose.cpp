...
FFTGeneratedTransposeNonSquareAction::FFTGeneratedTransposeNonSquareAction(
  ...
  if (err != CLFFT_SUCCESS) {
    fprintf(stderr,
            "FFTGeneratedTransposeNonSquareAction::initParams() failed!\n");
    return;
  }
  ...
  if (err != CLFFT_SUCCESS) {
    fprintf(stderr,
            "FFTGeneratedTransposeNonSquareAction::generateKernel failed\n");
    return;
  }
  ...
  if (err != CLFFT_SUCCESS) {
    fprintf(stderr,
            "FFTGeneratedTransposeNonSquareAction::compileKernels failed\n");
    return;
  }
...
clfftStatus FFTGeneratedTransposeNonSquareAction::initParams() {
  ...
  if (this->plan->large1D != 0) {
    ...
    // ToDo:ENABLE ASSERT
    // ARG_CHECK(this->plan->large1D == (this->signature.fft_N[1] *
...
clfftStatus FFTGeneratedTransposeNonSquareAction::generateKernel(
  ...
  if (this->signature.nonSquareKernelType ==
    ...
    if (this->signature.fft_hasPreCallback &&
      ...
      validLDSSize =
          ((2 * this->plan->ElementSize() * 16 * reShapeFactor * 16 *
            reShapeFactor) +
    ...
    OPENCL_V(
        clfft_transpose_generator::genTransposeKernelLeadingDimensionBatched(
            this->signature, programCode, lwSize, reShapeFactor),
        _T("genTransposeKernel() failed!"));
  } else if (this->signature.nonSquareKernelType ==
    ...
    OPENCL_V(clfft_transpose_generator::genTransposeKernelBatched(
                 this->signature, programCode, lwSize, reShapeFactor),
             _T("genTransposeKernel() failed!"));
  } else {
    ...
    if (this->signature.fft_hasPreCallback &&
      ...
      size_t lineSize = (this->signature.fft_N[0]) < (this->signature.fft_N[1])
                            ? this->signature.fft_N[0]
                            : this->signature.fft_N[1];
      validLDSSize =
          ((2 * this->plan->ElementSize() * lineSize) + requestedCallbackLDS) <
    ...
    // here we should decide generate what kind of swap kernel. 1:2 and 1:3
    // probably need different swap kernels
    /*
    if (this->signature.fft_N[0] == 2 * this->signature.fft_N[1] || 2 *
    this->signature.fft_N[0] == this->signature.fft_N[1])
    {
            OPENCL_V(clfft_transpose_generator::genSwapKernel(this->signature,
    programCode, kernelFuncName, lwSize, reShapeFactor), _T("genSwapKernel()
    failed!"));
    }
    else
    {
            OPENCL_V(clfft_transpose_generator::genSwapKernelGeneral(this->signature,
    programCode, kernelFuncName, lwSize, reShapeFactor), _T("genSwapKernel()
    failed!"));
    }
    */
    // general swap kernel takes care of all ratio
    OPENCL_V(clfft_transpose_generator::genSwapKernelGeneral(
                 this->signature, programCode, kernelFuncName, lwSize,
                 reShapeFactor),
             _T("genSwapKernel() failed!"));
  }
  ...
  OPENCL_V(fftRepo.setProgramCode(Transpose_NONSQUARE, this->getSignatureData(),
  ...
  if (this->signature.nonSquareKernelType ==
    ...
    if (this->signature.fft_3StepTwiddle) {
      OPENCL_V(fftRepo.setProgramEntryPoints(
                   Transpose_NONSQUARE, this->getSignatureData(),
                   "transpose_nonsquare_tw_fwd", "transpose_nonsquare_tw_back",
                   Device, QueueContext),
               _T("fftRepo.setProgramEntryPoint() failed!"));
    } else {
      OPENCL_V(fftRepo.setProgramEntryPoints(
                   Transpose_NONSQUARE, this->getSignatureData(),
                   "transpose_nonsquare", "transpose_nonsquare", Device,
                   QueueContext),
               _T("fftRepo.setProgramEntryPoint() failed!"));
    }
  } else if (this->signature.nonSquareKernelType ==
    ...
    /*
            if (this->signature.fft_3StepTwiddle &&
       (this->signature.transposeMiniBatchSize == 1))
            {
                    OPENCL_V(fftRepo.setProgramEntryPoints(Transpose_NONSQUARE,
       this->getSignatureData(), "transpose_square_tw_fwd",
       ...
       _T("fftRepo.setProgramEntryPoint() failed!"));
            ...
            {
                    OPENCL_V(fftRepo.setProgramEntryPoints(Transpose_NONSQUARE,
       this->getSignatureData(), "transpose_square", "transpose_square", Device,
       QueueContext), _T("fftRepo.setProgramEntryPoint() failed!"));
            }
    */
    OPENCL_V(fftRepo.setProgramEntryPoints(
                 Transpose_NONSQUARE, this->getSignatureData(),
                 "transpose_square", "transpose_square", Device, QueueContext),
             _T("fftRepo.setProgramEntryPoint() failed!"));
  } else {
    if (this->signature.fft_3StepTwiddle) // if miniBatchSize > 1 twiddling is
                                          // done in swap kernel
    {
      ...
      OPENCL_V(fftRepo.setProgramEntryPoints(
                   Transpose_NONSQUARE, this->getSignatureData(),
                   kernelFwdFuncName.c_str(), kernelBwdFuncName.c_str(), Device,
                   QueueContext),
               _T("fftRepo.setProgramEntryPoint() failed!"));
    } else
      OPENCL_V(fftRepo.setProgramEntryPoints(
                   Transpose_NONSQUARE, this->getSignatureData(),
                   kernelFuncName.c_str(), kernelFuncName.c_str(), Device,
                   QueueContext),
               _T("fftRepo.setProgramEntryPoint() failed!"));
  }
  return CLFFT_SUCCESS;
...
clfftStatus FFTGeneratedTransposeNonSquareAction::getWorkSizes(
    std::vector<size_t> &globalWS, std::vector<size_t> &localWS) {

  size_t wg_slice;
  size_t smaller_dim = (this->signature.fft_N[0] < this->signature.fft_N[1])
                           ? this->signature.fft_N[0]
                           : this->signature.fft_N[1];
  size_t bigger_dim = (this->signature.fft_N[0] >= this->signature.fft_N[1])
                          ? this->signature.fft_N[0]
                          : this->signature.fft_N[1];
  ...
  if (this->signature.nonSquareKernelType ==
    ...
    global_item_size =
        wg_slice * (wg_slice + 1) / 2 * 16 * 16 * this->plan->batchsize;
    ...
    if (dim_ratio == 2 && false) {
       ...
       * design*/
      clfft_transpose_generator::get_cycles(cycle_map, num_reduced_row,
                                            num_reduced_col);

      global_item_size = local_work_size_swap * num_grps_pro_row *
                         cycle_map[0] * this->plan->batchsize;
      ...
      if (dim_ratio % 2 == 0 || dim_ratio % 3 == 0 || dim_ratio % 5 == 0 ||
        ...
        while (
            LDS_per_WG >
            1024) // avoiding using too much lds memory. the biggest LDS memory
...
FFTGeneratedTransposeSquareAction::FFTGeneratedTransposeSquareAction(
    clfftPlanHandle plHandle, FFTPlan *plan, cl_command_queue queue,
    clfftStatus &err)
  ...
  if (err != CLFFT_SUCCESS) {
    fprintf(stderr,
            "FFTGeneratedTransposeSquareAction::initParams() failed!\n");
    return;
  }
  ...
  if (err != CLFFT_SUCCESS) {
    fprintf(stderr,
            "FFTGeneratedTransposeSquareAction::generateKernel failed\n");
    return;
  }
  ...
  if (err != CLFFT_SUCCESS) {
    fprintf(stderr,
            "FFTGeneratedTransposeSquareAction::compileKernels failed\n");
    return;
  }
...
//only ASCII strings 	Feed this generator the FFTPlan, and it returns the
//generated program as a string
clfftStatus FFTGeneratedTransposeSquareAction::generateKernel(
    FFTRepo &fftRepo, const cl_command_queue commQueueFFT) {
  // Requested local memory size by callback must not exceed the device LDS
  ...
  OPENCL_V(fftRepo.setProgramCode(Transpose_SQUARE, this->getSignatureData(),
                                  programCode, Device, QueueContext),
           _T("fftRepo.setclString() failed!"));
  ...
  if (this->signature.fft_3StepTwiddle) {
    OPENCL_V(fftRepo.setProgramEntryPoints(
                 Transpose_SQUARE, this->getSignatureData(),
                 "transpose_square_tw_fwd", "transpose_square_tw_back", Device,
                 QueueContext),
             _T("fftRepo.setProgramEntryPoint() failed!"));
  } else {
    OPENCL_V(fftRepo.setProgramEntryPoints(
                 Transpose_SQUARE, this->getSignatureData(), "transpose_square",
                 "transpose_square", Device, QueueContext),
             _T("fftRepo.setProgramEntryPoint() failed!"));
  }
...
clfftStatus
FFTGeneratedTransposeSquareAction::getWorkSizes(std::vector<size_t> &globalWS,
