...
FFTCopyAction::FFTCopyAction(clfftPlanHandle plHandle, FFTPlan *plan,
...
FFTTransposeGCNAction::FFTTransposeGCNAction(clfftPlanHandle plHandle,
                                             FFTPlan *plan,
...
FFTTransposeSquareAction::FFTTransposeSquareAction(clfftPlanHandle plHandle,
                                                   FFTPlan *plan,
                                                   cl_command_queue queue,
                                                   clfftStatus &err)
    : FFTAction(plan, err) {
...
FFTTransposeNonSquareAction::FFTTransposeNonSquareAction(
...
FFTStockhamAction::FFTStockhamAction(clfftPlanHandle plHandle, FFTPlan *plan,
                                     cl_command_queue queue, clfftStatus &err)
    : FFTAction(plan, err) {
  if (err != CLFFT_SUCCESS) {
    // FFTAction() failed, exit constructor
...
FFTAction::FFTAction(FFTPlan *fftPlan, clfftStatus &err) : plan(fftPlan) {
  err = CLFFT_SUCCESS;
}
...
clfftStatus FFTAction::enqueue(clfftPlanHandle plHandle, clfftDirection dir,
  ...
  clfftStatus status = selectBufferArguments(
      this->plan, clInputBuffers, clOutputBuffers, inputBuff, outputBuff);
  ...
  cl_kernel kern;
  lockRAII *kernelLock;
  OPENCL_V(fftRepo.getclProgram(this->getGenerator(), this->getSignatureData(),
                                prog, this->plan->bakeDevice,
                                this->plan->context),
           _T( "fftRepo.getclProgram failed" ));
  OPENCL_V(fftRepo.getclKernel(prog, dir, kern, kernelLock),
           _T( "fftRepo.getclKernels failed" ));

  scopedLock sLock(*kernelLock, _T("FFTAction::enqueue"));

  cl_uint uarg = 0;
  if (!this->plan->transflag && !(this->plan->gen == Copy)) {
    //	::clSetKernelArg() is not thread safe, according to the openCL spec for
    //the same cl_kernel object
    //	TODO:  Need to verify that two different plans (which would get through
    //our lock above) with exactly the same 	parameters would NOT share the same
    //cl_kernel objects

    /* constant buffer */
    OPENCL_V(clSetKernelArg(kern, uarg++, sizeof(cl_mem),
                            (void *)&this->plan->const_buffer),
  ...
  for (auto & i : inputBuff) {
    OPENCL_V(
        clSetKernelArg(kern, uarg++, sizeof(cl_mem), (void *)&i),
        _T( "clSetKernelArg failed" ));
  }
  //	Output buffer(s)
  ...
  //	           or 2 buffers (CLFFT_COMPLEX_PLANAR)
  for (auto & o : outputBuff) {
    OPENCL_V(
        clSetKernelArg(kern, uarg++, sizeof(cl_mem), (void *)&o),
        _T( "clSetKernelArg failed" ));
  }
  ...
  if (this->plan->hasPreCallback || this->plan->hasPostCallback) {
    if (this->plan->hasPreCallback) {
      OPENCL_V(clSetKernelArg(kern, uarg++, sizeof(cl_mem),
                              (void *)&this->plan->precallUserData),
               _T( "clSetKernelArg failed" ));
    }
    ...
    if (this->plan->hasPostCallback) {
      OPENCL_V(clSetKernelArg(kern, uarg++, sizeof(cl_mem),
                              (void *)&this->plan->postcallUserData),
               _T( "clSetKernelArg failed" ));
    }

    // Pass LDS size arument if set
    if ((this->plan->hasPreCallback &&
         this->plan->preCallback.localMemSize > 0) ||
        (this->plan->hasPostCallback &&
         this->plan->postCallbackParam.localMemSize > 0)) {
      int localmemSize = 0;
      if (this->plan->hasPreCallback &&
          this->plan->preCallback.localMemSize > 0)
      ...
      OPENCL_V(clSetKernelArg(kern, uarg++, localmemSize, nullptr),
               _T( "clSetKernelArg failed" ));
    }
  }

  OPENCL_V(
      clSetKernelArg(kern, uarg++, sizeof(int), (void *)&this->plan->offsetIn),
      _T( "clSetKernelArg failed" ));
  OPENCL_V(
  ...
  // std::cout << "work sizes are " << gWorkSize[0] << ", " << lWorkSize[0] <<
  // std::endl;
  /*
  ...
  cl_int call_status = clEnqueueNDRangeKernel(
      *commQueues, kern, static_cast<cl_uint>(gWorkSize.size()), nullptr,
      &gWorkSize[0], &lWorkSize[0], numWaitEvents, waitEvents, outEvents);
  OPENCL_V(call_status, _T( "clEnqueueNDRangeKernel failed" ));

  if (fftRepo.pStatTimer) {
    fftRepo.pStatTimer->AddSample(plHandle, this->plan, kern,
                                  numQueuesAndEvents, outEvents, gWorkSize,
                                  lWorkSize);
  }
...
clfftStatus FFTAction::writeKernel(const clfftPlanHandle plHandle,
                                   const clfftGenerators gen,
  ...
  if (!kernelFile.get()) {
    std::cerr << "Failed to open kernel file for writing: "
              << kernelPath.c_str() << std::endl;
    return CLFFT_FILE_CREATE_FAILURE;
  }

  std::string kernel;
  OPENCL_V(fftRepo.getProgramCode(gen, data, kernel, device, context),
           _T( "fftRepo.getProgramCode failed." ));
...
clfftStatus FFTAction::compileKernels(const cl_command_queue commQueueFFT,
  ...
  cl_program program;
  if (fftRepo.getclProgram(this->getGenerator(), this->getSignatureData(),
                           program, q_device,
                           fftPlan->context) == CLFFT_INVALID_PROGRAM) {
    FFTBinaryLookup lookup(this->getGenerator(), plHandle, fftPlan->context,
                           q_device);

    lookup.variantRaw(this->getSignatureData(),
                      this->getSignatureData()->datasize);
    ...
    if (lookup.found()) {
      ...
      //{
      #warning "Always writing generated kernel to file"
      OPENCL_V(writeKernel(plHandle, this->getGenerator(),
                           this->getSignatureData(), fftPlan->context,
                           fftPlan->bakeDevice),
               _T( "writeKernel failed." ));
      //}

      std::string programCode;
      OPENCL_V(fftRepo.getProgramCode(this->getGenerator(),
                                      this->getSignatureData(), programCode,
                                      q_device, fftPlan->context),
               _T( "fftRepo.getProgramCode failed." ));

      const char *source = programCode.c_str();
      program = clCreateProgramWithSource(fftPlan->context, 1, &source, nullptr,
                                          &status);
      ...
      status = clBuildProgram(program, 1, &q_device, "-g -cl-opt-disable", NULL,
                              NULL); // good for debugging kernels

      // if you have trouble creating smbols that GDB can pick up to set a
      // breakpoint after kernels are loaded into memory this can be used to
      // stop execution to allow you to set a breakpoint in a kernel after
      // kernel symbols are in memory.
#ifdef DEBUG_BREAK_GDB
      __debugbreak();
#endif
#else

#warning "clBuildPrograms options are -cl-unsafe-math-optimizations"
      status = clBuildProgram(program, 1, &q_device, "-cl-unsafe-math-optimizations", nullptr, nullptr);
#endif
      if (status != CL_SUCCESS) {
        if (status == CL_BUILD_PROGRAM_FAILURE) {
          size_t buildLogSize = 0;
          OPENCL_V(clGetProgramBuildInfo(program, q_device,
    ...
    fftRepo.setclProgram(this->getGenerator(), this->getSignatureData(),
                         program, q_device, fftPlan->context);
    ...
    if (buildFwdKernel) {
      lockRAII *kernelLock;
      if (fftRepo.getclKernel(program, CLFFT_FORWARD, kernel, kernelLock) ==
          CLFFT_INVALID_KERNEL) {
        std::string entryPoint;
        OPENCL_V(fftRepo.getProgramEntryPoint(
                     this->getGenerator(), this->getSignatureData(),
                     CLFFT_FORWARD, entryPoint, q_device, fftPlan->context),
                 _T( "fftRepo.getProgramEntryPoint failed." ));
    ...
    if (buildBwdKernel) {
      lockRAII *kernelLock;
      if (fftRepo.getclKernel(program, CLFFT_BACKWARD, kernel, kernelLock) ==
          CLFFT_INVALID_KERNEL) {
        std::string entryPoint;
        OPENCL_V(fftRepo.getProgramEntryPoint(
                     this->getGenerator(), this->getSignatureData(),
                     CLFFT_BACKWARD, entryPoint, q_device, fftPlan->context),
                 _T( "fftRepo.getProgramEntryPoint failed." ));
