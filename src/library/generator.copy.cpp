...
FFTGeneratedCopyAction::FFTGeneratedCopyAction(clfftPlanHandle plHandle,
                                               FFTPlan *plan,
...
bool FFTGeneratedCopyAction::buildForwardKernel() {
  ...
  bool h2c = (inputLayout == CLFFT_HERMITIAN_PLANAR) ||
             (inputLayout == CLFFT_HERMITIAN_INTERLEAVED);
  bool c2h = (outputLayout == CLFFT_HERMITIAN_PLANAR) ||
             (outputLayout == CLFFT_HERMITIAN_INTERLEAVED);
...
// Copy kernel
template <Precision PR> class CopyKernel {
  size_t N;
  size_t Nt;
...
public:
  ...
  void GenerateKernel(std::string &str) {
    ...
    if (outIlvd) {
      ...
      if (h2c) {
        str += "\t";
        str += "lwbOutRe2 = gbOutRe + oOffset";
        str += outF2;
        str += ";\n\t";
        str += "lwbOutIm2 = gbOutIm + oOffset";
        str += outF2;
        str += ";\n";
      }
    }
...
clfftStatus FFTGeneratedCopyAction::initParams() {
  ...
  if (this->plan->hasPreCallback) {
    ...
    if (this->plan->preCallback.localMemSize >
  ...
  if (this->plan->hasPostCallback) {
    ...
    // Requested local memory size by callback must not exceed the device LDS
    // limits after factoring the LDS size required by main FFT kernel Assumes
    // copy kernel does not have both pre and post callback
    if (this->plan->postCallbackParam.localMemSize >
        this->plan->envelope.limit_LocalMemSize) {
      fprintf(stderr, "Requested local memory size not available\n");
      return CLFFT_INVALID_ARG_VALUE;
    }
...
clfftStatus FFTGeneratedCopyAction::getWorkSizes(std::vector<size_t> &globalWS,
  ...
  c2h = ((this->signature.fft_outputLayout == CLFFT_HERMITIAN_PLANAR) ||
         (this->signature.fft_outputLayout == CLFFT_HERMITIAN_INTERLEAVED));
  ...
  case 5:
    assert(false);
  case 4:
    count *= this->signature.fft_N[2];
  case 3:
    count *= this->signature.fft_N[1];
  case 2: {
    if (general) {
      count *= 64;
    } else {
      count *=
          (DivRoundingUp<size_t>((1 + this->signature.fft_N[0] / 2), 64) * 64);
    }
  } break;
  case 1:
    assert(false);
...
clfftStatus
FFTGeneratedCopyAction::generateKernel(FFTRepo &fftRepo,
                                       const cl_command_queue commQueueFFT) {
  ...
  if (general) {
    OPENCL_V(fftRepo.setProgramEntryPoints(
                 this->getGenerator(), this->getSignatureData(), "copy_general",
                 "copy_general", Device, QueueContext),
             _T( "fftRepo.setProgramEntryPoint() failed!" ));
  } else {
    OPENCL_V(fftRepo.setProgramEntryPoints(this->getGenerator(),
                                           this->getSignatureData(), "copy_c2h",
                                           "copy_h2c", Device, QueueContext),
             _T( "fftRepo.setProgramEntryPoint() failed!" ));
  }
