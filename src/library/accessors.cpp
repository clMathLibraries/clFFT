...
clfftStatus clfftGetPlanBatchSize(const clfftPlanHandle plHandle,
                                  size_t *batchsize) {
...
clfftStatus clfftGetPlanOffsetIn(const clfftPlanHandle plHandle,
                                 size_t *offsetIn) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftGetPlanOffsetOut(const clfftPlanHandle plHandle,
                                  size_t *offsetOut) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftGetPlanContext(const clfftPlanHandle plHandle,
                                cl_context *context) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftGetPlanPrecision(const clfftPlanHandle plHandle,
...
clfftStatus clfftSetPlanPrecision(clfftPlanHandle plHandle,
                                  clfftPrecision precision) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftGetPlanScale(const clfftPlanHandle plHandle,
...
clfftStatus clfftSetPlanDim(clfftPlanHandle plHandle, const clfftDim dim) {
  ...
  //	We resize the vectors in the plan to keep their sizes consistent with
...
clfftStatus clfftGetPlanLength(const clfftPlanHandle plHandle,
...
clfftStatus clfftSetPlanLength(clfftPlanHandle plHandle, const clfftDim dim,
...
clfftStatus clfftGetPlanInStride(const clfftPlanHandle plHandle,
                                 const clfftDim dim, size_t *clStrides) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftGetPlanOutStride(const clfftPlanHandle plHandle,
                                  const clfftDim dim, size_t *clStrides) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftSetPlanDistance(clfftPlanHandle plHandle, size_t iDist,
                                 size_t oDist) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftSetLayout(clfftPlanHandle plHandle, clfftLayout iLayout,
  ...
  case CLFFT_COMPLEX_INTERLEAVED: {
    if ((oLayout == CLFFT_HERMITIAN_INTERLEAVED) ||
        (oLayout == CLFFT_HERMITIAN_PLANAR) || (oLayout == CLFFT_REAL))
      return CLFFT_NOTIMPLEMENTED;
  } break;
...
clfftStatus clfftSetResultLocation(clfftPlanHandle plHandle,
                                   clfftResultLocation placeness) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftGetTmpBufSize(const clfftPlanHandle plHandle,
                               size_t *buffersize) {
  FFTRepo &fftRepo = FFTRepo::getInstance();
  FFTPlan *fftPlan = nullptr;
  lockRAII *planLock = nullptr;
...
clfftStatus clfftLocalMemSize(const clfftPlanHandle plHandle,
                              cl_ulong *local_mem_size) {
  FFTRepo &repo = FFTRepo::getInstance();
  FFTPlan *plan = nullptr;
  lockRAII *lock = nullptr;
...
clfftStatus clfftSetPlanCallback(clfftPlanHandle plHandle, const char *funcName,
  ...
  switch (callbackType) {
  case PRECALLBACK: {
    ARG_CHECK(funcName != nullptr);
    ARG_CHECK(funcString != nullptr);
  ...
  case POSTCALLBACK: {
    ...
    fftPlan->postCallbackParam.localMemSize =
        (localMemSize > 0) ? localMemSize : 0;
