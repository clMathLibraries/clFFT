...
bool clfftGetRequestLibNoMemAlloc() { return request_nomemalloc; }
...
bool FFTBinaryLookup::CacheEntry::exclusive_create() {
  ...
  HANDLE handle = CreateFile(tmp.c_str(), GENERIC_WRITE,
...
#else
  int *fd = new int;
  *fd = open(this->m_filename.c_str(), O_CREAT | O_EXCL,
...
FFTBinaryLookup::FFTBinaryLookup(const clfftGenerators gen,
                                 const clfftPlanHandle plHandle,
                                 cl_context ctxt, cl_device_id device)
    : m_context(ctxt), m_device(device), m_program(nullptr), m_binary(nullptr),
      m_signature(nullptr), m_cache_enabled(cache_enabled) {
...
FFTBinaryLookup::~FFTBinaryLookup() {
  if (this->m_binary != nullptr) {
    delete[] this->m_binary;
    this->m_binary = nullptr;
  }

  if (this->m_signature != nullptr) {
    delete[] this->m_signature;
    this->m_signature = nullptr;
  }
}

FFTBinaryLookup::Variant::Variant()
    : m_kind((VariantKind)0), m_size(0), m_data(nullptr) {}
...
FFTBinaryLookup::Variant &
FFTBinaryLookup::Variant::operator=(const Variant &obj) {
...
enum BinaryRepresentation { LSB, MSB, UNKNOWN };

static enum BinaryRepresentation getStorageMode(char *data) {
  if (data[0] == 'C' && data[1] == 'L' && data[2] == 'B' && data[3] == '\0')
    return LSB;

  if (data[0] == 'B' && data[1] == 'L' && data[2] == 'C' && data[3] == '\0')
    return MSB;
...
void FFTBinaryLookup::finalizeVariant() {
  ...
  whole_variant_size_in_bytes +=
      this->m_variants.size() * sizeof(int); // for the variant kind
  whole_variant_size_in_bytes +=
      this->m_variants.size() * sizeof(size_t); // for the variant size

  // add every variant sizes
  for (auto & v : this->m_variants) {
  ...
  if (whole_variant_size_in_bytes != 0) {
    char md5_sum[33];
    md5sum(this->m_signature, (unsigned long)this->m_header.signature_size,
           md5_sum);
...
bool FFTBinaryLookup::loadBinaryAndSignature(std::ifstream &file) {
  {
    ...
    const std::istream &res =
        file.read((char *)this->m_binary, this->m_header.binary_size);
    if (!res.good())
      return false;
  }

  {
    ...
    const std::istream &res =
        file.read((char *)this->m_signature, this->m_header.signature_size);
...
bool FFTBinaryLookup::found() {
  ...
  // (this->m_cache_entry_name, cache path ??)

  if (tryLoadCacheFile()) {
    cl_int err = buildFromBinary(this->m_binary, this->m_header.binary_size);
...
static cl_int getSingleBinaryFromProgram(cl_program program,
  ...
  err = clGetProgramInfo(program, CL_PROGRAM_BINARIES,
                         1 * sizeof(unsigned char *), binary_address, nullptr);
...
cl_int FFTBinaryLookup::writeCacheFile(std::vector<unsigned char *> &data) {
  ...
  if (created) {
    // if it was created by the current thread, this one will write into cache
    // file
    cache_file.close();
    ...
    const std::string &srcFilename =
        this->m_path + this->m_cache_entry_name + ".cl";
...
cl_int FFTBinaryLookup::populateCache() {
  ...
  this->m_header.whole_file_size = this->m_header.header_size +
                                   this->m_header.binary_size +
                                   this->m_header.signature_size;
...
cl_int FFTBinaryLookup::buildFromSource(const char *source) {
  cl_int err;
  printf("Binary lookup!\n");
  this->m_program = FFTBinaryLookup::buildProgramFromSource(
      source, this->m_context, this->m_device, err);
...
cl_int FFTBinaryLookup::buildFromLoadedBinary(const void *data, size_t len) {
  cl_int err;
  this->m_program = FFTBinaryLookup::buildProgramFromBinary(
      (char *)data, len, this->m_context, this->m_device, err);

  return err;
}

cl_int FFTBinaryLookup::buildFromBinary(const void *data, size_t len) {
...
cl_program FFTBinaryLookup::buildProgramFromSource(const char *source,
                                                   ...
                                                   const char *options) {
  cl_program program =
      clCreateProgramWithSource(context, 1, (const char **)&source, nullptr, &err);

  if (err != CL_SUCCESS)
    return nullptr;

  err = clBuildProgram(program, 1, /* FIXME: 1 device */
                       &device, options, nullptr, nullptr);
...
cl_program
FFTBinaryLookup::buildProgramFromBinary(const char *data, size_t data_size,
                                        cl_context context, cl_device_id device,
                                        cl_int &err, const char *options) {
  cl_program program =
      clCreateProgramWithBinary(context,
                                1,          // num_device
                                &device,    // device_list
                                &data_size, // lengths
                                (const unsigned char **)&data, nullptr, &err);
  if (err != CL_SUCCESS) {
...
cl_program FFTBinaryLookup::getProgram() { return this->m_program; }
...
static void do_mkdir(const std::string &path) {
  ...
  if (st != 0) {
    if (errno != EEXIST) {
      std::string tmp =
          "Cannot not create directory '" + std::string(path) + "': ";
      throw tmp;
    }
  }
}
...
cl_int FFTBinaryLookup::retrieveDeviceAndDriverInfo() {
  ...
  cl_int err = clGetDeviceInfo(this->m_device, CL_DEVICE_VENDOR,
                               sizeof(m_device_vendor), &m_device_vendor, nullptr);
  if (err != CL_SUCCESS) {
    return err;
  }
  ...
  err = clGetDeviceInfo(this->m_device, CL_DRIVER_VERSION,
                        sizeof(m_driver_version), &m_driver_version, nullptr);
  if (err != CL_SUCCESS) {
    return err;
  }
  ...
  try {
    const std::string &root =
        (std::string(cache_path) + m_device_vendor + sep());
    do_mkdir(root.c_str());
