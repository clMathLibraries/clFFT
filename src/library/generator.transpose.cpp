...
void OffsetCalc(std::stringstream &transKernel,
  ...
  clKernWrite(transKernel, 3)
      << "size_t " << offset << " = " << offsetInOut << " ;" << std::endl;
  clKernWrite(transKernel, 3) << "g_index = get_group_id(0);" << std::endl;

  for (size_t i = params.fft_DataDim - 2; i > 0; i--) {
    clKernWrite(transKernel, 3) << offset << " += (g_index/numGroupsY_" << i
                                << ")*" << stride[i + 1] << ";" << std::endl;
    clKernWrite(transKernel, 3)
        << "g_index = g_index % numGroupsY_" << i << ";" << std::endl;
  }
...
void OffsetCalcLeadingDimensionBatched(std::stringstream &transKernel,
  ...
  clKernWrite(transKernel, 3)
      << "size_t " << offset << " = offsetIn;" << std::endl;
  clKernWrite(transKernel, 3) << "g_index = get_group_id(0);" << std::endl;
...
void Swap_OffsetCalc(std::stringstream &transKernel,
  ...
  clKernWrite(transKernel, 3)
      << "size_t " << offset << " = offsetOut;" << std::endl;
...
clfftStatus genTwiddleMath(const FFTKernelGenKeyParams &params,
  ...
  clKernWrite(transKernel, 9)
      << dtComplex
      << " Wm = TW3step( (t_gx_p*32 + lidx) * (t_gy_p*32 + lidy + loop*8) );"
      << std::endl;
  clKernWrite(transKernel, 9)
      << dtComplex
      << " Wt = TW3step( (t_gy_p*32 + lidx) * (t_gx_p*32 + lidy + loop*8) );"
      << std::endl;
...
// wants the twiddle factors generated inside of the transpose
clfftStatus
genTwiddleMathLeadingDimensionBatched(const FFTKernelGenKeyParams &params,
                                      std::stringstream &transKernel,
                                      const std::string &dtComplex, bool fwd) {
  ...
  if (params.fft_N[0] > params.fft_N[1]) {
    clKernWrite(transKernel, 9)
        << dtComplex << " Wm = TW3step( (" << params.fft_N[1]
        << " * square_matrix_index + t_gx_p*32 + lidx) * (t_gy_p*32 + lidy + "
           "loop*8) );"
        << std::endl;
    clKernWrite(transKernel, 9)
        << dtComplex << " Wt = TW3step( (" << params.fft_N[1]
        << " * square_matrix_index + t_gy_p*32 + lidx) * (t_gx_p*32 + lidy + "
           "loop*8) );"
        << std::endl;
  } else {
    clKernWrite(transKernel, 9)
        << dtComplex << " Wm = TW3step( (t_gx_p*32 + lidx) * ("
        << params.fft_N[0]
        << " * square_matrix_index + t_gy_p*32 + lidy + loop*8) );"
        << std::endl;
    clKernWrite(transKernel, 9)
        << dtComplex << " Wt = TW3step( (t_gy_p*32 + lidx) * ("
        << params.fft_N[0]
...
clfftStatus genTransposePrototype(
    const FFTGeneratedTransposeSquareAction::Signature &params,
    const size_t &lwSize, const std::string &dtPlanar,
    const std::string &dtComplex, const std::string &funcName,
    std::stringstream &transKernel, std::string &dtInput,
    std::string &dtOutput) {

  // Declare and define the function
  clKernWrite(transKernel, 0) << "__attribute__(( reqd_work_group_size( "
                              << lwSize << ", 1, 1 ) ))" << std::endl;
  ...
  case CLFFT_COMPLEX_PLANAR:
    ...
    clKernWrite(transKernel, 0)
        << "global " << dtInput << "* restrict inputA_R"
        << ", global " << dtInput << "* restrict inputA_I";
    break;
  ...
  if (params.fft_placeness == CLFFT_OUTOFPLACE)
    ...
    case CLFFT_COMPLEX_INTERLEAVED:
      ...
      dtOutput = dtComplex;
      clKernWrite(transKernel, 0)
          << ", global " << dtOutput << "* restrict outputA";
      break;
    case CLFFT_COMPLEX_PLANAR:
      dtInput = dtPlanar;
      dtOutput = dtPlanar;
      clKernWrite(transKernel, 0)
          << ", global " << dtOutput << "* restrict outputA_R"
          << ", global " << dtOutput << "* restrict outputA_I";
      break;
    case CLFFT_HERMITIAN_INTERLEAVED:
    case CLFFT_HERMITIAN_PLANAR:
      return CLFFT_TRANSPOSED_NOTIMPLEMENTED;
  ...
  if (params.fft_hasPreCallback) {
    ...
    if (params.fft_preCallback.localMemSize > 0) {
      clKernWrite(transKernel, 0)
          << ", __global void* pre_userdata, __local void* localmem";
    } else {
  ...
  if (params.fft_hasPostCallback) {
    ...
    if (params.fft_postCallback.localMemSize > 0) {
      clKernWrite(transKernel, 0)
          << ", __global void* post_userdata, __local void* localmem";
    } else {
...
clfftStatus genTransposePrototypeLeadingDimensionBatched(
    const FFTGeneratedTransposeNonSquareAction::Signature &params,
    const size_t &lwSize, const std::string &dtPlanar,
    const std::string &dtComplex, const std::string &funcName,
    std::stringstream &transKernel, std::string &dtInput,
    std::string &dtOutput) {
...
void permutation_calculation(size_t m, size_t n,
  ...
  */
  ...
  auto *table = new size_t[mn_minus_one + 1](); // init to zeros
  table[0] = 1;
...
// transpose kernels to perform nonsqaure transpose this function assumes a 1:2
// ratio
clfftStatus
genSwapKernel(const FFTGeneratedTransposeNonSquareAction::Signature &params,
  ...
  // clKernWrite( transKernel, 0 ) << "#pragma OPENCL EXTENSION cl_amd_printf :
  // enable\n" << std::endl;
  ...
  case CLFFT_DOUBLE_FAST:
    ...
    clKernWrite(transKernel, 3)
        << "#pragma OPENCL EXTENSION cl_khr_fp64 : enable" << std::endl;
    clKernWrite(transKernel, 0) << "#else" << std::endl;
    clKernWrite(transKernel, 3)
        << "#pragma OPENCL EXTENSION cl_amd_fp64 : enable" << std::endl;
  ...
  if (params.fft_hasPostCallback) {
    ...
    // limits after factoring the LDS size required by swap kernel
    if (params.fft_postCallback.localMemSize > 0) {
      bool validLDSSize = false;

      validLDSSize =
          ((2 * input_elm_size_in_bytes * (num_elements_loaded * 2)) +
    ...
    clKernWrite(transKernel, 0)
        << params.fft_postCallback.funcstring << std::endl;
  ...
  if (params.fft_hasPreCallback) {
    ...
    // Insert callback function code at the beginning
    clKernWrite(transKernel, 0)
        << params.fft_preCallback.funcstring << std::endl;
    clKernWrite(transKernel, 0) << std::endl;
  }
  ...
  /*Generating the  swapping logic*/
  {
    ...
    clKernWrite(transKernel, 0) << "{  " << last_datablk_idx << ",  "
    ...
    clKernWrite(transKernel, 0) << "__constant size_t cycle_stat["
                                << cycle_map[0] << "][2] = {" << std::endl;
    for (size_t i = 0; i < num_cycles_minus_1; i++) {
      clKernWrite(transKernel, 0) << "{  " << cycle_stat[i * 2] << ",  "
    ...
    case CLFFT_COMPLEX_INTERLEAVED:
      clKernWrite(transKernel, 0)
          << "void swap(global " << dtComplex << "* inputA, " << tmpBuffType
          << " " << dtComplex << "* Ls, " << tmpBuffType << " " << dtComplex
          << " * Ld, size_t is, size_t id, size_t pos, size_t end_indx, size_t "
             "work_id";
      break;
    case CLFFT_COMPLEX_PLANAR:
      clKernWrite(transKernel, 0)
          << "void swap(global " << dtPlanar << "* inputA_R, global "
          << dtPlanar << "* inputA_I, " << tmpBuffType << " " << dtComplex
          << "* Ls, " << tmpBuffType << " " << dtComplex
          << "* Ld, size_t is, size_t id, size_t pos, size_t end_indx, size_t "
             "work_id";
      break;
    case CLFFT_HERMITIAN_INTERLEAVED:
    ...
    case CLFFT_REAL:
      clKernWrite(transKernel, 0)
          << "void swap(global " << dtPlanar << "* inputA, " << tmpBuffType
    ...
    if (params.fft_hasPostCallback) {
      clKernWrite(transKernel, 0)
          << ", size_t iOffset, __global void* post_userdata";
      if (params.fft_postCallback.localMemSize > 0) {
    ...
    if (params.fft_hasPreCallback) {
      clKernWrite(transKernel, 0)
          << ", size_t iOffset, __global void* pre_userdata";
      if (params.fft_preCallback.localMemSize > 0) {
        clKernWrite(transKernel, 0) << ", __local void* localmem";
      }
    }
    ...
    clKernWrite(transKernel, 3)
        << "for (size_t j = get_local_id(0); j < end_indx; j += "
    ...
    case CLFFT_COMPLEX_INTERLEAVED:
      ...
      if (params.fft_hasPreCallback) {
        ...
        clKernWrite(transKernel, 9)
            << "Ls[j] = " << params.fft_preCallback.funcname
            << "(inputA, ( is *" << smaller_dim << " + " << num_elements_loaded
            << " * work_id + j + iOffset), pre_userdata";
        // clKernWrite(transKernel, 9) << "Ls[j] = " <<
        // params.fft_preCallback.funcname << "(inputA + iOffset, ( is *" <<
        // smaller_dim << " + " << num_elements_loaded << " * work_id + j),
        // pre_userdata";
        if (params.fft_preCallback.localMemSize > 0) {
        ...
        clKernWrite(transKernel, 9)
            << "Ld[j] = " << params.fft_preCallback.funcname
            << "(inputA, ( id *" << smaller_dim << " + " << num_elements_loaded
            << " * work_id + j + iOffset), pre_userdata";
        // clKernWrite(transKernel, 9) << "Ld[j] = " <<
        // params.fft_preCallback.funcname << "(inputA + iOffset, ( id *" <<
        // smaller_dim << " + " << num_elements_loaded << " * work_id + j),
        // pre_userdata";
        if (params.fft_preCallback.localMemSize > 0) {
          clKernWrite(transKernel, 0) << ", localmem";
        ...
        clKernWrite(transKernel, 6) << "if (pos == 0){" << std::endl;
        clKernWrite(transKernel, 9)
            << "Ls[j] = inputA[is *" << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j];" << std::endl;
        clKernWrite(transKernel, 9)
            << "Ld[j] = inputA[id *" << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j];" << std::endl;
        clKernWrite(transKernel, 6) << "}" << std::endl;
      ...
      if (params.fft_hasPostCallback) {
        clKernWrite(transKernel, 6)
            << params.fft_postCallback.funcname << "(inputA, (iOffset + id*"
            << smaller_dim << " + " << num_elements_loaded
            << " * work_id + j), post_userdata, Ls[j]";
        if (params.fft_postCallback.localMemSize > 0) {
          clKernWrite(transKernel, 0) << ", localmem";
        }
        clKernWrite(transKernel, 0) << ");" << std::endl;
        ...
        clKernWrite(transKernel, 6)
            << "inputA[id*" << smaller_dim << " + " << num_elements_loaded
            << " * work_id + j + iOffset] = Ls[j];" << std::endl;
      } else {
        clKernWrite(transKernel, 6)
            << "inputA[id*" << smaller_dim << " + " << num_elements_loaded
    ...
    case CLFFT_COMPLEX_PLANAR:
      if (params.fft_hasPreCallback) {
        ...
        clKernWrite(transKernel, 9)
            << "Ls[j] = " << params.fft_preCallback.funcname
            << "(inputA_R, inputA_I, (is * " << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j + iOffset), pre_userdata";
        if (params.fft_preCallback.localMemSize > 0) {
          clKernWrite(transKernel, 0) << ", localmem";
        }
        clKernWrite(transKernel, 0) << ");" << std::endl;
        ...
        clKernWrite(transKernel, 9)
            << "Ld[j] = " << params.fft_preCallback.funcname
            << "(inputA_R, inputA_I, (id * " << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j + iOffset), pre_userdata";
        if (params.fft_preCallback.localMemSize > 0) {
          clKernWrite(transKernel, 0) << ", localmem";
        }
        ...
        clKernWrite(transKernel, 6) << "if (pos == 0){" << std::endl;
        clKernWrite(transKernel, 9)
            << "Ls[j].x = inputA_R[is*" << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j];" << std::endl;
        clKernWrite(transKernel, 9)
            << "Ls[j].y = inputA_I[is*" << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j];" << std::endl;
        clKernWrite(transKernel, 9)
            << "Ld[j].x = inputA_R[id*" << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j];" << std::endl;
        clKernWrite(transKernel, 9)
            << "Ld[j].y = inputA_I[id*" << smaller_dim << " + "
            << num_elements_loaded << " * work_id + j];" << std::endl;
        clKernWrite(transKernel, 6) << "}" << std::endl;
      ...
      if (params.fft_hasPostCallback) {
        clKernWrite(transKernel, 6)
            ...
            << "(inputA_R, inputA_I, (iOffset + id*" << smaller_dim << " + "
            << num_elements_loaded
            << " * work_id + j), post_userdata, Ls[j].x, Ls[j].y";
        if (params.fft_postCallback.localMemSize > 0) {
          clKernWrite(transKernel, 0) << ", localmem";
        }
        clKernWrite(transKernel, 0) << ");" << std::endl;
        ...
        clKernWrite(transKernel, 6)
            << "inputA_R[id*" << smaller_dim << " + " << num_elements_loaded
            << " * work_id + j] = Ls[j].x;" << std::endl;
        clKernWrite(transKernel, 6)
            << "inputA_I[id*" << smaller_dim << " + " << num_elements_loaded
            << " * work_id + j] = Ls[j].y;" << std::endl;
      }
      break;
    default:
      return CLFFT_TRANSPOSED_NOTIMPLEMENTED;
    ...
    clKernWrite(transKernel, 3)
        << "size_t g_index = get_group_id(0);" << std::endl;
    ...
    for (size_t i = 2; i < params.fft_DataDim - 1; i++) {
      clKernWrite(transKernel, 3)
          << "const size_t numGroupsY_" << i << " = numGroupsY_" << i - 1
    ...
    case CLFFT_REAL:
      ...
      clKernWrite(transKernel, 3)
          << "__local " << dtInput << " tmp_tot_mem["
          << (num_elements_loaded * 2) << "];" << std::endl;
      clKernWrite(transKernel, 3) << tmpBuffType << " " << dtInput
                                  << " *te = tmp_tot_mem;" << std::endl;

      clKernWrite(transKernel, 3)
          << tmpBuffType << " " << dtInput << " *to = (tmp_tot_mem + "
          << num_elements_loaded << ");" << std::endl;

      // Do not advance offset when postcallback is set as the starting address
      // of global buffer is needed
    ...
    case CLFFT_COMPLEX_PLANAR:
      ...
      clKernWrite(transKernel, 3)
          << "__local " << dtComplex << " tmp_tot_mem["
          << (num_elements_loaded * 2) << "];" << std::endl;
      clKernWrite(transKernel, 3) << tmpBuffType << " " << dtComplex
                                  << " *te = tmp_tot_mem;" << std::endl;

      clKernWrite(transKernel, 3)
          << tmpBuffType << " " << dtComplex << " *to = (tmp_tot_mem + "
          << num_elements_loaded << ");" << std::endl;
    ...
    case CLFFT_COMPLEX_PLANAR:
      ...
      break;
    case CLFFT_REAL:
      clKernWrite(transKernel, 3) << tmpBuffType << " " << dtPlanar
                                  << " *tmp_swap_ptr[2];" << std::endl;
    }
    ...
    clKernWrite(transKernel, 3) << "size_t start = cycle_stat[g_index / "
                                << num_grps_pro_row << "][0];" << std::endl;
    clKernWrite(transKernel, 3) << "size_t end = cycle_stat[g_index / "
                                << num_grps_pro_row << "][1];" << std::endl;

    clKernWrite(transKernel, 3)
        << "size_t end_indx = " << num_elements_loaded << ";" << std::endl;
    clKernWrite(transKernel, 3) << "size_t work_id = g_index % "
                                << num_grps_pro_row << ";" << std::endl;

    clKernWrite(transKernel, 3)
        << "if( work_id == " << (num_grps_pro_row - 1) << " ){" << std::endl;
    clKernWrite(transKernel, 6)
        << "end_indx = "
    ...
    case CLFFT_REAL:
      clKernWrite(transKernel, 6)
          << "swap(inputA, tmp_swap_ptr[swap_inx], tmp_swap_ptr[1 - swap_inx], "
             "swap_table[loop][0], swap_table[loop][1], swap_table[loop][2], "
             "end_indx, work_id";
      break;
    case CLFFT_COMPLEX_PLANAR:
      clKernWrite(transKernel, 6)
          << "swap(inputA_R, inputA_I, tmp_swap_ptr[swap_inx], tmp_swap_ptr[1 "
             "- swap_inx], swap_table[loop][0], swap_table[loop][1], "
             "swap_table[loop][2], end_indx, work_id";
      break;
    }
    if (params.fft_hasPostCallback) {
...
clfftStatus genSwapKernelGeneral(
    const FFTGeneratedTransposeNonSquareAction::Signature &params,
    std::string &strKernel, std::string &KernelFuncName, const size_t &lwSize,
...
*/
  ...
  while (LDS_per_WG >
         1024) // avoiding using too much lds memory. the biggest LDS memory we
               // will allocate would be 1024*sizeof(float2/double2)*2
  {
    if (LDS_per_WG % 2 == 0) {
      LDS_per_WG /= 2;
      continue;
  ...
  */
  ...
  clKernWrite(transKernel, 0)
      << "__constant size_t swap_table[" << permutationTable.size() + 2
      << "][1] = {" << std::endl;
  clKernWrite(transKernel, 0) << "{0}," << std::endl;
  clKernWrite(transKernel, 0) << "{" << smaller_dim * dim_ratio - 1 << "},"
                              << std::endl; // add the first and last row to the
  ...
  if (twiddleSwapKernel) {
    std::string str;
    StockhamGenerator::TwiddleTableLarge twLarge(smaller_dim * smaller_dim *
                                                 dim_ratio);
    if ((params.fft_precision == CLFFT_SINGLE) ||
        (params.fft_precision == CLFFT_SINGLE_FAST))
  ...
  std::string smaller_dim_str = static_cast<const std::ostringstream &>(
                                    std::ostringstream() << smaller_dim)
                                    .str();
  std::string dim_ratio_str =
      static_cast<const std::ostringstream &>(std::ostringstream() << dim_ratio)
          .str();
  if (params.fft_N[0] > params.fft_N[1])
  ...
  for (size_t bothDir = 0; bothDir < 2; bothDir++) {
    ...
    clKernWrite(transKernel, 3)
        << "//each wg handles 1/" << WG_per_line << " row of " << LDS_per_WG
        << " in memory" << std::endl;
    clKernWrite(transKernel, 3)
    ...
    clKernWrite(transKernel, 3)
        << "size_t group_id = get_group_id(0);" << std::endl;
    ...
    case CLFFT_COMPLEX_INTERLEAVED:
      clKernWrite(transKernel, 3)
          << "inputA += batch_offset*" << smaller_dim * bigger_dim << ";"
          << std::endl;
      break;
    case CLFFT_HERMITIAN_INTERLEAVED:
    case CLFFT_HERMITIAN_PLANAR:
    ...
    case CLFFT_COMPLEX_PLANAR: {
      clKernWrite(transKernel, 3)
          << "inputA_R += batch_offset*" << smaller_dim * bigger_dim << ";"
          << std::endl;
      clKernWrite(transKernel, 3)
          << "inputA_I += batch_offset*" << smaller_dim * bigger_dim << ";"
          << std::endl;
      break;
    }
    default:
    ...
    clKernWrite(transKernel, 3)
        << "group_id -= batch_offset*"
        << (permutationTable.size() + 2) * WG_per_line << ";" << std::endl;

    clKernWrite(transKernel, 3) << std::endl;
    if (WG_per_line == 1)
      clKernWrite(transKernel, 3)
          << "size_t prev = swap_table[group_id][0];" << std::endl;
    else
      clKernWrite(transKernel, 3) << "size_t prev = swap_table[group_id/"
    ...
    case CLFFT_COMPLEX_INTERLEAVED: {
      clKernWrite(transKernel, 3)
          << "__local " << dtInput << " prevValue[" << LDS_per_WG << "];"
          << std::endl; // lds within each wg should be able to store a row
                        // block (smaller_dim) of element
      clKernWrite(transKernel, 3) << "__local " << dtInput << " nextValue["
                                  << LDS_per_WG << "];" << std::endl;
      break;
    }
    case CLFFT_COMPLEX_PLANAR: {
      clKernWrite(transKernel, 3)
          << "__local " << dtComplex << " prevValue[" << LDS_per_WG << "];"
          << std::endl; // lds within each wg should be able to store a row
                        // block (smaller_dim) of element
      clKernWrite(transKernel, 3) << "__local " << dtComplex << " nextValue["
                                  << LDS_per_WG << "];" << std::endl;
      break;
    }
    case CLFFT_HERMITIAN_INTERLEAVED:
        ...
        params.fft_N[1]) // decides whether we have a tall or wide rectangle
    {
      ...
      if (WG_per_line == 1) // might look like: size_t group_offset = prev*729;
        clKernWrite(transKernel, 3) << "size_t group_offset = (prev*"
                                    << smaller_dim << ");" << std::endl;
      else // if smaller_dim is 2187 > 1024 this should look like size_t
           // group_offset = prev*2187 + (group_id % 3)*729;
        clKernWrite(transKernel, 3)
            << "size_t group_offset = (prev*" << smaller_dim
            << ") + (group_id % " << WG_per_line << ")*" << LDS_per_WG << ";"
            << std::endl;
    }
    ...
    if (twiddleSwapKernelIn) {
      ...
      clKernWrite(transKernel, 6)
          << dtComplex << " twiddle_factor;" << std::endl;
    }
    switch (params.fft_inputLayout) {
    case CLFFT_REAL:
    case CLFFT_COMPLEX_INTERLEAVED: {
        ...
        if (i + 256 < LDS_per_WG) {
          if (params.fft_hasPreCallback) {
            clKernWrite(transKernel, 3)
                ...
                << "(inputA-batch_offset*" << smaller_dim * bigger_dim
                << ", batch_offset*" << smaller_dim * bigger_dim
                << "+group_offset+idx+" << i << ", pre_userdata);" << std::endl;
          } else {
            if (twiddleSwapKernelIn) {
              if (params.fft_N[0] > params.fft_N[1]) // decides whether we have
                                                     // a tall or wide rectangle
              {
                // input is wide; output is tall; read input index realted
                clKernWrite(transKernel, 6)
                    << "p = (group_offset+idx+" << i << ")/" << bigger_dim
                    << ";" << std::endl;
                clKernWrite(transKernel, 6)
                    << "q = (group_offset+idx+" << i << ")%" << bigger_dim
                    << ";" << std::endl;
              } else {
                // input is tall; output is wide; read input index realted
                clKernWrite(transKernel, 6)
                    << "p = (group_offset+idx+" << i << ")/" << smaller_dim
                    << ";" << std::endl;
                clKernWrite(transKernel, 6)
                    << "q = (group_offset+idx+" << i << ")%" << smaller_dim
                    << ";" << std::endl;
              }
              clKernWrite(transKernel, 6)
                  << "twiddle_factor = TW3step(p*q);" << std::endl;
              if (fwd) {
                // forward
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i << "].x = inputA[group_offset+idx+"
                    << i << "].x * twiddle_factor.x - inputA[group_offset+idx+"
                    << i << "].y * twiddle_factor.y;" << std::endl;
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i << "].y = inputA[group_offset+idx+"
                    << i << "].x * twiddle_factor.y + inputA[group_offset+idx+"
                ...
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i << "].y = inputA[group_offset+idx+"
                    << i << "].y * twiddle_factor.x - inputA[group_offset+idx+"
                    << i << "].x * twiddle_factor.y;" << std::endl;
              }
            } else
              clKernWrite(transKernel, 3)
                  << "prevValue[idx+" << i << "] = inputA[group_offset+idx+"
                  << i << "];" << std::endl;
          }
        } else {
          // need to handle boundary
          clKernWrite(transKernel, 3)
              << "if(idx+" << i << "<" << LDS_per_WG << "){" << std::endl;
          if (params.fft_hasPreCallback) {
            clKernWrite(transKernel, 3)
                << "prevValue[idx+" << i
                << "] = " << params.fft_preCallback.funcname
          ...
          if (params.fft_hasPreCallback) {
            ...
            if (twiddleSwapKernelIn) {
              ...
              if (fwd) {
                ...
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i
                    << "].x = inputA_R[group_offset+idx+" << i
                    << "] * twiddle_factor.x - inputA_I[group_offset+idx+" << i
                    << "] * twiddle_factor.y;" << std::endl;
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i
                    << "].y = inputA_R[group_offset+idx+" << i
                    << "] * twiddle_factor.y + inputA_I[group_offset+idx+" << i
                    << "] * twiddle_factor.x;" << std::endl;
              } else {
                // backward
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i
                ...
                clKernWrite(transKernel, 3)
                    << "prevValue[idx+" << i
                    << "].y = inputA_I[group_offset+idx+" << i
                    << "] * twiddle_factor.x - inputA_R[group_offset+idx+" << i
                    << "] * twiddle_factor.y;" << std::endl;
              }
            } else {
              clKernWrite(transKernel, 3)
                  << "prevValue[idx+" << i << "].x = inputA_R[group_offset+idx+"
                  << i << "];" << std::endl;
              clKernWrite(transKernel, 3)
                  << "prevValue[idx+" << i << "].y = inputA_I[group_offset+idx+"
                  << i << "];" << std::endl;
            }
          }
        } else {
    ...
    {
      clKernWrite(transKernel, 6)
          << "next = (prev*" << smaller_dim << ")%"
          << smaller_dim * dim_ratio - 1 << ";" << std::endl;
      // takes care the last row
      clKernWrite(transKernel, 6)
          << "if (prev == " << smaller_dim * dim_ratio - 1 << ")" << std::endl;
      clKernWrite(transKernel, 9)
          << "next = " << smaller_dim * dim_ratio - 1 << ";" << std::endl;
      if (WG_per_line == 1) {
        clKernWrite(transKernel, 6)
      ...
      clKernWrite(transKernel, 6)
          << "next = (prev*" << dim_ratio << ")%" << smaller_dim * dim_ratio - 1
          << ";" << std::endl;
      // takes care the last row
      clKernWrite(transKernel, 6)
          << "if (prev == " << smaller_dim * dim_ratio - 1 << ")" << std::endl;
      ...
      if (WG_per_line == 1) // might look like: size_t group_offset = prev*729;
        clKernWrite(transKernel, 6)
            << "group_offset = (next*" << smaller_dim << ");" << std::endl;
      else // if smaller_dim is 2187 > 1024 this should look like size_t
           // group_offset = next*2187 + (group_id % 3)*729;
        clKernWrite(transKernel, 6)
            << "group_offset = (next*" << smaller_dim << ") + (group_id % "
            << WG_per_line << ")*" << LDS_per_WG << ";" << std::endl;
    }
          ...
          if (params.fft_hasPreCallback) {
            ...
            if (twiddleSwapKernelIn) {
              ...
              if (fwd) {
                // forward
                clKernWrite(transKernel, 3)
                    << "nextValue[idx+" << i << "].x = inputA[group_offset+idx+"
                    << i << "].x * twiddle_factor.x - inputA[group_offset+idx+"
                    << i << "].y * twiddle_factor.y;" << std::endl;
                clKernWrite(transKernel, 3)
                    << "nextValue[idx+" << i << "].y = inputA[group_offset+idx+"
                    << i << "].x * twiddle_factor.y + inputA[group_offset+idx+"
                    << i << "].y * twiddle_factor.x;" << std::endl;
              } else {
                // backward
              ...
              clKernWrite(transKernel, 6)
                  << "nextValue[idx+" << i << "] = inputA[group_offset+idx+"
                  << i << "];" << std::endl;
          }
        else {
          // need to handle boundary
        ...
        if (i + 256 < LDS_per_WG) {
          if (params.fft_hasPreCallback) {
            ...
            if (twiddleSwapKernelIn) {
              ...
              clKernWrite(transKernel, 6)
                  << "nextValue[idx+" << i << "].x = inputA_R[group_offset+idx+"
                  << i << "];" << std::endl;
              clKernWrite(transKernel, 6)
                  << "nextValue[idx+" << i << "].y = inputA_I[group_offset+idx+"
                  << i << "];" << std::endl;
            }
          }
        } else {
        ...
        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
          if (i + 256 < LDS_per_WG) {
            ...
            if (fwd) {
              // forward
              clKernWrite(transKernel, 6)
                  << "inputA[group_offset+idx+" << i << "].x = prevValue[idx+"
                  << i << "].x * twiddle_factor.x - prevValue[idx+" << i
                  << "].y * twiddle_factor.y;" << std::endl;
              clKernWrite(transKernel, 6)
                  << "inputA[group_offset+idx+" << i << "].y = prevValue[idx+"
                  << i << "].x * twiddle_factor.y + prevValue[idx+" << i
              ...
              clKernWrite(transKernel, 6)
                  << "inputA[group_offset+idx+" << i << "].y = prevValue[idx+"
                  << i << "].y * twiddle_factor.x - prevValue[idx+" << i
                  << "].x * twiddle_factor.y;" << std::endl;
            }
            // clKernWrite(transKernel, 6) << "inputA[group_offset+idx+" << i <<
            ...
            // clKernWrite(transKernel, 9) << "inputA[group_offset+idx+" << i <<
            // "] = prevValue[idx+" << i << "];" << std::endl;
            clKernWrite(transKernel, 6) << "}" << std::endl;
          }
        }
      } else if (!twiddleSwapKernelOut) // could be twiddleSwapKernelIn
      {
        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
          ...
          if (params.fft_hasPostCallback) {
            ...
            if (i + 256 < LDS_per_WG)
              clKernWrite(transKernel, 6)
                  << "inputA[group_offset+idx+" << i << "] = prevValue[idx+"
                  << i << "];" << std::endl;
            else {
              // need to handle boundary
              clKernWrite(transKernel, 6)
    ...
    case CLFFT_COMPLEX_PLANAR: {
        ...
        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
          if (i + 256 < LDS_per_WG) {
            ...
            if (fwd) {
              // forward
              clKernWrite(transKernel, 6)
                  << "inputA_R[group_offset+idx+" << i << "] = prevValue[idx+"
                  << i << "].x * twiddle_factor.x - prevValue[idx+" << i
                  << "].y * twiddle_factor.y;" << std::endl;
              clKernWrite(transKernel, 6)
                  << "inputA_I[group_offset+idx+" << i << "] = prevValue[idx+"
                  << i << "].x * twiddle_factor.y + prevValue[idx+" << i
                  << "].y * twiddle_factor.x;" << std::endl;
            } else {
              // backward
      ...
      } else if (!twiddleSwapKernelOut) // could be twiddleSwapKernelIn
      {
        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
          ...
          if (params.fft_hasPostCallback) {
            if (i + 256 < LDS_per_WG) {
              // clKernWrite(transKernel, 6) << "inputA_R[group_offset+idx+" <<
              // i << "] = prevValue[idx+" << i << "].x;" << std::endl;
              // clKernWrite(transKernel, 6) << "inputA_I[group_offset+idx+" <<
              // i << "] = prevValue[idx+" << i << "].y;" << std::endl;
              clKernWrite(transKernel, 6)
                  << params.fft_postCallback.funcname
                  << "(inputA_R - batch_offset*" << smaller_dim * bigger_dim
                  << ", inputA_I - batch_offset*" << smaller_dim * bigger_dim
    ...
    case CLFFT_COMPLEX_PLANAR: {
      for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
        if (i + 256 < LDS_per_WG)
          clKernWrite(transKernel, 6)
              << "prevValue[idx+" << i << "] = nextValue[idx+" << i << "];"
              << std::endl;
        else {
          // need to handle boundary
          clKernWrite(transKernel, 6)
    ...
    if (WG_per_line == 1)
      clKernWrite(transKernel, 3) << "}while(next!=swap_table[group_id][0]);"
                                  << std::endl; // end of do-while
    else
      clKernWrite(transKernel, 3)
          << "}while(next!=swap_table[group_id/" << WG_per_line << "][0]);"
          << std::endl;                              // end of do-while
...
[M0
 ...
 M2]
*/
clfftStatus genTransposeKernelBatched(
    const FFTGeneratedTransposeSquareAction::Signature &params,
    std::string &strKernel, const size_t &lwSize, const size_t reShapeFactor) {
  strKernel.reserve(4096);
  std::stringstream transKernel(std::stringstream::out);
  ...
  bool twiddleTransposeKernel =
      ...
      (params.transposeMiniBatchSize ==
       1); // when transposeMiniBatchSize == 1 it is guaranteed to be a sqaure
           // matrix transpose
  //	If twiddle computation has been requested, generate the lookup function

  if (twiddleTransposeKernel) {
    std::string str;
    StockhamGenerator::TwiddleTableLarge twLarge(params.fft_N[0] *
                                                 params.fft_N[1]);
    if ((params.fft_precision == CLFFT_SINGLE) ||
        (params.fft_precision == CLFFT_SINGLE_FAST))
      twLarge.GenerateTwiddleTable<StockhamGenerator::P_SINGLE>(str);
  ...
  for (size_t bothDir = 0; bothDir < 2; bothDir++) {
    ...
    clKernWrite(transKernel, 3)
        << "const size_t numGroupsY_1 = " << wgPerBatch << ";" << std::endl;
    ...
    switch (params.fft_inputLayout) {
    case CLFFT_COMPLEX_INTERLEAVED:
      // Do not advance offset when precallback is set as the starting address
      // of global buffer is needed
      if (!params.fft_hasPreCallback) {
        clKernWrite(transKernel, 3)
            << "inputA += iOffset;"
    ...
    if (params.fft_placeness == CLFFT_OUTOFPLACE) {
      ...
      case CLFFT_COMPLEX_INTERLEAVED:
        if (params.fft_hasPreCallback)
          clKernWrite(transKernel, 3)
              << "global " << dtInput << " *outputA = inputA + iOffset;"
              << std::endl;
        else
          clKernWrite(transKernel, 3)
              << "global " << dtInput << " *outputA = inputA;" << std::endl;
        break;
      case CLFFT_COMPLEX_PLANAR:
        if (params.fft_hasPreCallback) {
          clKernWrite(transKernel, 3)
              << "global " << dtInput << " *outputA_R = inputA_R + iOffset;"
              << std::endl;
          clKernWrite(transKernel, 3)
              << "global " << dtInput << " *outputA_I = inputA_I + iOffset;"
              << std::endl;
        } else {
          clKernWrite(transKernel, 3)
              << "global " << dtInput << " *outputA_R = inputA_R;" << std::endl;
          clKernWrite(transKernel, 3)
              << "global " << dtInput << " *outputA_I = inputA_I;" << std::endl;
        }
        break;
      case CLFFT_HERMITIAN_INTERLEAVED:
      case CLFFT_HERMITIAN_PLANAR:
    ...
    clKernWrite(transKernel, 3)
        << "if (row == (float)(size_t)row) row -= 1; " << std::endl;
    clKernWrite(transKernel, 3)
        << "const size_t t_gy = (size_t)row;" << std::endl;
    ...
    clKernWrite(transKernel, 3)
        << "const long t_gy_p = t_gx_p - t_gy;" << std::endl;

    clKernWrite(transKernel, 3) << "" << std::endl;

    clKernWrite(transKernel, 3)
        << "const size_t d_lidx = get_local_id(0) % 16;" << std::endl;
    clKernWrite(transKernel, 3)
        << "const size_t d_lidy = get_local_id(0) / 16;" << std::endl;
    ...
    clKernWrite(transKernel, 3) << "const size_t idx = lidx + t_gx_p*"
                                << 16 * reShapeFactor << ";" << std::endl;
    clKernWrite(transKernel, 3) << "const size_t idy = lidy + t_gy_p*"
                                << 16 * reShapeFactor << ";" << std::endl;

    clKernWrite(transKernel, 3) << "" << std::endl;

    clKernWrite(transKernel, 3)
        << "const size_t starting_index_yx = t_gy_p*" << 16 * reShapeFactor
        << " + t_gx_p*" << 16 * reShapeFactor * params.fft_N[0] << ";"
        << std::endl;

    clKernWrite(transKernel, 3) << "" << std::endl;

    clKernWrite(transKernel, 3)
        << "__local " << dtComplex << " xy_s["
        << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
    clKernWrite(transKernel, 3)
        << "__local " << dtComplex << " yx_s["
        << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
    ...
    // into the corresponding shared memories. Afterwards I use non-contiguous
    // access from local memory and write contiguously back into the arrays

    if (mult_of_16) {
      clKernWrite(transKernel, 3) << "size_t index;" << std::endl;
      clKernWrite(transKernel, 3)
          << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
          << "; ++loop){" << std::endl;
      clKernWrite(transKernel, 6) << "index = lidy*" << 16 * reShapeFactor
      ...
      case CLFFT_COMPLEX_INTERLEAVED: {
        if (params.fft_hasPreCallback) {
          if (params.fft_preCallback.localMemSize > 0) {
            clKernWrite(transKernel, 6)
                << "tmpm = " << params.fft_preCallback.funcname
                << "(inputA, iOffset + (idy + loop * " << 16 / reShapeFactor
                << ")*" << params.fft_N[0] << " + idx, pre_userdata, localmem);"
                << std::endl;
            clKernWrite(transKernel, 6)
                << "tmpt = " << params.fft_preCallback.funcname
                << "(inputA, iOffset + (lidy + loop * " << 16 / reShapeFactor
                << ")*" << params.fft_N[0]
                << " + lidx + starting_index_yx, pre_userdata, localmem);"
                << std::endl;
          } else {
            clKernWrite(transKernel, 6)
                << "tmpm = " << params.fft_preCallback.funcname
                << "(inputA, iOffset + (idy + loop * " << 16 / reShapeFactor
                << ")*" << params.fft_N[0] << " + idx, pre_userdata);"
                << std::endl;
            clKernWrite(transKernel, 6)
                << "tmpt = " << params.fft_preCallback.funcname
                << "(inputA, iOffset + (lidy + loop * " << 16 / reShapeFactor
                << ")*" << params.fft_N[0]
                << " + lidx + starting_index_yx, pre_userdata);" << std::endl;
          }
        } else {
          clKernWrite(transKernel, 6)
              << "tmpm = inputA[(idy + loop *" << 16 / reShapeFactor << ")*"
              << params.fft_N[0] << " + idx];" << std::endl;
          clKernWrite(transKernel, 6)
              << "tmpt = inputA[(lidy + loop *" << 16 / reShapeFactor << ")*"
      ...
      case CLFFT_COMPLEX_PLANAR:
        ...
        if (params.fft_hasPreCallback) {
          if (params.fft_preCallback.localMemSize > 0) {
            clKernWrite(transKernel, 6)
                << "tmpm = " << params.fft_preCallback.funcname
                << "(inputA_R, inputA_I, iOffset + (idy + loop *"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + idx, pre_userdata, localmem);" << std::endl;
            clKernWrite(transKernel, 6)
                << "tmpt = " << params.fft_preCallback.funcname
                << "(inputA_R, inputA_I, iOffset + (lidy + loop *"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + lidx + starting_index_yx, pre_userdata, localmem);"
                << std::endl;
          } else {
          ...
          clKernWrite(transKernel, 6)
              << "tmpm.x = inputA_R[(idy + loop *" << 16 / reShapeFactor << ")*"
              << params.fft_N[0] << " + idx];" << std::endl;
          clKernWrite(transKernel, 6)
              << "tmpm.y = inputA_I[(idy + loop *" << 16 / reShapeFactor << ")*"
              << params.fft_N[0] << " + idx];" << std::endl;

          clKernWrite(transKernel, 6)
              << "tmpt.x = inputA_R[(lidy + loop *" << 16 / reShapeFactor
              << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
              << std::endl;
          clKernWrite(transKernel, 6)
              << "tmpt.y = inputA_I[(lidy + loop *" << 16 / reShapeFactor
              << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
              << std::endl;
        }
        break;
      ...
      clKernWrite(transKernel, 3)
          ...
          << "; ++loop){" << std::endl;
      clKernWrite(transKernel, 6)
          << "index = lidx*" << 16 * reShapeFactor << " + lidy + "
          << 16 / reShapeFactor << "*loop;" << std::endl;

      // Handle planar and interleaved right here
      switch (params.fft_outputLayout) {
      case CLFFT_COMPLEX_INTERLEAVED:
        if (params.fft_hasPostCallback) {
          if (params.transposeMiniBatchSize <
              2) // which means the matrix was not broken down into sub square
                 // matrics
            clKernWrite(transKernel, 6)
                << params.fft_postCallback.funcname << "(outputA, ((idy + loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
            ...
            clKernWrite(transKernel, 6)
                << params.fft_postCallback.funcname << "(outputA-"
                << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                << params.transposeMiniBatchSize << "), ((idy + loop*"
          ...
          if (params.transposeMiniBatchSize < 2)
            clKernWrite(transKernel, 6)
                ...
                << params.fft_N[0]
                << " + lidx + starting_index_yx), post_userdata, xy_s[index]";
          else {
            size_t blockOffset = params.fft_inStride[2];
            // clKernWrite(transKernel, 6) << params.fft_postCallback.funcname
            // << "(outputA-iOffset, ((lidy + loop*" << 16 / reShapeFactor <<
            // ")*" << params.fft_N[0] << " + lidx + starting_index_yx +iOffset),
            // post_userdata, xy_s[index]"; clKernWrite(transKernel, 0) <<
            // std::endl;
            clKernWrite(transKernel, 6)
                << params.fft_postCallback.funcname << "(outputA-"
                << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                << params.transposeMiniBatchSize << "), ((lidy + loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + lidx + starting_index_yx + " << blockOffset
                << "*( (get_group_id(0)/numGroupsY_1 )%"
                << params.transposeMiniBatchSize << ") "
        ...
        if (params.fft_hasPostCallback) {
          if (params.transposeMiniBatchSize <
            ...
            clKernWrite(transKernel, 6)
                ...
                << ")*" << params.fft_N[0]
                << " + idx), post_userdata, yx_s[index].x, yx_s[index].y";
          else {
            size_t blockOffset = params.fft_inStride[2];
            clKernWrite(transKernel, 6)
                << params.fft_postCallback.funcname << "(outputA_R - "
                << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                << params.transposeMiniBatchSize << "), outputA_I -"
                << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                << params.transposeMiniBatchSize << "), ((idy + loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0] << " + idx +"
                << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
          ...
          if (params.transposeMiniBatchSize <
            ...
            clKernWrite(transKernel, 6)
                ...
                << ")*" << params.fft_N[0]
                << " + lidx+ starting_index_yx), post_userdata, xy_s[index].x, "
                   "xy_s[index].y";
          else {
            size_t blockOffset = params.fft_inStride[2];
            clKernWrite(transKernel, 6)
          ...
          clKernWrite(transKernel, 6)
              << "outputA_R[(idy + loop*" << 16 / reShapeFactor << ")*"
              << params.fft_N[0] << " + idx] = yx_s[index].x;" << std::endl;
          clKernWrite(transKernel, 6)
              << "outputA_I[(idy + loop*" << 16 / reShapeFactor << ")*"
              << params.fft_N[0] << " + idx] = yx_s[index].y;" << std::endl;

          clKernWrite(transKernel, 6)
              << "outputA_R[(lidy + loop*" << 16 / reShapeFactor << ")*"
              << params.fft_N[0]
              << " + lidx+ starting_index_yx] = xy_s[index].x;" << std::endl;
          clKernWrite(transKernel, 6)
              << "outputA_I[(lidy + loop*" << 16 / reShapeFactor << ")*"
              << params.fft_N[0]
      ...
      clKernWrite(transKernel, 3) << "size_t index;" << std::endl;
      clKernWrite(transKernel, 3)
          << "if (" << params.fft_N[0] << " - (t_gx_p + 1) *"
          << 16 * reShapeFactor << ">0){" << std::endl;
      clKernWrite(transKernel, 6)
          << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
          << "; ++loop){" << std::endl;
      clKernWrite(transKernel, 9) << "index = lidy*" << 16 * reShapeFactor
                                  << " + lidx + loop*256;" << std::endl;
      ...
      case CLFFT_COMPLEX_INTERLEAVED:
        if (params.fft_hasPostCallback) {
          if (params.transposeMiniBatchSize <
            ...
            clKernWrite(transKernel, 9)
                << params.fft_postCallback.funcname
                << "(outputA - iOffset, iOffset + ((idy + loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + idx), post_userdata, yx_s[index]";
          if (params.fft_postCallback.localMemSize > 0) {
            clKernWrite(transKernel, 0) << ", localmem";
          ...
          if (params.transposeMiniBatchSize <
            ...
            clKernWrite(transKernel, 9)
                << params.fft_postCallback.funcname
                << "(outputA - iOffset, iOffset + ((lidy + loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + lidx + starting_index_yx), post_userdata, xy_s[index]";
          if (params.fft_postCallback.localMemSize > 0) {
            clKernWrite(transKernel, 0) << ", localmem";
      ...
      case CLFFT_COMPLEX_PLANAR:
        if (params.fft_hasPostCallback) {
          if (params.transposeMiniBatchSize <
            ...
            clKernWrite(transKernel, 9)
                << params.fft_postCallback.funcname
                << "(outputA_R-iOffset, outputA_I-iOffset, iOffset+((idy + "
                   "loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + idx), post_userdata, yx_s[index].x, yx_s[index].y";
          if (params.fft_postCallback.localMemSize > 0) {
          ...
          if (params.transposeMiniBatchSize <
            ...
            clKernWrite(transKernel, 9)
                << params.fft_postCallback.funcname
                << "(outputA_R-iOffset, outputA_I-iOffset, iOffset+((lidy + "
                   "loop*"
                << 16 / reShapeFactor << ")*" << params.fft_N[0]
                << " + lidx + starting_index_yx), post_userdata, "
                   "xy_s[index].x, xy_s[index].y";
...
[M0 M2 M2]
*/
clfftStatus genTransposeKernelLeadingDimensionBatched(
    const FFTGeneratedTransposeNonSquareAction::Signature &params,
    std::string &strKernel, const size_t &lwSize, const size_t reShapeFactor) {
  strKernel.reserve(4096);
  std::stringstream transKernel(std::stringstream::out);
  ...
  for (size_t bothDir = 0; bothDir < 2; bothDir++) {
    ...
    clKernWrite(transKernel, 3)
        << "iOffset += square_matrix_offset ;" << std::endl;
    ...
    clKernWrite(transKernel, 3)
        << "if (row == (float)(int)row) row -= 1; " << std::endl;
    ...
    case CLFFT_COMPLEX_INTERLEAVED:
      clKernWrite(transKernel, 3)
          << "__local " << dtInput << " xy_s["
          << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
      clKernWrite(transKernel, 3)
          << "__local " << dtInput << " yx_s["
          << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
    ...
    if (mult_of_16) {
      ...
      // If requested, generate the Twiddle math to multiply constant values
      if (params.fft_3StepTwiddle)
        genTwiddleMathLeadingDimensionBatched(params, transKernel, dtComplex,
                                              fwd);
      ...
      case CLFFT_COMPLEX_INTERLEAVED:
        if (params.fft_hasPostCallback) {
          ...
          clKernWrite(transKernel, 6)
              << params.fft_postCallback.funcname << "(outputA, ((lidy + loop*"
              << 16 / reShapeFactor << ")*" << params.fft_N[0]
              << " + lidx+ starting_index_yx), post_userdata, xy_s[index]";
          if (params.fft_postCallback.localMemSize > 0) {
            clKernWrite(transKernel, 0) << ", localmem";
      ...
      clKernWrite(transKernel, 3) << "size_t index;" << std::endl;
      clKernWrite(transKernel, 3)
          << "if (" << smaller_dim << " - (t_gx_p + 1) *" << 16 * reShapeFactor
          << ">0){" << std::endl;
      clKernWrite(transKernel, 6)
          << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
          << "; ++loop){" << std::endl;
