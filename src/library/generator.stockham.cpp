...
FFTGeneratedStockhamAction::FFTGeneratedStockhamAction(clfftPlanHandle plHandle,
                                                       FFTPlan *plan,
...
void ReadParameterFile(ParamRead &readParam) {
  ...
  while (std::getline(file, line)) {
    ...
    if (pos != std::string::npos) {
      ...
      readParam.workGroupSize = strtol(val.c_str(), nullptr, 10);
      continue;
    }
    ...
    if (pos != std::string::npos) {
      ...
      size_t numEnd = line.find_first_not_of(numbers, numStart);
      std::string val = line.substr(numStart, numEnd - numStart);
      readParam.numTransformsPerWg = strtol(val.c_str(), nullptr, 10);
      continue;
    }
    ...
    if (pos != std::string::npos) {
      ...
      while (std::string::npos != line.find_first_of(numbers)) {
        ...
        size_t numEnd = line.find_first_not_of(numbers, numStart);
        std::string val = line.substr(numStart, numEnd - numStart);
        readParam.radices.push_back(strtol(val.c_str(), nullptr, 10));
        line.erase(0, numEnd);
      }
      continue;
    }
  ...
  // std::cout << strRad << " "; for(size_t i=0; i<readParam.radices.size();
  // i++) std::cout << readParam.radices[i] << " "; std::cout << std::endl;
  // std::cout << std::endl;
}
...
template <Precision PR> class KernelCoreSpecs {
  struct SpecRecord {
    size_t length;
    size_t workGroupSize;
...
void DetermineSizes(const size_t &MAX_WGS, const size_t &length,
                    size_t &workGroupSize, size_t &numTrans, Precision &pr) {
  assert(MAX_WGS >= 64);
...
public:
  TwiddleTable(size_t length) : N(length) {
    ...
    // We compute twiddle factors in double precision for both P_SINGLE and
    // P_DOUBLE
    wc = new double[N];
    ws = new double[N];
  }
  ...
  void GenerateTwiddleTable(const std::vector<size_t> &radices,
                            std::string &twStr) {
    const double TWO_PI = -6.283185307179586476925286766559;

    // Make sure the radices vector sums up to N
    size_t sz = 1;
    for (unsigned long radice : radices) {
      sz *= radice;
    }
    assert(sz == N);
    ...
    size_t L = 1;
    size_t nt = 0;
    for (unsigned long radix : radices) {
      L *= radix;
...
template <Precision PR> class Pass {
  ...
  size_t workGroupSize; // size of the workgroup = (length / cnPerWI)
                        // this number is essentially number of work-items
                        // needed to compute 1 transform this number will be
                        // different from the kernel class workGroupSize if
                        // there are multiple transforms per workgroup
...
#define SR_COMP_BOTH 2 // real & imag
  ...
  // operations: reading, twiddle multiplication, writing
  void SweepRegs(size_t flag, bool fwd, bool interleaved, size_t stride,
                 ...
                 size_t numPrev, std::string &passStr, bool initZero = false,
    ...
    if (numB && (numB % 2 == 0) && (regC == 1) && (stride == 1) &&
        ...
        (nextPass == nullptr) && interleaved && (component == SR_COMP_BOTH) &&
        linearRegs && enableGrouping && !fft_doPostCallback) {
      assert((numButterfly * workGroupSize) == algLS);
      assert(bufferRe.compare(bufferIm) ==
             0); // Make sure Real & Imag buffer strings are same for
                 // interleaved data

      passStr += "\n\t";
      passStr += "__global ";
      passStr += RegBaseType<PR>(4);
      passStr += " *buff4g = (__global ";
      passStr += RegBaseType<PR>(4);
      passStr += " *)";
      passStr += bufferRe;
        ...
        for (size_t i = 0; i < numB; i++) {
               ...
               c++) // component loop: 0 - real, 1 - imaginary
          {
            ...
            if (fft_doPreCallback && c == 0 && component == SR_COMP_BOTH) {
              ...
              passStr += fft_preCallback.funcname;
              passStr += "(";
              if (interleaved) {
                passStr += buffer;
                passStr += ", ";
    ...
    if (linearRegs && (flag == SR_WRITE) && (nextPass == nullptr)) {
      for (size_t r = 0; r < radix; r++) {
        butterflyIndex = numPrev;

        for (size_t i = 0; i < numB; i++) {
          if (realSpecial && (nextPass == nullptr) && (r > (radix / 2)))
            break;

          if (realSpecial && (nextPass == nullptr) && (r == radix / 2) && (i != 0))
            break;
               ...
               c++) // component loop: 0 - real, 1 - imaginary
          {
            ...
            if (fft_doPostCallback && !r2c) {
              if (interleaved || c == (cEnd - 1)) {
                ...
                passStr += ", ";
                passStr += bufOffset;
                passStr += ", post_userdata, (";
                passStr += regIndexC0;
                passStr += ")";
                if (!(interleaved ||
                      (c2r && bufferRe.compare(bufferIm) == 0))) {
                  passStr += ", (";
                  passStr += regIndex;
                  passStr += ")";
                }
        ...
        for (size_t r = 0; r < radix; r++) {
               ...
               c++) // component loop: 0 - real, 1 - imaginary
          {
            ...
            for (size_t v = 0; v < regC;
                 v++) // TODO: vectorize the reads; instead of reading
                      // individually for consecutive reads of vector elements
            {
                                           ...
                                           // require that 'r' loop be innermost
      {
        for (size_t v = 0; v < regC; v++) {
          for (size_t r = 0; r < radix; r++) {
            ...
            if (regC != 1) {
              regRealIndex += ".s";
              regRealIndex += SztToStr(v);
              regImagIndex += ".s";
              regImagIndex += SztToStr(v);
            }
  ...
  void SweepRegsRC(size_t flag, bool fwd, bool interleaved, size_t stride,
    ...
    assert(r2c || c2r);
    assert(linearRegs);
    bool singlePass = ((position == 0) && (nextPass == nullptr));
          ...
          if (setZero) {
            ...
            if (act) {
              ...
              if (fft_doPreCallback) {
                passStr += ", pre_userdata";
                passStr +=
                    (fft_preCallback.localMemSize > 0) ? ", localmem);" : ");";
              } else {
                passStr += "]";
          ...
          if (!cid) {
            ...
            if (fwd) {
              ...
              if (fft_doPostCallback && !rcFull) {
                if (interleaved) {
                  val1Str += "\n\t";
                  val1Str += fft_postCallback.funcname;
                  val1Str += "(";
                  val1Str += buffer;
                  val1Str += ", ";
                  val1Str += offset;
                  ...
                  val1StrExt += "\n\t";
                  val1StrExt += fft_postCallback.funcname;
                  val1StrExt += "(";
                  val1StrExt += bufferRe;
                  val1StrExt += ", ";
                  val1StrExt += bufferIm;
                  ...
                  val1StrExt += idxStr;
                  val1StrExt += " )*";
                  val1StrExt += SztToStr(stride);
                  val1StrExt += ", post_userdata, ";
                }
              } else {
                val1Str += "\n\t";
                val1Str += buffer;
...
public:
  Pass(size_t positionVal, size_t lengthVal, size_t radixVal, size_t cnPerWIVal,
      ...
      : position(positionVal), length(lengthVal), radix(radixVal),
        ...
        numB4(0), nextPass(nullptr), fft_doPreCallback(false),
  ...
  void SetPrecallback(bool hasPrecallback,
                      clfftCallbackParam precallbackParam) {
    fft_doPreCallback = hasPrecallback;
    fft_preCallback = precallbackParam;
  }

  void SetPostcallback(bool hasPostcallback,
  ...
  void GeneratePass(bool fwd, std::string &passStr, bool fft_3StepTwiddle,
                    ...
                    size_t outStride, double scale, bool gIn = false,
    ...
    if (!rcSimple && oddp &&
        ((r2c && (nextPass == nullptr)) || (c2r && (position == 0)))) {
      passStr += "\n\t";
      passStr += "uint brv = 0;\n\t";
      passStr += "\n\t";
      passStr += regB2Type;
    ...
    if (r2c) {
        ...
        if (rcSimple) {
          ...
          if (fft_doPreCallback) {
            SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG, 1.0f,
                      false, bufferInRe2, bufferInIm2, "inOffset2", 1, numB1, 0,
                      passStr);
          } else {
            SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG, 1.0f,
      ...
      if (position == 0) {
        ...
        if (length > 1) {
          ...
          if (fft_doPreCallback && !inInterleaved) {
            ...
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL,
                        1.0f, false, false, false, bufferInRe, bufferInRe,
                        "inOffset", passStr);
          }
          passStr += "\n\t}\n";
          ...
          if (fft_doPreCallback) {
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL,
                        1.0f, false, true, false, bufferInRe2, bufferInIm2,
                        "inOffset2", passStr);
          } else {
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL,
                        1.0f, false, true, false, bufferInIm2, bufferInIm2,
                        "inOffset", passStr);
          }
          ...
          if (oddp) {
            ...
            if (fft_doPreCallback) {
              SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL,
                          1.0f, false, true, true, bufferInRe2, bufferInIm2,
                          "inOffset2", passStr);
            } else {
              SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL,
        ...
        SweepRegs(SR_READ, fwd, outInterleaved, processBufStride, SR_COMP_REAL,
                  ...
                  numB1, 0, passStr, false, false, oddp);
        passStr += "\n\n\tbarrier(CLK_LOCAL_MEM_FENCE);\n";
      ...
      if ((!halfLds) || (halfLds && (position == 0))) {
        ...
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 1, numB1, 0,
                  passStr, false, isPrecallVector);
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 2, numB2, numB1,
                  passStr, false, isPrecallVector);
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 4, numB4,
                  2 * numB2 + numB1, passStr, false, isPrecallVector);
        passStr += "\n\t}\n";
        passStr += "\n\telse\n\t{";
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 1, numB1, 0,
                  passStr, true, isPrecallVector);
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 2, numB2, numB1,
                  passStr, true, isPrecallVector);
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 4, numB4,
                  2 * numB2 + numB1, passStr, true, isPrecallVector);
        passStr += "\n\t}\n";
      }
    }
    ...
    if (halfLds) {
      ...
      // since we are using only half the lds. Number of barriers will increase
      // at the cost of halving the lds.

      if (nextPass == nullptr) // last pass
      {
        if (r2c && !rcSimple) {
          if (!singlePass) {
            ...
            if (outInterleaved) {
              if (fft_doPostCallback) {
                passStr += fft_postCallback.funcname;
                passStr += "(bufOut, outOffset, post_userdata, ";
                passStr += "(";
              ...
              if (fft_doPostCallback) {
                ...
                passStr += ", ";
                passStr += bufferOutIm;
                passStr += ", outOffset, post_userdata, ";
                passStr += bufferInRe;
                passStr += "[inOffset]";
                if (scale != 1.0) {
                  passStr += " * ";
            ...
            if (outInterleaved) {
              ...
              if (fft_doPostCallback) {
                ...
                passStr += ", ";
                passStr += bufferOutIm2;
                passStr += ", outOffset2, post_userdata, ";
                passStr += bufferInIm;
                passStr += "[inOffset]";
                if (scale != 1.0) {
                  passStr += " * ";
...
// FFT kernel
template <Precision PR> class Kernel {
  size_t length;        // Length of FFT
  ...
  bool halfLds; // LDS used to store one component (either real or imaginary) at
                // a time for passing intermediate data between the passes, if
                // this is set then each pass-function should accept same set of
                // registers

  bool linearRegs; // scalar registers

  // Future optimization ideas
  // bool limitRegs;							// TODO: Incrementally write to LDS,
  // thereby using same set of registers for more than 1 butterflies bool
  // combineReadTwMul;					// TODO: Combine reading into registers and
  // Twiddle multiply
...
public:
...

  {
    ...
    if (params.fft_inputLayout == CLFFT_REAL)
      r2c = true;
    if (params.fft_outputLayout == CLFFT_REAL)
      c2r = true;
    r2c2r = (r2c || c2r);

    if (r2c) {
      rcFull = ((params.fft_outputLayout == CLFFT_COMPLEX_INTERLEAVED) ||
                (params.fft_outputLayout == CLFFT_COMPLEX_PLANAR))
                   ? true
                   : false;
    }
    if (c2r) {
      rcFull = ((params.fft_inputLayout == CLFFT_COMPLEX_INTERLEAVED) ||
                (params.fft_inputLayout == CLFFT_COMPLEX_PLANAR))
                   ? true
                   : false;
    }
    ...
    if (blockCompute) {
      assert(length <= 256); // 256 parameter comes from prototype experiments
                             // largest length at which block column possible
                             // given 32KB LDS limit if LDS limit is different
                             // this number need to be changed appropriately
      halfLds = false;
      linearRegs = true;
    }
    ...
    const size_t *pRadices = nullptr;
    size_t nPasses;
    KernelCoreSpecs<PR> kcs;
    kcs.GetRadices(length, nPasses, pRadices);
    if ((params.fft_MaxWorkGroupSize >= 256) && (pRadices != nullptr)) {
  ...
  void GenerateKernel(std::string &str, cl_device_id Dev_ID) {
    ...
    outInterleaved = ((params.fft_outputLayout == CLFFT_COMPLEX_INTERLEAVED) ||
                      (params.fft_outputLayout == CLFFT_HERMITIAN_INTERLEAVED))
                         ? true
                         : false;
    ...
    for (size_t d = 0; d < 2; d++) {
      ...
      for (p = passes.begin(); p != passes.end(); p++) {
        ...
        p->GeneratePass(fwd, str, tw3Step, params.fft_twiddleFront, inIlvd,
                        outIlvd, inRl, outRl, ins, outs, s, gIn, gOut);
      }
    ...
    for (size_t d = 0; d < 2; d++) {
      ...
      clGetDeviceInfo(Dev_ID, CL_DEVICE_VENDOR, 0, nullptr, &SizeParam_ret);
      char *nameVendor = new char[SizeParam_ret];
      clGetDeviceInfo(Dev_ID, CL_DEVICE_VENDOR, SizeParam_ret, nameVendor,
                      nullptr);
      ...
      callbackstr.clear();
      bool hasCallback =
          params.fft_hasPreCallback || params.fft_hasPostCallback;
      ...
      if (hasCallback) {
        ...
        if (params.fft_preCallback.localMemSize > 0 ||
            params.fft_postCallback.localMemSize > 0) {
      ...
      if (params.fft_placeness == CLFFT_INPLACE) {
        ...
        if (r2c2r) {
          ...
          if (outInterleaved) {
            ...
            str += "__global ";
            str += rType;
            str += " * restrict gbOutRe, ";
            str += "__global ";
            str += rType;
            str += " * restrict gbOutIm";
          }
        } else {
          if (inInterleaved) {
            str += "__global const ";
      ...
      if (blockCompute) {
        ...
        for (size_t c = 0; c < 2; c++) {
          std::string comp = "";
          std::string readBuf =
              (params.fft_placeness == CLFFT_INPLACE) ? "lwb" : "lwbIn";
          if (!inInterleaved)
            comp = c ? ".y" : ".x";
          if (!inInterleaved)
            readBuf = (params.fft_placeness == CLFFT_INPLACE)
                          ? (c ? "lwbIm" : "lwbRe")
                          : (c ? "lwbInIm" : "lwbInRe");
          ...
          if ((blockComputeType == BCT_C2C) || (blockComputeType == BCT_C2R)) {
            ...
            if ((blockComputeType == BCT_C2C) && params.fft_hasPreCallback) {
              if (c == 0) {
                str += "\t\tretCallback = ";
                str += params.fft_preCallback.funcname;
                str += "(";

                if (inInterleaved) {
                  str += (params.fft_placeness == CLFFT_INPLACE) ? "gb, "
                                                                 : "gbIn, ";
                } else {
                  str += (params.fft_placeness == CLFFT_INPLACE)
                             ? "gbRe, gbIm, "
                             : "gbInRe, gbInIm, ";
                }
                ...
                str += bufOffset;
                str += ", pre_userdata";
                str += (params.fft_preCallback.localMemSize > 0)
                           ? str += ", localmem);\n"
                           : ");\n";
              }
      ...
      if (r2c2r) {
        if (rcSimple) {
          ...
          if (inInterleaved || inReal) {
            ...
            inBuf = (params.fft_hasPreCallback)
                        ? "gbInRe, gbInRe, gbInIm, gbInIm, "
                        : "lwbInRe, lwbInRe2, lwbInIm, lwbInIm2, ";

          if (outInterleaved || outReal)
            outBuf =
                params.fft_hasPostCallback
                    ? ((params.fft_placeness == CLFFT_INPLACE) ? "gb, gb"
                                                               : "gbOut, gbOut")
                    : "lwbOut, lwbOut2";
          else
            outBuf = params.fft_hasPostCallback
                         ? "gbOutRe, gbOutRe, gbOutIm, gbOutIm"
                         : "lwbOutRe, lwbOutRe2, lwbOutIm, lwbOutIm2";
        }
      } else {
        if (params.fft_placeness == CLFFT_INPLACE) {
          if (inInterleaved) {
      ...
      if (numPasses == 1) {
        ...
        if (hasCallback) {
          // if pre-calback set
          if (params.fft_hasPreCallback) {
            str += (r2c2r && !rcSimple) ? ", iOffset2, pre_userdata"
                                        : ", pre_userdata";
          }
          ...
          if (params.fft_postCallback.localMemSize > 0) {
            // if precallback localmem also requested, send the localmem with
            // the right offset
            if (params.fft_hasPreCallback &&
                params.fft_preCallback.localMemSize > 0) {
              str += ", ((__local char *)localmem + ";
              str += SztToStr(params.fft_preCallback.localMemSize);
              str += ")";
            } else {
              str += ", localmem";
            }
      ...
      if (blockCompute) {
        ...
        for (size_t c = 0; c < 2; c++) {
          std::string comp = "";
          std::string writeBuf =
              (params.fft_placeness == CLFFT_INPLACE) ? "lwb" : "lwbOut";
          if (!outInterleaved)
            comp = c ? ".y" : ".x";
          if (!outInterleaved)
            writeBuf = (params.fft_placeness == CLFFT_INPLACE)
                           ? (c ? "lwbIm" : "lwbRe")
                           : (c ? "lwbOutIm" : "lwbOutRe");
          ...
          if ((blockComputeType == BCT_C2C) || (blockComputeType == BCT_R2C)) {
            if (blockComputeType == BCT_R2C && params.fft_hasPostCallback) {
              if (outInterleaved)
                writeBuf =
                    (params.fft_placeness == CLFFT_INPLACE) ? "gb" : "gbOut";
              else
                writeBuf = (params.fft_placeness == CLFFT_INPLACE)
                               ? "gbRe, gbIm"
                               : "gbOutRe, gbOutIm";
              ...
              str += "\t\t";
              str += params.fft_postCallback.funcname;
              str += "(";
              str += writeBuf;
              str += ", (";
              str += outOffset;
              ...
              // in the planar case, break from for loop since both real and
              // imag components are handled together in post-callback
              if (!outInterleaved)
                break;
            } else {
              str += "\t\t";
...
clfftStatus FFTGeneratedStockhamAction::initParams() {
  ...
  //    devices.
  //
  const FFTEnvelope *pEnvelope = nullptr;
  OPENCL_V(this->plan->GetEnvelope(&pEnvelope), _T("GetEnvelope failed"));
  BUG_CHECK(nullptr != pEnvelope);
  ...
  this->signature.fft_MaxWorkGroupSize =
      this->plan->envelope.limit_WorkGroupSize;
  ...
  bool real_transform = ((this->plan->inputLayout == CLFFT_REAL) ||
                         (this->plan->outputLayout == CLFFT_REAL));
  ...
  size_t t_wgs, t_nt;
  Precision pr =
      (this->signature.fft_precision == CLFFT_SINGLE) ? P_SINGLE : P_DOUBLE;
  switch (pr) {
  case P_SINGLE: {
    KernelCoreSpecs<P_SINGLE> kcs;
    kcs.GetWGSAndNT(this->signature.fft_N[0], t_wgs, t_nt);
    if (this->signature.blockCompute) {
      this->signature.blockSIMD =
          Kernel<P_SINGLE>::BlockSizes::BlockWorkGroupSize(
              this->signature.fft_N[0]);
      this->signature.blockLDS =
          Kernel<P_SINGLE>::BlockSizes::BlockLdsSize(this->signature.fft_N[0]);
    }
  } break;
  case P_DOUBLE: {
    ...
    if (this->signature.blockCompute) {
      this->signature.blockSIMD =
          Kernel<P_DOUBLE>::BlockSizes::BlockWorkGroupSize(
              this->signature.fft_N[0]);
      this->signature.blockLDS =
          Kernel<P_DOUBLE>::BlockSizes::BlockLdsSize(this->signature.fft_N[0]);
    }
  } break;
  }

  if ((t_wgs != 0) && (t_nt != 0) &&
      (this->plan->envelope.limit_WorkGroupSize >= 256)) {
    wgs = t_wgs;
    nt = t_nt;
  } else
    DetermineSizes(this->plan->envelope.limit_WorkGroupSize,
                   this->signature.fft_N[0], wgs, nt, pr);
#endif
  ...
  if (this->plan->large1D != 0) {
    ...
    if (!(this->plan->realSpecial))
      ARG_CHECK(this->plan->large1D ==
                (this->signature.fft_N[1] * this->signature.fft_N[0]));
  }
...
clfftStatus
FFTGeneratedStockhamAction::getWorkSizes(std::vector<size_t> &globalWS,
                                         std::vector<size_t> &localWS) {
  //    How many complex numbers in the input mutl-dimensional array?
  //
  unsigned long long count = 1;
  for (unsigned long & u : this->plan->length) {
    count *= std::max<size_t>(1, u);
  }
  ...
  count = DivRoundingUp<unsigned long long>(
      count, this->signature.fft_R); // count of WorkItems
  count = DivRoundingUp<unsigned long long>(
      count, this->signature.fft_SIMD); // count of WorkGroups

  // for real transforms we only need half the work groups since we do twice the
  // work in 1 work group
  if (!(this->signature.fft_RCsimple) &&
      ((this->signature.fft_inputLayout == CLFFT_REAL) ||
       (this->signature.fft_outputLayout == CLFFT_REAL)))
...
clfftStatus FFTGeneratedStockhamAction::generateKernel(
    FFTRepo &fftRepo, const cl_command_queue commQueueFFT) {
  cl_int status = CL_SUCCESS;
  cl_device_id Device = nullptr;
  status = clGetCommandQueueInfo(commQueueFFT, CL_QUEUE_DEVICE,
                                 sizeof(cl_device_id), &Device, nullptr);
  OPENCL_V(status, _T( "clGetCommandQueueInfo failed" ));

  cl_context QueueContext = nullptr;
  status = clGetCommandQueueInfo(commQueueFFT, CL_QUEUE_CONTEXT,
                                 sizeof(cl_context), &QueueContext, nullptr);
  OPENCL_V(status, _T( "clGetCommandQueueInfo failed" ));
  ...
  if ((this->signature.fft_hasPreCallback &&
       this->signature.fft_preCallback.localMemSize > 0) ||
      (this->signature.fft_hasPostCallback &&
       this->signature.fft_postCallback.localMemSize > 0)) {
    ...
    if (this->signature.fft_hasPostCallback &&
        this->signature.fft_postCallback.localMemSize > 0)
      requestedCallbackLDS += this->signature.fft_postCallback.localMemSize;

    if (this->plan->blockCompute) {
      validLDSSize =
          ((this->signature.blockLDS * this->plan->ElementSize()) +
           requestedCallbackLDS) < this->plan->envelope.limit_LocalMemSize;
    } else {
      ...
      // TODO - Need to abstract this out. Repeating the same compute as in
      // GenerateKernel.
      // Set half lds only for power-of-2 problem sizes & interleaved data
      bool halfLds =
          ((this->signature.fft_inputLayout == CLFFT_COMPLEX_INTERLEAVED) &&
           (this->signature.fft_outputLayout == CLFFT_COMPLEX_INTERLEAVED))
              ? true
              : false;
      ...
      size_t ldsSize = halfLds ? length * numTrans : 2 * length * numTrans;
      size_t elementSize =
          ((this->signature.fft_precision == CLFFT_DOUBLE) ||
           (this->signature.fft_precision == CLFFT_DOUBLE_FAST))
              ? sizeof(double)
              : sizeof(float);

      validLDSSize = ((ldsSize * elementSize) + requestedCallbackLDS) <
                     this->plan->envelope.limit_LocalMemSize;
    }
  ...
  OPENCL_V(fftRepo.setProgramCode(this->getGenerator(),
                                  this->getSignatureData(), programCode, Device,
                                  QueueContext),
           _T( "fftRepo.setclString() failed!" ));
  OPENCL_V(fftRepo.setProgramEntryPoints(this->getGenerator(),
                                         this->getSignatureData(), "fft_fwd",
                                         "fft_back", Device, QueueContext),
           _T( "fftRepo.setProgramEntryPoint() failed!" ));
