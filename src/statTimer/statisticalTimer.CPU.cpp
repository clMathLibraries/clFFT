...
template <typename RangeType, typename ValType> struct PruneRange {
...
CpuStatTimer &CpuStatTimer::getInstance() {
...
CpuStatTimer::CpuStatTimer() : nEvents(0), nSamples(0), normalize(true) {
  ...
  //	If the function fails (monotonic clock not supported), we default to a
...
void CpuStatTimer::Clear() {
  labelID.clear();
  clkStart.clear();
  clkTicks.clear();
}

void CpuStatTimer::Reset() {
...
void CpuStatTimer::Reserve(size_t nEvents, size_t nSamples) {
...
void CpuStatTimer::setNormalize(bool norm) { normalize = norm; }

void CpuStatTimer::Start(size_t id) {
#if defined(_WIN32)
  ::QueryPerformanceCounter(
      reinterpret_cast<LARGE_INTEGER *>(&clkStart.at(id)));
#else
  if (clkFrequency) {
    struct timeval s;
...
void CpuStatTimer::Stop(size_t id) {
  cl_ulong n;
...
void CpuStatTimer::AddSample(const size_t id, const cl_ulong n) {
...
size_t CpuStatTimer::getUniqueID(const std::string &label, cl_uint groupID) {
...
cl_double CpuStatTimer::getMean(size_t id) const {
  if (clkTicks.empty())
    return 0;

  size_t N = clkTicks.at(id).size();

  Accumulator<cl_ulong> sum = std::for_each(
      clkTicks.at(id).begin(), clkTicks.at(id).end(), Accumulator<cl_ulong>());

  return static_cast<cl_double>(sum.acc) / N;
}

cl_double CpuStatTimer::getVariance(size_t id) const {
  if (clkTicks.empty())
    return 0;
...
cl_double CpuStatTimer::getStdDev(size_t id) const {
  cl_double variance = getVariance(id);

  return sqrt(variance);
}

cl_double CpuStatTimer::getAverageTime(size_t id) const {
  if (normalize)
...
cl_double CpuStatTimer::getMinimumTime(size_t id) const {
  auto iter =
...
std::vector<size_t> CpuStatTimer::pruneOutliers(size_t id, cl_double multiple) {
  // if( clkTicks.empty( ) )
  ...
  // clkVector::iterator	newEnd	= std::remove_if( clks.begin( ), clks.end(
  // ), PruneRange< cl_double,cl_ulong >( mean, multiple*stdDev ) );
...
size_t CpuStatTimer::pruneOutliers(cl_double multiple) {
  size_t tCount = 0;
  ...
  //" << labelID[l].first << std::endl; 	tCount += lCount;
  //}

  return tCount;
}

void CpuStatTimer::Print() {
  // double flops = fFunc( );
  ...
  //	tout << std::setw( 10 ) << "Time:" << std::setw( 10 )  << commatize(
  //static_cast< cl_ulong >( timeNs ) )
  //		<< " ns" << std::endl;
  //	tout << std::setw( 10 ) << "Gflops:" << std::setw( 10 ) << gFlops <<
  //std::endl;
  //}
}

//	Defining an output print operator
std::ostream &operator<<(std::ostream &os, const CpuStatTimer &st) {
