...
struct StatData {
  ...
  StatData(clfftPlanHandle id, FFTPlan *plan, cl_kernel kern, cl_uint nEv,
           cl_event *Ev, const std::vector<size_t> &gWorkSize,
           const std::vector<size_t> &lWorkSize)
      : deltaNanoSec(0), kernel(kern), batchSize(plan->batchsize),
        dim(plan->dim), plHandle(id), planX(plan->planX), planY(plan->planY),
        planZ(plan->planZ), planTX(plan->planTX), planTY(plan->planTY),
        planTZ(plan->planTZ), planRCcopy(plan->planRCcopy),
        planCopy(plan->planCopy), gen(plan->gen), inStride(plan->inStride),
        outStride(plan->outStride), iDist(plan->iDist), oDist(plan->oDist),
        lengths(plan->length), enqueueWorkSize(gWorkSize),
        enqueueLocalWorkSize(lWorkSize), placeness(plan->placeness) {
  ...
  double calcFlops() {
    ...
    double flops = cumulativeBatch * 5 * fftLength *
                   (log(static_cast<double>(fftLength)) / log(2.0));
...
class GpuStatTimer : public baseStatTimer {
  ...
  //	Between each Start/Stop pair, we need to count how many AddSamples were
  //made.
   ...
   *Instead, you must go through getInstance( ) to get a reference to the class.
   */
  GpuStatTimer();
...
public:
   ...
   */
  virtual void AddSample(clfftPlanHandle plHandle, FFTPlan *plan,
                         cl_kernel kern, cl_uint numQueuesAndEvents,
                         cl_event *ev, const std::vector<size_t> &gWorkSize,
                         const std::vector<size_t> &lWorkSize);
  ...
  void Reserve(size_t nEvents, size_t nSamples) override ;

  size_t getUniqueID(const std::string &label,
                     cl_uint groupID) override ;

  //	Calculate the average/mean of data for a given event
  void setNormalize(bool norm) override ;

  void Print() override ;

  //	Using the stdDev of the entire population (of an id), eliminate those
  //samples that fall 	outside some specified multiple of the stdDev.  This
  //assumes that the population 	form a gaussian curve.
  size_t pruneOutliers(cl_double multiple) override ;
  std::vector<size_t> pruneOutliers(size_t id,
                                    cl_double multiple) override ;
};
