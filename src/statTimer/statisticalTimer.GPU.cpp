...
template <typename T> struct Accumulator : public std::unary_function<T, void> {
  T acc;
...
//	Functor object to help with accumulating values in vectors
template <> struct Accumulator<StatData> {
  StatData acc;

  Accumulator() {}
  void operator()(const StatData &x) { acc.deltaNanoSec += x.deltaNanoSec; }
};
...
template <> struct PruneRange<StatData, cl_double> {
  StatData mean;
  cl_double stdDev;
...
GpuStatTimer &GpuStatTimer::getInstance() {
  static GpuStatTimer timer;
  return timer;
}

GpuStatTimer::GpuStatTimer()
    : nEvents(0), nSamples(0), currID(0), currSample(0), currRecord(0) {}

GpuStatTimer::~GpuStatTimer() {}

void GpuStatTimer::Clear() {
  labelID.clear();
  timerData.clear();
...
//will think 	they will need, and nSamples is a hint on how many samples we think
//we will take 	per event
void GpuStatTimer::Reserve(size_t nE, size_t nS) {
  Clear();
...
void GpuStatTimer::setNormalize(bool norm) {}

void GpuStatTimer::Start(size_t id) {
  currID = id;
  currSample = 0;
}

void GpuStatTimer::Stop(size_t id) { ++currRecord; }

void GpuStatTimer::AddSample(clfftPlanHandle plHandle, FFTPlan *plan,
                             cl_kernel kern, cl_uint numEvents, cl_event *ev,
                             const std::vector<size_t> &gWorkSize,
                             const std::vector<size_t> &lWorkSize) {
  if ((numEvents != 0) && (ev == nullptr))
    return;
  ...
  if (currRecord == 0) {
    ...
    timerData.at(currID).back().push_back(
        StatData(plHandle, plan, kern, numEvents, ev, gWorkSize, lWorkSize));
  } else {
    timerData.at(currID)
        .at(currSample)
...
//readable text string 	to an index into internal data structures.
size_t GpuStatTimer::getUniqueID(const std::string &label, cl_uint groupID) {
  ...
  for (cl_uint id = 0; id < labelID.size(); ++id) {
    for (size_t s = 0; s < timerData.at(id).size(); ++s) {
      for (size_t n = 0; n < timerData.at(id).at(s).size(); ++n) {
        ...
        for (auto & outEvent : sd.outEvents) {
    ...
    for (size_t n = 0; n < timerData.at(id).at(s).size(); ++n) {
      ...
      for (size_t i = 0; i < sd.outEvents.size(); ++i) {
        if (::clGetEventProfilingInfo(
                sd.outEvents[i], CL_PROFILING_COMMAND_START, sizeof(cl_ulong),
                &profStart, nullptr) != CL_SUCCESS) {
          profStart = 0;
        }

        if (::clGetEventProfilingInfo(sd.outEvents[i], CL_PROFILING_COMMAND_END,
                                      sizeof(cl_ulong), &profEnd,
                                      nullptr) != CL_SUCCESS) {
          profEnd = 0;
        }
...
std::vector<StatData> GpuStatTimer::getMean(size_t id) {
  //	Prep the data; query openCL for the timer information
  queryOpenCL(id);

  std::vector<StatData> meanVec;
  for (size_t s = 0; s < timerData.at(id).size(); ++s) {
    Accumulator<StatData> sum =
        std::for_each(timerData.at(id).at(s).begin(),
                      timerData.at(id).at(s).end(), Accumulator<StatData>());

    StatData tmp = timerData[id][s].front();
    tmp.doubleNanoSec = static_cast<cl_double>(sum.acc.deltaNanoSec) /
                        timerData.at(id).at(s).size();
...
std::vector<StatData> GpuStatTimer::getVariance(size_t id) {
  ...
  for (cl_uint v = 0; v < variance.size(); ++v) {
    ...
    for (cl_uint n = 0; n < timerData[id][v].size(); ++n) {
      cl_double diff =
          static_cast<cl_double>(timerData[id][v][n].deltaNanoSec) -
          variance[v].doubleNanoSec;
      diff *= diff;
      sum += diff;
    }
...
std::vector<StatData> GpuStatTimer::getStdDev(size_t id) {
  std::vector<StatData> stddev = getVariance(id);

  for (auto & v : stddev) {
    v.doubleNanoSec = sqrt(v.doubleNanoSec);
  }

  return stddev;
}

std::vector<StatData> GpuStatTimer::getAverageTime(size_t id) {
  return getMean(id);
}

std::vector<StatData> GpuStatTimer::getMinimumTime(size_t id) {
  //	Prep the data; query openCL for the timer information
  queryOpenCL(id);

  std::vector<StatData> minTime;
  for (auto & s : timerData.at(id)) {
    auto iter = std::min_element(
        s.begin(), s.end());

    if (iter != s.end()) {
      iter->doubleNanoSec = static_cast<cl_double>(iter->deltaNanoSec) /
                            s.size();
...
std::vector<size_t> GpuStatTimer::pruneOutliers(size_t id, cl_double multiple) {
  ...
  for (size_t s = 0; s < timerData.at(id).size(); ++s) {
    ...
    auto newEnd = std::remove_if(
        timerData.at(id).at(s).begin(), timerData.at(id).at(s).end(),
        PruneRange<StatData, cl_double>(mean[s],
                                        multiple * stdDev[s].doubleNanoSec));

    StatDataVec::difference_type dist =
        std::distance(newEnd, timerData.at(id).at(s).end());
...
size_t GpuStatTimer::pruneOutliers(cl_double multiple) {
  ...
  std::cout << std::endl;
  std::cout << std::setfill('=') << std::setw(tableHalf) << header << " ( "
            << multiple << " )" << std::setw(tableHalf - sizeTitle) << "="
            << std::endl;
  tout << std::setfill(_T(' '));
  ...
  for (cl_uint l = 0; l < labelID.size(); ++l) {
    ...
    for (cl_uint c = 0; c < lCount.size(); ++c) {
      std::cout << labelID[l].first << "[ " << c << " ]"
                << ": Pruning " << lCount[c] << " samples out of " << currRecord
                << std::endl;
      tCount += lCount[c];
    }
  }
...
void GpuStatTimer::Print() {
  ...
  for (cl_uint id = 0; id < labelID.size(); ++id) {
    ...
    //	Print label of timer, in a header
    std::cout << std::endl
              << std::setw(tableHalf + halfString) << std::setfill('=')
              << labelID[id].first << std::setw(tableHalf - halfString) << "="
              << std::endl;
    tout << std::setfill(_T(' '));
    ...
    for (cl_uint t = 0; t < mean.size(); ++t) {
      ...
      tout << std::setw(tableFourth) << _T( "Handle:" ) << std::setw(tableThird)
           << mean[t].plHandle << std::endl;

      if (mean[t].kernel != nullptr) {
        tout << std::setw(tableFourth) << _T( "Kernel:" )
             << std::setw(tableThird)
             << reinterpret_cast<void *>(mean[t].kernel) << std::endl;
      }
      ...
      if (mean[t].outEvents.size() != 0) {
        tout << std::setw(tableFourth) << _T( "OutEvents:" )
             << std::setw(tableThird);
      ...
      switch (mean[t].gen) {
      case Stockham:
        tout << std::setw(tableThird) << _T("Stockham");
        break;
      case Transpose_GCN:
        tout << std::setw(tableThird) << _T("Transpose_GCN");
        break;
      case Transpose_SQUARE:
        tout << std::setw(tableThird) << _T("Transpose_SQUARE");
        break;
      case Transpose_NONSQUARE:
        tout << std::setw(tableThird) << _T("Transpose_NONSQUARE");
        break;
      case Copy:
        tout << std::setw(tableThird) << _T("Copy");
        break;
      }
      tout << std::endl;
      ...
      tout << std::setw(tableFourth) << _T("Placeness:")
           << std::setw(tableThird)
           << (mean[t].placeness == CLFFT_INPLACE ? "InPlace" : "OutOfPlace")
           << std::endl;

      tout << std::setw(tableFourth) << _T("Input Dist:")
           << std::setw(tableThird) << mean[t].iDist << std::endl;
      tout << std::setw(tableFourth) << _T("Output Dist:")
           << std::setw(tableThird) << mean[t].oDist << std::endl;
      ...
      tout << std::setw(tableFourth) << _T( "Time (ns):" )
           << std::setw(3 * tableFourth)
           << commatize(static_cast<cl_ulong>(time)) << std::endl;
      tout << std::endl;
    }
  }
}
...
std::ostream &operator<<(std::ostream &os, const GpuStatTimer &st) {
  ...
  //{
  //	cl_ulong min	= 0;
  //	clkVector::const_iterator iter	= std::min_element( st.clkTicks.at( l
  //).begin( ), st.clkTicks.at( l ).end( ) );

  //	if( iter != st.clkTicks.at( l ).end( ) )
  //		min		= *iter;

  //	os << st.labelID[l].first << ", " << st.labelID[l].second << std::fixed
  //<< std::endl; 	os << "Min:," << min << std::endl; 	os << "Mean:," <<
  //st.getMean( l ) << std::endl; 	os << "StdDev:," << st.getStdDev( l ) <<
  //std::endl; 	os << "AvgTime:," << st.getAverageTime( l ) << std::endl; 	os <<
  //"MinTime:," << st.getMinimumTime( l ) << std::endl;
